...
USAGE
    ...
    # a newline built by printf, so an indented copy of this script (the macOS
    # launcher is a heredoc) still holds exactly one newline and nothing after it
    NL=$(printf '\nx')
    NL=${NL%x}
    NMOX_REQ=
    nmox_request() {
    ...
    nmox_path() {
        # the RAW name: command substitution below strips trailing newlines, so
        # 'z<newline>' would otherwise become a different file, z
        case "$1" in *"$NL"*)
            printf 'nmox: a name holding a newline cannot be handed over\n' >&2
            exit 2 ;;
        esac
    ...
    while [ "$n" -gt 0 ]; do
        ...
        if [ "$diff" -gt 0 ]; then
            ...
            # that side is handed over as nothing
            if [ "$a" = /dev/null ]; then
                np=
            elif [ ! -f "$a" ]; then
            ...
            if [ "$diff" = 2 ]; then
                ...
                printf 'nmox: -d needs at least one file\n' >&2
                exit 2
            else
