...
await step("tools/call search_text for a value in .env (expect no hits from .env)", async () => { const r = await client.callTool({ name: "search_text", arguments: { query: "=" } }); const files = (r.structuredContent?.hits || []).map(h => h.file); return files.length + " hits; env files among them: " + files.filter(f => /(^|\/)\.env/.test(f)).length; });
