...
class PasswordsTest {
    ...
    void degradeFlipsSeamOnceAndStaysQuiet() throws Exception {
        ...
        var degrade = Passwords.class.getDeclaredMethod("degrade", Throwable.class);
        degrade.setAccessible(true);
        try {
            Passwords.keyringUsable = true; // simulate the first-ever failure
            degrade.invoke(null, new IllegalStateException("no backend (test)"));
            assertThat(Passwords.keyringUsable)
            ...
            Passwords.keyringUsable = false; // the posture every other test forces
        }
    }
}
