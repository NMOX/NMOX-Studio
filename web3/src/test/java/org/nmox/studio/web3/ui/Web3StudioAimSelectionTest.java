...
/**
 * Tech-debt ledger 72, closed v2.186.0: Contract Studio publishes the
 * aimed project's node as its ambient selection, so the platform's
 ...
 */
class Web3StudioAimSelectionTest {
    ...
    private static File selectedDir(Web3StudioTopComponent window) throws Exception {
        File[] out = {null};
    ...
    void publishesWhileShowingOnly(@TempDir File aimA, @TempDir File aimB) throws Exception {
        Web3StudioTopComponent[] holder = new Web3StudioTopComponent[1];
        SwingUtilities.invokeAndWait(() -> holder[0] = new Web3StudioTopComponent());
        Web3StudioTopComponent tc = holder[0];
