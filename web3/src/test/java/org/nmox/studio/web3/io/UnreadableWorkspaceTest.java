 ...
 * sharpest here, where the deployment address book lives (a row of it
 * survived 36 days and a platform move in the v2.38.4 walk).
 *
 * <p>The corrupt-file law has held since v1.36.0: a {@code .nmoxweb3.json}
 * that fails to PARSE is copied to {@code .bak} before the empty fallback
 * is returned. The READ failure shared that fallback and had none of its
 * protection — it returned the same empty workspace a fresh project gets,
 * with a null backup, so {@code Web3StudioTopComponent} recorded the file
 * as its own and the next save wrote an empty workspace over every
 * network and every deployment.
 *
 ...
 * bytes became 75. So did a permissions error and any transient fault.
 */
class UnreadableWorkspaceTest {
    ...
    void overCapIsUnreadable(@TempDir File dir) throws Exception {
        writeOverCap(new File(dir, Web3WorkspaceIO.FILENAME));
    ...
    void corruptIsNotUnreadable(@TempDir File dir) throws Exception {
        Files.writeString(new File(dir, Web3WorkspaceIO.FILENAME).toPath(), "{ not json");
        Web3WorkspaceIO.LoadOutcome outcome = Web3WorkspaceIO.loadGuarded(dir);
        assertThat(outcome.unreadable())
