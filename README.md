...
i18next, vue-i18n, Angular XLIFF, Lingui, Paraglide and react-intl, with
key completion inside `t('…')` and ⌘-click to the source catalog);
...
53-device task rack that starts wired for the project's own kind and
travels as a file (Share…/Import… — what a rack holds is shown before it
mounts, and it arrives at rest), PREFLIGHT ship gates, CI export, pull
requests on the git chip (list, review threads, checkout — through your own
`gh`),
...
status lines, Welcome, Options — speaks fifteen languages: English, Español, Français, Deutsch, Русский, Українська, Polski,
Português (Brasil), Bahasa Indonesia, Filipino, Tiếng Việt, 简体中文,
हिन्दी, עברית and العربية — and Hebrew and Arabic lay the whole window out right to
left, not just the words inside it.
...
[العربية](docs/user-guide.ar.md).
The product's own website speaks all fifteen too — the page it serves on
