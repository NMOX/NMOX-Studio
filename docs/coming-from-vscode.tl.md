...
| **Search view** (`search.useIgnoreFiles`) | **Hanapin sa mga proyekto** (⇧⌘F). Gaya sa VS Code, nilalaktawan nito ang binabalewala ng mga file na `.gitignore` ng repositoryo at ng `.git/info/exclude`, kaya wala sa mga resulta ang `node_modules` at `dist/` kapag nakalista sila sa `.gitignore`; sa labas ng repositoryo, nilalaktawan nito ayon sa pangalan ang `node_modules`, `dist`, `build` at ang iba pang folder ng build. Lagyan ng tsek ang **Maghanap sa mga generated na source** sa dialog nito para hanapan din ang mga iyon. Hindi binabasa ang inyong pandaigdigang git excludes file. |
