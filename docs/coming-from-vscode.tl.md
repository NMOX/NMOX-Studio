...
| **`settings.json`** | Kasangkapan ▸ Mga Opsyon (sa macOS, NMOX Studio ▸ Settings…). Sinusunod kapag nag-save ang `.editorconfig` ng iyong proyekto. |
| **Problems panel** | **Action Items** (⌘6): mga natuklasan ng lint at type mula sa mga device na PURITY at TYPEGUARD ng rack, katabi ng mga kulot na guhit sa editor. |
...
- **Sa Linux, binubuksan dati ng Ctrl+\` ang window switcher** — ang fallback ng plataporma para sa mga desktop (KDE) na umaagaw ng Ctrl+Tab. Binubuksan na nito ngayon ang Terminal; nasa Ctrl+Tab pa rin ang switcher.
