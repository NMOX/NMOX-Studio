...
Die Zeile `brew trust` ist Homebrews einmalige Bestätigung für jeden Tap von Dritten — bei Aktualisierungen werden Sie nicht erneut gefragt. Die Anwendung ist mit einer Apple Developer ID signiert und von Apple notarisiert, daher akzeptiert Gatekeeper sie so, wie sie ist — der Cask kopiert sie und tut sonst nichts damit. Die Installation von Hand aus dem DMG funktioniert genauso.
...
> **macOS, erster Start:** Doppelklicken Sie die Anwendung. macOS fragt einmal, ob eine aus dem Internet geladene App geöffnet werden soll, und sagt dabei, dass Apple sie geprüft hat: Klicken Sie auf **Öffnen**. Die Anwendung ist mit einer Apple Developer ID signiert und notarisiert, und das Ticket ist an App und DMG geheftet, sodass die Prüfung offline funktioniert — kein Rechtsklick, kein `xattr`. Die integrierte Aktualisierung installiert in Ihr Benutzerverzeichnis statt in das App-Bundle, sodass ein Update diese Signatur nie beschädigt.
>
> Hat eine Installation von 3.0.0, 3.0.1 oder 3.0.2 mit *„NMOX Studio.app“ nicht geöffnet* (*Not Opened*) geantwortet, lag das an einem Fehler darin, wie die Anwendung ihr Startskript aufrief; behoben in 3.1.0. Installieren Sie 3.1.0 oder neuer (`brew upgrade --cask nmox-studio` oder ein neuer Download).
...
**Extras ▸ Plugins ▸ Aktualisierungen** (oder **Hilfe ▸ Nach Aktualisierungen suchen**) bietet die Produktmodule jeder neueren Version an, aus dem Aktualisierungszentrum „NMOX Studio-Updates“, das auf die neueste GitHub-Version zeigt. Installieren, bei Aufforderung neu starten, fertig. Die Plattform prüft auch von sich aus, standardmäßig wöchentlich (einstellbar unter **Extras ▸ Plugins ▸ Einstellungen**), und unabhängig davon weist die IDE einmal am Tag auf eine neuere Version hin; das schalten Sie unter Optionen ▸ Allgemein ab (unter macOS NMOX Studio ▸ Einstellungen…, sonst Extras ▸ Optionen). Jedes Modul ist signiert und das Zertifikat liegt dem Produkt bei, daher installieren sich Aktualisierungen ohne Zertifikatsabfragen.

Die Aktualisierung ersetzt Module, nicht die Anwendung um sie herum. Die mitgelieferte Java-Laufzeit, der Starter und die NetBeans-Plattform selbst ändern sich nur, wenn Sie eine Version installieren (`brew upgrade --cask nmox-studio` oder ein neuer Download), und eine Version, die eines davon ändert, sagt das in ihren Anmerkungen — der Befehl `nmox` und die Korrekturen am macOS-Starter in 3.1.0 sind Beispiele. Eine Installation, die älter als 2.35.0 ist, kann sich gar nicht in der Anwendung aktualisieren, weil 2.35.0 die Plattform gewechselt hat: Installieren Sie stattdessen eine aktuelle Version.

<a id="2-first-launch"></a>
## 2. Erster Start

Aus einem Terminal öffnet `nmox .` den Ordner, in dem Sie gerade stehen, so wie `code .` es tut: `cd myproject && nmox .`. Ein Ordner wird genau so angepeilt, wie „Ordner öffnen…“ auf der Willkommensseite ihn anpeilt, mit Manifest oder ohne; eine Datei öffnet sich im Editor (`nmox src/app.js`), und zwar an einer bestimmten Zeile, wenn Sie eine nennen, so wie bei `code -g` (`nmox src/app.js:42` — eine Spalte wird akzeptiert, und der Editor öffnet am Anfang der Zeile). Ein Name, den es nicht gibt, wird im Terminal abgelehnt (`nmox: typo.js: no such file or folder`), statt irgendetwas zu starten. VS Codes `-r` wird akzeptiert, und `-n` öffnet im einzigen Fenster; `-a` und `-v` werden mit ihrem Namen abgelehnt. `nmox -w file` öffnet die Datei und wartet, bis Sie ihren Tab schließen, und `nmox -d left right` vergleicht zwei Dateien in der Diff-Ansicht; damit wird NMOX Studio zum Editor und Difftool von git: `git config --global core.editor "nmox -w"`, und `nmox --help` gibt die Zeilen aus, die es zum Difftool und zum Mergetool machen. Speichern Sie die Nachricht, schließen Sie den Tab, und git macht weiter. Ist das Mergetool gesetzt (`mergetool.nmox.cmd` auf `nmox -w "$MERGED"`), öffnet `git mergetool` jede Datei mit Konflikten im Editor. In dieser Datei und in jeder anderen, die gits Konfliktmarken trägt, ist die aktuelle Seite jedes Konflikts in einer Farbe getönt und die eingehende in einer anderen, und die Zeile `<<<<<<<` trägt eine Warnung, deren Schnellkorrektur (⌘. auf dem Mac, sonst Alt+Enter, oder die Glühbirne am Rand) die drei Möglichkeiten von VS Code anbietet: **Aktuelle Änderung übernehmen**, **Eingehende Änderung übernehmen** und **Beide Änderungen übernehmen**. Jede ist eine einzige Bearbeitung, die ein einziges ⌘Z zurücknimmt; hat sich der Block geändert, seit die Warnung erschien, wird nichts ersetzt, und die Statuszeile sagt es. Ein Block, der nicht genau gits Form hat (ein zweiter Trenner, ein Block in einem Block), bekommt kein Angebot statt einer Vermutung. Die Tönungen sind die Einträge „Merge-Konflikt“ unter Optionen ▸ Schriften und Farben ▸ Highlighting. **Team ▸ NMOX Studio mit Git verwenden…** zeigt diese Einstellungen neben ihren aktuellen Werten und setzt sie für Sie. Eine Commit-Nachricht öffnet sich als Datei von git selbst: Die `#`-Zeilen sind Kommentare, nur was Sie schreiben, wird auf Rechtschreibung geprüft, und eine Zusammenfassungszeile, die über die 72 Zeichen hinausgeht, bei denen gits Werkzeuge abschneiden, bekommt eine Warnung. Eine `git rebase -i`-Liste hebt jeden Befehl und jeden Commit hervor, und Kommentar umschalten nimmt eine Zeile heraus, ohne sie zu löschen. Ansonsten kehrt der Befehl sofort zurück — das erste `nmox` startet die IDE im Hintergrund, und jedes weitere übergibt seinen Ordner an die bereits laufende IDE. Ein bloßes `nmox` startet nur die IDE. So kommt `nmox` in Ihren PATH:

- **macOS, Homebrew:** Der Cask verlinkt ihn für Sie.
- **macOS, aus dem DMG:** Verlinken (nicht kopieren) Sie den Starter der Anwendung —
  `sudo mkdir -p /usr/local/bin && sudo ln -s "/Applications/NMOX Studio.app/Contents/MacOS/nmox-studio" /usr/local/bin/nmox`.
  Über einen Link gestartet, weiß er, dass er aus einem Terminal kommt; aus dem Finder oder dem Dock gestartet, verhält er sich wie immer.
- **Windows:** Das Kästchen *„nmox“ zum PATH hinzufügen* im Installationsprogramm, standardmäßig angehakt. Öffnen Sie danach ein neues Terminal; ein bereits offenes behält seinen alten PATH.
- **Linux:** Das `.deb` installiert `/usr/bin/nmox`. Aus dem Tarball verlinken Sie ihn selbst: `ln -s "$PWD/nmox-studio-<version>/bin/nmox" ~/.local/bin/nmox`.

Unter Linux und Windows können Sie NMOX Studio einen Ordner auch ohne Terminal übergeben, und er wird genauso angepeilt:

- **Linux (das `.deb`):** Ihr Dateimanager führt NMOX Studio für einen Ordner unter *Öffnen mit* auf. Es wird dadurch nicht Ihr Standardprogramm für Ordner; das bleibt der Dateimanager.
- **Windows:** Haken Sie im Installationsprogramm das Kästchen *„Mit NMOX Studio öffnen“ zum Kontextmenü von Ordnern im Explorer hinzufügen* an (es ist anfangs nicht angehakt, wie bei VS Code). Der Explorer bietet dann **Mit NMOX Studio öffnen** auf einem Ordner und auf der leeren Fläche darin an; unter Windows 11 steht es unter *Weitere Optionen anzeigen*. Die Deinstallation entfernt es wieder.

Unter macOS verwenden Sie `nmox .` oder **Datei ▸ Ordner öffnen…**. *Öffnen mit* im Finder und das Dock-Symbol können NMOX Studio noch keinen Ordner übergeben; deshalb bietet sich die App dort nicht an.

Die IDE öffnet sich mit drei Tabs am Editorbereich: **Willkommen → Task-Rack → Browser**. Jedes weitere Fenster ist ein ⌥⌘-Kürzel entfernt und steht in der Spalte WERKZEUGE der Willkommensseite. Im linken Dock: **Projekt-Studio** (Dateibaum und Vorlagen), die Basis **Arbeitsplatz** und der **NPM-Explorer**. Ein Ordner `~/NMOX` wird als Standardarbeitsbereich angelegt; das Rack zeigt dorthin, bis Sie ein Projekt öffnen.
...
| Kürzel | Öffnet |
|---|---|
| **⌘I** | Schnellsuche — erreicht alles (siehe §9) |
| **⇧⌘P** | Ebenfalls die Schnellsuche — das Kürzel, das VS Code Befehlspalette nennt |
| **⌘9** | Task-Rack |
| **⌥⌘0** | Arbeitsplatz |
| **⌥⌘1** | Aufgaben-Board |
| **⌥⌘2** | Tests |
...
| **⇧⌘E** | Projekt-Studio, mit dem Fokus im Dateibaum |
| **⇧⌘X** | Extras ▸ Plugins |
| **⌃\`** | Ein Terminal im Projektordner oder das bereits offene (Strg+\` unter Windows und Linux) |
| **⌥⌘P / ⌥⇧⌘K** | Projekt wechseln… / Experimente… |

Sie kommen von VS Code? [Von VS Code kommend](coming-from-vscode.de.md) ordnet die Tastenkürzel und die Ideen einander zu, mit der Schreibweise für Windows und Linux neben der für macOS.

**Orte im Terminal öffnen sich per ⌘-Klick** (Strg-Klick unter Windows und Linux). Klicken Sie auf eine Stelle, die ein Werkzeug ausgegeben hat, und die Datei öffnet sich an dieser Zeile: `src/app.ts:42:7` von tsc, `(/abs/app.js:10:5)` in einem Stack-Frame von Node oder Jest, `tests/test_x.py:12:` von pytest, `--> src/main.rs:3:5` von rustc, `File "x.py", line 12` aus einem Python-Traceback. Ein absoluter Pfad öffnet sich so, wie er ausgegeben wurde. Ein relativer wird vom Ordner des angepeilten Projekts aus gelesen, wo ⌃\` seine Shell startet; nach einem `cd` in einen Unterordner kann der Pfad eine Datei nennen, die es dort nicht gibt, und die Statuszeile sagt, nach welchem Pfad sie gesucht hat, statt zu raten. Eine URL oder ein `host:port` ist nie ein Link.

<a id="3-projects"></a>
## 3. Projekte

**Öffnen:** Jeder Ordner mit einem der 63 erkannten Manifeste öffnet sich als echtes Projekt — `package.json`, `Cargo.toml`, `go.mod`, `pom.xml`, `composer.json`, `foundry.toml`, `bower.json`, `Gruntfile.js` und Verwandte — einschließlich der Manifeste der Contract-Chains: Ein Aiken-Repository (`aiken.toml`) oder ein Clarinet-Repository (`Clarinet.toml`) öffnet sich mit seinen echten, verdrahteten Spuren. Auch ein bloßer Ordner mit HTML und `<script>`-Tags und **ohne** Manifest öffnet sich, als STATIC-Projekt: Das klassische Web ist erstklassig, kein Fehler.
...
**Der Dateibaum** ist das Projekt-Studio (⇧⌘E). Ein Rechtsklick auf eine Datei oder einen Ordner bietet Neu, Ausschneiden, Kopieren, Einfügen, Löschen und Umbenennen und — wie im Explorer von VS Code — **Pfad kopieren**, **Relativen Pfad kopieren** (relativ zum Projekt) und **Im Finder anzeigen** (**Im Datei-Explorer anzeigen** unter Windows, **Übergeordneten Ordner öffnen** unter Linux). Für die Datei, die Sie bearbeiten, tun **Bearbeiten ▸ Pfad kopieren** (⌥⌘C, Strg+Alt+C unter Windows und Linux) und **Bearbeiten ▸ Relativen Pfad kopieren** dasselbe — auch aus dem Terminal oder dem Baum heraus, für den Editor, in dem Sie zuletzt getippt haben.
...
**Ausführen, Bauen, Testen — und Anhalten:** Das ▶ der Werkzeugleiste (F6) führt das Projekt so aus, wie seine Toolchain es ausführt: das Skript `dev`, `start` oder `serve` aus der package.json (das erste, das sie hat), `cargo run`, `go run`, `dotnet run`, und für einen Ordner mit HTML einen kleinen statischen Server auf dem ersten freien Port ab 8080. Ein Node-Projekt ohne eines dieser drei Skripte sagt das, wenn Sie ▶ drücken, und zeigt seine Skripte im NPM-Explorer, wo ein Doppelklick eines davon ausführt. Bauen, Testen und Aufräumen liegen daneben und im Menü Ausführen. Ein Entwicklungsserver, der seine Adresse ankündigt, lässt das ⇄-Zeichen in der Statusleiste aufleuchten und öffnet die Seite im eingebauten Browser. Alles läuft beim ersten Mal hinter der Arbeitsbereichs-Vertrauensabfrage. Ein Start, der nicht gelingen konnte, sagt das und bietet an, den Environment Doctor zu öffnen. Zum Anhalten: Das ■ rechts von Debuggen (⌥⌘.) hält jeden laufenden Befehl auf einmal an und sagt, was es angehalten hat; **Ausführen ▸ Build/Ausführen stoppen** hält einen an und bietet danach **Wiederholen**. Das ■ sieht alles, was das Produkt für Sie startet, Installationen eingeschlossen; beim Überfahren nennt der Hinweis genau, was ein Druck anhalten würde, und seit wann jedes läuft.
...
**Richten Sie einen Agenten auf Ihre IDE.** Extras ▸ Agent Port (MCP)… öffnet einen MCP-Zugang, den ein fremder Assistent abfragen kann: **von Bauart nur lesend**, aus, bis Sie ihn einschalten, nur auf der lokalen Schnittstelle lauschend und auf das beim Start erzeugte Token angewiesen.

Das Rack ist erweiterbar: Erweiterungen von Dritten können Geräte hinzufügen (deren NBM über Extras ▸ Plugins installieren). Zum Selberschreiben siehe [device-spi.md](device-spi.md).
...
- **Die Minimap** — ein Umriss der ganzen Datei neben der Bildlaufleiste jedes Editors; klicken oder ziehen bewegt den Ausschnitt. Das ganze Dokument passt immer in den Streifen: Die Zeilen schrumpfen, während die Datei wächst. Ansicht ▸ Minimap schaltet sie für alle offenen Editoren auf einmal.
...
- **LSP** — Öffnen Sie eine Datei, deren Sprachserver installiert ist (typescript, gopls, rust-analyzer, pyright, …), und Sie bekommen Diagnosen, Hover-Hinweise und Gehe-zu-Definition. Die Fehler und Warnungen des Servers stehen außerdem als Zeilen in der **Aufgabenliste** (⌘6), benannt nach dem Server (`[lsp:gopls]`), für jede Datei, zu der der Server etwas gemeldet hat. Manche Server melden nur die Dateien, die Sie geöffnet haben; gopls meldet das ganze Paket. Fehlt ein Server? Die IDE bietet den Installationsbefehl an, statt stillschweigend zu scheitern.

  ![Die Aufgabenliste mit zwei Fehlern von gopls, einer davon in einer nie geöffneten Datei, und der Zahl ✕ 2 ⚠ 0 in der Statuszeile](images/lsp-action-items.png)

- **Das Testfenster (⌥⌘2)** zeigt jeden Test des Projekts, *bevor irgendetwas läuft*, und startet einen, eine Datei oder alle.
- **`.editorconfig` wird beachtet** — beim Tippen und beim Speichern. `indent_style`, `indent_size` und `tab_width` bestimmen, was Tab, Eingabe und Neu-Einrücken schreiben: Ein Tab-Projekt bekommt Tabs, ein Projekt mit vier Leerzeichen bekommt vier Leerzeichen, je Datei und je Glob-Abschnitt; jedes Speichern wendet `trim_trailing_whitespace` und `insert_final_newline` an. Eine Änderung an `.editorconfig` erreicht offene Editoren innerhalb weniger Sekunden. Ein Tabzeichen, das schon in der Datei steht, wird weiterhin in der Tabbreite aus den Optionen gezeichnet, und `charset` und `end_of_line` werden nicht angewandt. Den Rest erledigen Ihre Formatierungsgeräte (GLOSS und Co.).
- **Die `.vscode/settings.json` eines Repositorys wird genauso beachtet**: `editor.tabSize`, `editor.insertSpaces` und `editor.indentSize` bestimmen die Einrückung, `files.trimTrailingWhitespace` und `files.insertFinalNewline` (wenn `true`) wirken beim Speichern, und ein Sprachblock wie `"[typescript]": {…}` überschreibt sie für seine Sprache. Hat das Projekt auch eine `.editorconfig`, gewinnt die `.editorconfig` überall, wo beide etwas sagen. `editor.detectIndentation` aus VS Code, das die eigene Einrückung einer Datei gewinnen lässt, hat hier kein Gegenstück.
...
### Links in Ihrer Dokumentation

Ein README wird auf GitHub gelesen, und einen Link darin, der ins Leere führt, findet der nächste Leser, nicht Sie. **Extras ▸ Markdown-Links prüfen…** liest jede Markdown-Datei des angepeilten Projekts und prüft jeden relativen Link und jedes Bild so, wie GitHub sie darstellen wird: Die genannte Datei oder der genannte Ordner muss existieren, und ein `#heading` muss eine Überschrift dieser Datei nach GitHubs Ankerregel sein (kleingeschrieben, Satzzeichen entfernt, Leerzeichen als Bindestriche, eine wiederholte Überschrift mit `-1`, `-2` nummeriert). Ein Link, der ins Leere führt, ist ein Fehler; eine fehlende Überschrift und ein Link, der aus dem Projekt hinausführt, sind Warnungen. Sie erscheinen als Wellenlinien am Link und als Einträge in den Aufgaben, und die Statuszeile fasst den Lauf zusammen (`Markdown-Links: 12 Dateien, 148 Links, 1 führt ins Leere`).

Links mit einem Schema (`https:`, `mailto:`) werden nicht geprüft: Nichts verlässt Ihren Rechner. Ebenso wenig alles in einem Codeblock oder in Inline-Code, denn ein Link in einem Beispiel ist ein Beispiel, und auch nicht das Fragment eines Links in eine Quelldatei (`app.js#L10` ist GitHubs Zeilenanker). Ein Link, der mit `/` beginnt, wird vom Projektstamm aus gelesen, so wie GitHub ihn liest.
...
Klicken Sie in den Rand, wählen Sie **Datei debuggen (Haltepunkte)**, und das Programm hält dort an — mit Aufrufliste, Variablen und Ausdrucksauswertung. JavaScript und TypeScript laufen ab Werk über den mitgelieferten Adapter; Python nutzt debugpy und Go delve, die Sie selbst installieren. Ein Repository mit `.vscode/launch.json` bietet eine weitere Tür: Tippen Sie den Namen einer Konfiguration in die Schnellsuche, und Eingabe startet das Node- oder Python-`program` dieser Konfiguration in ihrem `cwd`, mit ihren `args` und ihrem `env`, oder öffnet die `url` einer Chrome-Konfiguration mit ihrem `webRoot`; eine Konfiguration, die `envFile`, `runtimeExecutable` oder sonst etwas setzt, das der Debugger nicht weitergeben kann, wird in der Statuszeile mit Namen abgelehnt, statt ohne es gestartet zu werden. **In Chrome debuggen** tut dasselbe für eine Seite: Die Haltepunkte in Ihrem Quelltext halten in der IDE, während der Browser mit einem Wegwerfprofil läuft. Alles geht zuerst durch die Arbeitsbereichs-Vertrauensabfrage.
...
**Ansicht ▸ Präsentationsmodus** vergrößert auf einen Schlag jeden Editor, die Seite im eingebauten Browser, das Ausgabefenster und das Terminal — und stellt beim Verlassen alles genau wieder her. **Ansicht ▸ Tastenanschläge anzeigen** blendet den eben gedrückten Tastengriff groß ein, aber nie das, was Sie tippen. **Bearbeiten ▸ Als Markdown kopieren** kopiert die Auswahl als abgegrenzten Block mit der richtigen Sprachmarke, und die Variante **mit Link** hängt den GitHub-Link auf dieselben Zeilen an. **Bearbeiten ▸ Auf GitHub öffnen** und **Bearbeiten ▸ GitHub-Link kopieren** (auch per Rechtsklick, im Editor und im Baum des Projekt-Studios) liefern denselben Link ohne den Block: die Zeile des Cursors oder die Zeilen der Auswahl, eine Datei oder einen Ordner (seine `tree`-Seite; der Wurzelordner des Projekts ist die Startseite des Repositorys). Öffnen geht in Ihren eigenen Browser, in dem Sie angemeldet sind, sodass Blame und Review-Kommentare funktionieren. Eine Datei außerhalb eines Git-Repositorys, ein Repository ohne `origin` oder ein origin, der nicht GitHub ist, wird in der Statuszeile abgelehnt, und nichts wird kopiert — ebenso ein Editor mit ungespeicherten Änderungen, weil der Link andere Zeilen zeigen würde, als Sie sehen. **Team ▸ Neuer Pull Request auf GitHub** (auch im Menü des Git-Zeichens) öffnet in Ihrem Browser die Seite „New Pull Request“ von GitHub selbst für den Zweig, den Sie ausgecheckt haben: der Schritt nach einem Push, ohne Repository und Zweig von Hand zu suchen. Ein losgelöster HEAD wird abgelehnt, denn er hat keinen Zweig, den er vorschlagen könnte. **Extras ▸ Screenshot speichern…** malt das ganze Fenster in doppelter Größe, mit Varianten für den Editor-Tab allein, für die Zwischenablage und für den Projektbaum als Markdown.
...
Dahinter stehen mit `↑2 ↓1` die Commits, die Ihr Zweig hat und sein Upstream nicht (zum Pushen), und umgekehrt (zum Pullen) — nur angezeigt, wenn sie nicht null sind, und nur für einen Zweig mit Upstream. Sein Menü beginnt mit **Branch wechseln…**, **Commit…**, **Pull…** und **Push…**, den eigenen Dialogen des Git-Moduls.

Neben dem Zeichen beantwortet eine leisere Notiz die Frage, die Sie am häufigsten stellen: Wer hat die Zeile geschrieben, auf der der Cursor steht? `Ada Lovelace, vor 3 Tagen · Fix the parser` folgt dem Cursor, in Ihrer Sprache, und ihr Tooltip nennt den Commit und das Datum. Ein Klick bietet **Annotationen anzeigen** (die der ganzen Datei, dasselbe **Anmerken**, das das Zeichen anbietet), **Commit auf GitHub öffnen** (die Seite des Commits selbst, wenn origin ein GitHub-Remote ist) oder **Commit-ID kopieren**; auf einer Zeile, die noch nicht committet ist, führt der Klick direkt zu den Anmerkungen. Die Notiz fragt git einmal pro gespeicherter Fassung der Datei, nie beim Start und nie für eine Datei außerhalb eines Repositorys, und schweigt bei einer Datei, die git nicht verfolgt. Eine Zeile, die Sie geändert, aber noch nicht committet haben, lautet **Noch nicht committet**. Solange die Datei ungespeicherte Änderungen hat, steht dort **Zeilenautor: ungespeicherte Änderungen** statt eines Namens, denn git liest die Datei so, wie sie gespeichert ist, und Zeile 40 auf dem Bildschirm muss nicht Zeile 40 auf der Platte sein; speichern Sie, und der Name kommt zurück. **Ansicht ▸ Zeilenautor** schaltet die Notiz aus und ein.
...
**Hilfe ▸ NMOX-Studio-Website (lokal)** liefert die Seite des Produkts aus seinem eigenen Rack aus, auf der lokalen Schnittstelle. Sie spricht die fünfzehn Sprachen, die die IDE spricht; der Wähler steht im Fuß der Seite.
...
Seiten in komplexen Schriften werden geformt gemalt: Arabisch, Persisch, Urdu (Nastaliq eingeschlossen), Kurdisch, Paschtu, Sindhi, Uigurisch, Syrisch, Thaana und N’Ko verbinden ihre Buchstaben und lesen sich in ihrer eigenen Reihenfolge, Zahlen in der ihren; Hindi und die übrigen indischen Schriften (Bengalisch, Gurmukhi, Gujarati, Oriya, Tamil, Telugu, Kannada, Malayalam, Singhalesisch), Thai, Tibetisch, Birmanisch und Khmer setzen ihre Vokalzeichen und Ligaturen an die richtige Stelle; und als eigene Zeichen geschriebene Akzente (so schreibt macOS Dateinamen) sitzen auf ihren Buchstaben. Das WebKit von JavaFX tut nichts davon von sich aus. Unter macOS und Windows schaltet der Browser WebKits eigene Maschine für komplexen Text ein, sodass Formularfelder, fette und kursive Wendungen, Blocksatz und Auswahlen genau vermessen werden. Unter Linux, wo es diesen Schalter nicht gibt, formt der Browser den Text selbst und passt seine Breitenschätzungen an die Schriften an, die Ihre Distribution installiert hat; eine Wendung voller Ligaturen oder in Nastaliq gesetzt kann deshalb ein paar Pixel danebenliegen. Laotisch wird nicht geformt: JavaFX’ eigener Text verschiebt dessen Vokale ebenfalls. Hebräisch, Armenisch, Georgisch und Äthiopisch werden von sich aus richtig dargestellt, Niqqud eingeschlossen.
...
### Das Zeichen ⇄ bereitgestellt

In der Statuszeile erscheint ein **⇄ bereitgestellt**-Zeichen, sobald Server laufen: der Lauf der IDE selbst, die bedienenden Geräte und jeder Befehl, der eine lokale Adresse ausgegeben hat. Klicken Sie und wählen Sie einen aus — er öffnet sich im eingebauten Browser, oder im Browser des Systems, wenn jener Reiter ihn nicht nehmen kann.

### ⌘I, der allgemeine Finder

Ein einziges Feld erreicht Ihre Projekte (die zuletzt benutzten und die bekannten), jedes Rack-Gerät — samt Sprung zu seinen Bedienelementen —, die **laufenden Server** (Eingabe öffnet sie im Browser), die Anfragen des API-Studios, die Verbindungen und Tabellen des DB-Studios, die Verträge, die Infrastrukturknoten, die Karten der Aufgabentafel, deren Treffer die Spalte nennt, in der sie stehen, die **Befehlsnamen von VS Code** (*Format Document*, *Toggle Terminal*, *Git: Commit*, *Open Settings* — jeder unter *VS-Code-Befehle* neben der Aktion gelistet, die hier dasselbe tut, sodass ihr Name hier derjenige ist, den Sie beim nächsten Mal tippen) und die **npm-Skripte** des angepeilten Projekts: Tippen Sie `dev` oder `test`, und der Treffer lautet etwa *Skript ausführen: dev — vite*; Eingabe führt ihn mit dem Paketmanager des Projekts aus (npm, yarn oder pnpm), genau wie ein Doppelklick im NPM-Explorer — bei einem Projekt, dem Sie noch nicht vertraut haben, fragt zuerst das Arbeitsbereichsvertrauen, der Lauf reiht sich beim ■ der Werkzeugleiste ein, und ein Entwicklungsserver, der eine Adresse ausgibt, lässt das ⇄-Zeichen aufleuchten. In einem Monorepo sind es die Skripte, die der NPM-Explorer zeigt. Ein Repository mit `.vscode/tasks.json` listet seine Aufgaben genauso — *Aufgabe ausführen: build — make all* —, und Eingabe führt die Aufgabe hinter derselben Vertrauensfrage aus, im Fenster Output und unter dem ■ der Werkzeugleiste; eine Shell-Aufgabe läuft in der Shell, die VS Code verwenden würde (Ihre `$SHELL`, unter macOS eine Login-Shell; unter Windows PowerShell), oder in der, die ihr `options.shell` nennt; eine Aufgabe, die einen Wert braucht, den nur VS Code liefern kann, oder die von einer anderen Aufgabe abhängt, sagt das in der Statuszeile, statt zu laufen. Die `.vscode/launch.json` des Repositorys listet ihre Konfigurationen daneben — *Debuggen: Launch Program — ${workspaceFolder}/server.js* —, und Eingabe startet nach derselben Vertrauensfrage den Haltepunkt-Debugger für diese Konfiguration.

**In Projekten suchen (⇧⌘F) durchsucht Ihren Code, nicht das, was das Repository ignoriert.** In einem Git-Repository überspringt die Suche alles, was die eigenen `.gitignore`-Dateien des Repositorys (die im Wurzelverzeichnis und alle verschachtelten) und `.git/info/exclude` ignorieren, dazu `.git` selbst — die Suche nach einem Funktionsnamen findet also `src/`, nicht die Kopien in `node_modules` und `dist/`, die die `.gitignore` der Vorlagen aufführt. Sie geht nie über das hinaus, was Git ignoriert: Ein Ordner, den Ihr Repository verfolgt, wird durchsucht, wie auch immer er heißt. Außerhalb eines Repositorys überspringt sie außerdem die Build- und Paketordner, die jeder Durchlauf des Produkts überspringt (`node_modules`, `dist`, `build`, `out`, `target`, `coverage`, `.next`, `.nuxt`, `.svelte-kit`, `.angular`, `.venv`, `__pycache__`). Die Ordner bleiben im Projektbaum; nur die Suche geht an ihnen vorbei. Um sie trotzdem zu durchsuchen, aktivieren Sie **In generierten Quellen suchen** im Dialog „In Projekten suchen“ (die Einstellung bleibt, bis Sie sie wieder deaktivieren); In Projekten ersetzen bietet dieses Kästchen nicht an und schreibt nie um, was das Repository ignoriert. Ihre globale Git-Ausschlussdatei wird nicht gelesen; ein Pfad, den nur sie ignoriert, wird also durchsucht.

### Die Statuszeile sagt, was lebt

Neben dem Server-Zeichen stehen das angepeilte Projekt mit seiner Werkzeugkette und der Git-Zweig samt der Zahl geänderter Dateien. All das wird von der Platte oder aus Aufzeichnungen gelesen, die das Produkt ohnehin führt — Hinsehen kostet keine Prozesse. Solange irgendetwas, das die IDE prüft, ein Problem hat, zeigt eine Zahl **✕ 2 ⚠ 1** die Fehler und Warnungen aller Sprachserver und Werkzeuge; ein Klick darauf öffnet die Aufgabenliste.
...
Extras ▸ Optionen ▸ Tastaturbelegung (unter macOS NMOX Studio ▸ Einstellungen… ▸ Tastaturbelegung) wechselt das ganze Profil: die Bewegungs- und Ausschneidekürzel von Emacs in jedem Editor, oder die Sätze von Eclipse und IDEA, wenn dort Ihr Muskelgedächtnis liegt. Jedes NMOX-Kürzel (die ⌥⌘-Fensterfamilie, ⌘P für Gehe zu Datei, ⌥⌘E für Emmet, die Kürzel aus VS Code) ist in allen fünf Profilen eingetragen, ein Profilwechsel kostet Sie also nie die Studio-Kürzel. Eine Ausnahme ist gewollt: Im Eclipse-Profil bleibt ⇧⌘E das eigene *Switch to Editor* von Eclipse, weil jemand, der Eclipse gewählt hat, genau das erwartet.
...
NMOX Studio spricht fünfzehn Sprachen: English, Español, Français, Deutsch, Русский, Українська, Polski, Português (Brasil), Bahasa Indonesia, Filipino, Tiếng Việt, 简体中文, हिन्दी, עברית und العربية. Wählen Sie Ihre unter **Optionen ▸ Allgemein ▸ Sprache** — jede steht in ihrem eigenen Namen, damit Sie Ihre immer finden. Die Wahl wird in Ihre Starteinstellungen geschrieben (`etc/nmoxstudio.conf`, als `--locale`-Argument) und greift auch sofort. Es wechseln: Menüs, Dialoge, Kurzhinweise, Statuszeilen, der Willkommensschirm und die Optionen. Es bleibt: das Vokabular der Rack-Frontplatten (GO, STOP, EXPLAIN — Gerätebeschriftungen wie an einem Synthesizer) sowie die tieferen Dialoge der Plattform, für die es noch keine Übersetzung gibt. Vielleicht müssen Sie nie wählen: eine frische Installation spricht schon die Sprache Ihres Systems, und auch aus einem Land, das wir nie genannt haben — Taiwan, Singapur, Portugal und Québec landen in ihrer Sprache statt im Englischen, weil die Kataloge nach einer Sprache benannt sind und nie nach einem Land.
...
### Der Environment Doctor

Im Menü Extras prüft er 66 äußere Werkzeuge im Betrieb — node, npm, docker, forge, composer, gopls… — und zeigt die gefundene Fassung sowie den Installationsbefehl für alles, was fehlt.
...
Kein Fenster, kein Fehler, beim ersten Start nach der Installation: bei einem signierten Build sollte das nicht passieren. Falls doch, ist die Kopie beschädigt oder wurde nach dem Download verändert — prüfen Sie mit `codesign --verify --deep --strict "/Applications/NMOX Studio.app"` und laden Sie sie neu, wenn das fehlschlägt. Die Protokolle liegen unter `~/Library/Application Support/nmoxstudio/…/var/log/`, falls Sie ein Ticket aufmachen müssen.
