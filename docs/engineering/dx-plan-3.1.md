...
| 28 | the switcher walk, again | VS Code's command-palette titles found nothing: the platform matches a query as one substring of an action's name, so "toggle terminal" never meets "Terminal in Project" | a *VS Code commands* category maps 53 titles (ten of them the focused editor's own, *Format Document* among them) to the action that does the same thing here, showing both names and the chord | `VsCodeCommandSearchProviderTest`; `ActionIdsResolveTest` reads every row |
