...
| 4 | The project-wide editor scans across a monorepo's packages: a token, class or route declared in another package is found, the file's own package first | 4 | **Done on `claude/after-3.2`.** Which packages is answered two ways, because the question differs. Tokens and classes: the workspace packages the file's package *depends on*, in the order its `package.json` declares them, found where the package manager linked them (else among the declared packages), inside the workspace and outside `node_modules`, eight at most, in fair shares of a second 60-file budget. Walked on a two-package workspace: `var(--s` in `packages/web` offers `--space-lg` from `@acme/tokens`, and an unrelated sibling's `--stranger-hue` stays out. Rename Class edits only its own package and refuses both a new name a dependency declares and an old one (its usages here would lose the rule). Routes: a `web` package rarely depends on the `api` package it calls, so the jump reads the packages that *declare a server framework*, an exact match anywhere beating a `:param` match, and a workspace with more than 512 packages or eight server packages says the miss is about the ones read. A hostile review of the first cut found five defects (a manifest of 100,000 names hanging completion for minutes, hash order where "declared first" was promised, the old-name rename, workspace globs reading manifests outside the repository, and a self check comparing a real path to an unresolved one); all fixed. `WorkspaceDependenciesTest` (fourteen), `WorkspacesContainmentTest` (four), `RoutesLookupTest` (six); sixteen mutants by name, one equivalent and pinned by its source |
| 5 | Project Studio's tree expanding a 250-file folder, and the Workbench's detection sweep, on the fixture | — | **Measured: nothing to fix.** The head build aimed at the fixture under a 200 s JFR recording, the tree driven by keyboard: `packages` (200 folders) and a package's `test` folder (about 180 files, with their icons) expand at once, the Workbench names the project `node` on first show, and of the recording's 108 EDT samples the largest shares are the platform's class loading (19) and its heap meter's paint (20); the product's own frames are two. The watchers are 56 of 376 CPU samples over the 200 s, with the walk's tree activity in it. Zero SEVERE |
...
- Design tokens and CSS classes reach the workspace packages a package
  depends on, and the fetch-path → route jump reaches the workspace's
  server packages (row 4).
- A workspace glob that climbs out of the repository, or a package folder
  that is a link out of it, is no longer read (WAYPOINT's list too).
