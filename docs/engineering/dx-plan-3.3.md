...
| 4 | The project-wide editor scans across a monorepo's packages: a token, class or route declared in another package is found, the file's own package first | 4 | **Tokens and classes done on `claude/after-3.2`**, and the question "which siblings" answered by the package itself: the workspace packages its `package.json` depends on, found where the package manager linked them (else among the declared packages), inside the workspace and outside `node_modules`, eight at most, sharing a second 60-file budget. Walked on a two-package workspace: `var(--s` in `packages/web` offers `--space-lg` from `@acme/tokens`, and an unrelated sibling's `--stranger-hue` stays out. Rename Class still edits only its own package and refuses a name a dependency declares. `WorkspaceDependenciesTest` (eight), six mutants by name. The route jump says when it stopped at its cap (`RoutesLookupTest`, a mutant by name). **Routes still to build:** a `web` package's `fetch` rarely depends on the `api` package that serves it, so the dependency list is the wrong question there. The proposal: the packages that *declare a server framework* (`express`, `fastify`, `koa`, `hono`, `@hapi/hapi` in their dependencies) are the candidates, read after the file's own package, an exact match anywhere beating a `:param` match, and a workspace with more packages than the enumeration reads saying so the way the capped census does |
