...
| 2 | Go to File, Go to Symbol and Find in Projects on the fixture: time to first result, and what each reads | 3, 5 | Measured (finding 5): the cold costs are the platform's, not the product's. Go to Symbol's 2,000-file index is the one to widen (finding 4) |
