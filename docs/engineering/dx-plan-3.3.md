...
| 4 | The project-wide editor scans across a monorepo's packages: a token, class or route declared in a sibling package (the workspace's own `packages/*`) is found, the file's own package first | 4 | **In part:** the route jump says when it stopped at its cap instead of claiming no route exists (`RoutesLookupTest`, a mutant by name). Reading sibling packages still to design: which siblings, in what order, under what cap |
