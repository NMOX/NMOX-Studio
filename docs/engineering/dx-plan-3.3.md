...
5. **Go to File and Find in Projects answer correctly, and the first time
   ...
   climbing to about 700 MB on the first run. Profiled (JFR, a cold search
   on a fresh userdir, done in about 12 s): the cost is the platform's —
   a FileObject and a DataObject per file, then a reference-queue thread
   spending 213 of the recording's 746 samples removing the weak file
   listeners those DataObjects registered once they are collected. The
   product's own share (the `.gitignore` answer 3.2 added, the project
   lookup) is a handful of samples.
...
| 2 | Go to File, Go to Symbol and Find in Projects on the fixture: time to first result, and what each reads | 3, 5 | Measured (finding 5): the cold costs are the platform's, not the product's. **Go to Symbol reads the package being edited first** (done on `claude/after-3.2`): walked on the fixture with a file of `pkg-150` open, a function in it is found where the alphabetical 2,000 files never reach; the partial-index message still shows. `ProjectSymbolsTest`, two mutants by name. A larger cap still to measure |
