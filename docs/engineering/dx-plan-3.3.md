...
| 5 | Find in Projects' results open readable: the first real layout of a results split moves a divider under 120 px to the platform's 250 (or 40% of the window), once, and the platform's listener saves that | 6 | **Done on `claude/after-3.2`.** Walked twice: a fresh userdir's first search reads "Found 1 match of three in 1 file" with the tree 352 px wide, and a userdir already carrying the saved 14 opens healed; `replace_results_divider=352` saved both times. `SearchResultsSplitTest`, three mutants by name |
