...
- [`OpenFolderFromOsGateTest`](../../application/src/test/java/org/nmox/studio/application/OpenFolderFromOsGateTest.java): A folder handed over by the operating system is aimed on all three: the macOS bundle declares folders and names itself to the JVM (without `CFProcessPath` AWT dropped every Finder event), the Linux entry claims `inode/directory` and runs `nmox`, and the Windows folder verbs run `--aim "%V\."`, speak every installer language and leave on uninstall (v3.1.0).
