...
- [`CyrillicMnemonicsTest`](../../application/src/test/java/org/nmox/studio/application/CyrillicMnemonicsTest.java): Every Russian and Ukrainian mnemonic reaches a key: the platform's own `Mnemonics`, loaded from the cluster beside the shipped `org-openide-awt_<lang>.jar` table, is run over every value that underlines a non-ASCII letter (after 3.2.0, ledger 122: about 340 values per language underlined a letter no key reached).
