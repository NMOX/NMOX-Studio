...
- [`EditedFileGateTest`](../../application/src/test/java/org/nmox/studio/application/EditedFileGateTest.java): Code that names the file being edited, from a document or from a tab and in every module (Copy Path, GitHub links, line blame, the on-save formatters, the Agent Port, Annotate, the Workbench, Save Editor Screenshot) reads `EditedFile` or `EditorTabs`, never a DataObject's primary file (3.2.0: read from the properties editor's bytecode as a locale bundle naming its base; the platform's loader forms no such group, so this holds a rule for any DataObject of several files).
...
- [`RtlDocsPathDirectionGateTest`](../../application/src/test/java/org/nmox/studio/application/RtlDocsPathDirectionGateTest.java): In every Hebrew and Arabic document a path beginning with `.` `~` `/` (or a glob's `*`) after a right-to-left word carries an LRM before it, one ending with `/` carries one after it, and the mark sits outside the code span; the bidi measurement that decided it is pinned in the same class (3.2.0, ledger 121).
...
- [`ToolbarOverlayGateTest`](../../application/src/test/java/org/nmox/studio/application/ToolbarOverlayGateTest.java): The platform toolbar's overlays exist in every language and every choice pattern renders (v2.102.0). Since 3.2.0 its population is DERIVED: every `Toolbars/<toolbar>/<item>` button of every toolbar a shipped configuration shows, its tooltip key read from the layer (generic factories) or from the checked `main-toolbar.txt` ledger (actions built in code), and every key translated in every shipped language (3.2.0: a German main toolbar read New File..., Open Project... and Pause I/O Checks in English — the buttons paint the action's SHORT_DESCRIPTION, not the menu row's key).
...
- [`DiffViewOverlayLedgerTest`](../../application/src/test/java/org/nmox/studio/application/DiffViewOverlayLedgerTest.java): Every key the platform's diff view names in its shipped bytecode is overlaid in every language or recorded with a reason, and each still says the English the ledger records (3.2.0: `nmox -d` and `git difftool` opened it in English).
...
- [`TerminalCommandGateTest`](../../application/src/test/java/org/nmox/studio/application/TerminalCommandGateTest.java): `nmox` works on all three OSes: the macOS launcher and the Linux command run for real through chains of links, aim folders, open files and return at once, refuse a missing path on stderr with exit 2 before anything starts, the two Unix rules stay identical, `nmox.cmd` never expands an argument twice, and the Windows PATH task has a message for every installer language and never writes a leading separator (v3.1.0).
...
- [`MenuRowsSpeakTest`](../../application/src/test/java/org/nmox/studio/application/MenuRowsSpeakTest.java): Every platform menu row is translated, with no mnemonic collision (v2.143.0); a collision is two rows pressing the same KEY, so `&Файл` and `(&A)` collide once the Russian table maps Ф to A (after 3.2.0).
- [`TranslatedMnemonicsTest`](../../application/src/test/java/org/nmox/studio/application/TranslatedMnemonicsTest.java): Every translated mnemonic reaches a key and no two top menu bar entries press the same one: the platform's own `Mnemonics`, loaded from the cluster beside the language's shipped `org-openide-awt_<lang>.jar` table, is run over every value that underlines a non-ASCII letter, and a language with no table may underline none (after 3.2.0, ledger 122: about 340 values each in Russian and Ukrainian reached no key, Hindi underlined Devanagari on its whole top bar, and four top bars clashed).
...
- [`MainWindowChromeSpeaksTest`](../../application/src/test/java/org/nmox/studio/application/MainWindowChromeSpeaksTest.java): Every editor tab and Quick Search category the cluster's layers declare speaks every language, and so does a checked ledger of code-named main-window chrome (Favorites, the Quick Search field, the status line) (3.2.0: a German walk read Source, History, Favorites and the Quick Search hint in English). The same gate holds the editor toolbar: every button DERIVED from `Editors/**/Toolbars/Default`, its tooltip key derived from the action's layer `ShortDescription` or named in the checked `editor-toolbar.txt` ledger, and every key overlaid in every language (3.2.0: every button above an editor, the SQL and XML ones included, read its tooltip in English).
