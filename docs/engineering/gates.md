...
- [`EditedFileGateTest`](../../editor/src/test/java/org/nmox/studio/editor/share/EditedFileGateTest.java): Surfaces that name the file being edited (Copy Path, GitHub links, Copy as Markdown, line blame, the editor's accessible name) read `EditedFile`, never a DataObject's primary file (3.2.0: a `Bundle_de.properties` editor copied, blamed and linked `Bundle.properties`).
