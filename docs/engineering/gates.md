...
- [`MainWindowChromeSpeaksTest`](../../application/src/test/java/org/nmox/studio/application/MainWindowChromeSpeaksTest.java): Every editor tab and Quick Search category the cluster's layers declare speaks every language, and so does a checked ledger of code-named main-window chrome (Favorites, the Quick Search field, the status line) (3.2.0: a German walk read Source, History, Favorites and the Quick Search hint in English).
