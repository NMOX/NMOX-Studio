...
catalogs as data (no spawn, no trust gate), so the three checks the IDE holds over
its own bundles — missing per locale, placeholder-set parity, and the honest
"identical to source" report that v2.137.0 measured cannot be a gate — apply to
i18next, vue-i18n, svelte-i18n, react-intl, Lingui, Paraglide, Angular XLIFF and
the I18n Kit's shape, through the `i18n` DiagnosticsBus tool so squiggles, Action
Items and the Agent Port come free. No new rack device: a device costs the
53-count, a 42-character gloss in fifteen languages and a faceplate law, and buys
nothing a Tools row and the bus do not. Two find-sec-bugs findings in the fresh
code (an XXE detector that is intra-procedural — set the features on the factory
the parse reads, in the same method; a ReDoS-flagged call regex → a hand scanner)
...
any language the user wants. Let's start with English, French, Russian,
Hindi, and Spanish and German." Ledger 85 had this recorded as a
dossier-first project, and the recon is why it went smoothly.

**What the platform already gives you, measured not assumed.** The
platform's `CLIOptions` accepts `--locale`; the launcher script sources
`<userdir>/etc/nmoxstudio.conf` AFTER the install's own conf, so a
`default_options` line appended there reaches it. That makes the
language switch a LAUNCHER edit, not a preference — which is why it
needs a restart, and why the dialog says so rather than pretending.
The file is sourced by a shell, so `UiLocale` writes only a
`[a-z]{2}(:[A-Z]{2})?` code inside a marker block it owns.

**The probe that had to come first.** A hand-written
`src/main/resources/<pkg>/Bundle_fr.properties` lands in
`target/classes` beside the annotation-generated English
`Bundle.properties`, French resolves, and an untranslated key falls
back to English — proven with a throwaway key before a single
translation was written. That fallback is what makes a partial
translation safe.

**The agent fan-out.** Extraction ran as one worker per module (never
an orchestrator — the first attempt returned having changed nothing
because it thought it was delegating), then translation as one worker
per (language × module group), 25 in all, all writing disjoint files.
The house rule that paid: a brief that says what does NOT move
(protocol, generated bytes, wire strings, faceplate vocabulary) is
worth more than one that says what does.

**The failure class this arc produced.** Nineteen source-grep gates broke —
each had pinned an English literal at a call site as a proxy for a law
(the confirm exists, the menu carries the verb, the row is visible),
and the literal moved into an annotation at the top of the file while
the law stayed exactly where it was. The lesson is the v1.326.0 one
again, one layer up: *a gate anchored on a literal is anchored on a
mechanism*. Re-anchoring on the call site — and, where the promise
itself is the law, reading the shipped bundle VALUE — made most of them
stronger than before.

**The trust the gates now extend, and its boundary.** The `Plain*`
markup gates accept `Bundle.X(…)` as the product's own authored
sentence. That is only true while the value's HEAD is the product's own
words: a value starting with `{0}` puts an argument first, and an
argument can be user text beginning with `<html>`. `BundleHeadGateTest`
derives its population from the built bundles and refuses exactly that
shape at a paint sink — the markup-render class (v1.208.0, v2.86.0)
carried across an arc that could easily have re-opened it. Two more
shapes of the same trust surfaced late: a sweep turns one-literal
calls into plural-aware TERNARIES of two bundle values, so
`PlainStatusGateTest` now accepts a conditional only when EVERY branch
is accepted; and thirteen dialog options compared with `==` were safe
as interned literals and are not safe as `Bundle.X()` calls, which
SpotBugs said out loud at each site.

**A floor is what catches a scan that found nothing.**
`LocaleBundleParityTest` reads the assembled cluster, and its first
run in a clean build reported perfect parity — over zero jars. It
filtered on `org-nmox-studio-` while a module's code-name-base makes
the assembled jar `org-nmox-NMOX-Studio-ui.jar`. Every per-item
assertion passed vacuously; only the floor on how many packages it
expected to see could fail. The gate now names the prefix it wants and
refuses an empty match, and the floor is the measured 71. The rule
generalizes: *a gate that derives its population from an artifact
needs an assertion that the population is not empty*, or a clean build
is the one place it will lie.
