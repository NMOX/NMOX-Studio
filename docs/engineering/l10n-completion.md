...
| The editor toolbar (3.2.0) — Back, Forward, Last Edit, the find buttons, Toggle Highlight Search, rectangular selection, the bookmarks, Shift Line, macro recording, Comment/Uncomment, and the SQL and XML editors' own buttons | `MainWindowChromeSpeaksTest` — the buttons DERIVED from every `Editors/**/Toolbars/Default` registration; a button's tooltip key derived from its action's layer `ShortDescription` where the layer states one, else a checked ledger (`editor-toolbar.txt`) read from the running toolbar and the action's bytecode. Two measured surprises: Back and Forward paint `NavigationHistory*Action_Tooltip` (NbEditorKit installs its own action over editor-lib's `jump-list-*`, whose overlay the button never read), and the bookmark buttons paint their key with the mnemonic cut, so the overlay carries one. `Go back to {0}` (history non-empty) is held by bytecode and the gate, not walked |
