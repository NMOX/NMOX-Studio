...
| **`settings.json`** | 工具 ▸ 选项（macOS 上是 NMOX Studio ▸ Settings…）。你项目的 `.editorconfig` 在保存时会被遵守。 |
| **问题面板** | **操作项**（⌘6）：来自机架上 PURITY 和 TYPEGUARD 设备的代码检查和类型检查结果，与编辑器里的波浪线相对应。 |
| **大纲** | **导航器**（⌘7）。 |
...
- **在 Linux 上，Ctrl+\` 以前会打开窗口切换器** —— 那是平台为抢占 Ctrl+Tab 的桌面（KDE）准备的后备方案。现在它打开终端；切换器仍然在 Ctrl+Tab 上。
