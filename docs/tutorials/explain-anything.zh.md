...
![KVASIR 正在解释一次真实的失败运行](../images/zh/kvasir-explain.png)
...
每一副面孔都会打开一个对话窗口：接着追问，模型能看到这场交流的完整历史（上限为十轮交流，会写在对话记录里）。**快速/深入**的选择（所选服务商的快速模型和强力模型）会被记住，并在每场对话里固定下来，这样对话记录永远不会谎报是谁回答的。
