...
Вставте її в `.mcp.json` вашого агента або натисніть **Копіювати для Claude
Code** — і отримаєте один рядок, який Claude Code приймає натомість:

```bash
claude mcp add --transport http nmox-studio http://127.0.0.1:PORT/mcp --header "Authorization: Bearer TOKEN"
```

Токен ніколи не потрапляє в журнал. Якщо ви не попросите іншого, він існує
лише в цьому діалозі й зникає разом із портом, тож наступний запуск дає нову
адресу й новий токен. Позначте **Зберігати цю адресу й токен** — і його буде
збережено в сховищі ключів системи й використано знову, з тим самим портом,
тож агент, налаштований один раз, під’єднається й завтра; якщо цей порт
зайняла інша програма, NMOX Studio відкриває новий і повідомляє про це.
Позначте ще **Запускати разом із NMOX Studio** — і порт стартує разом з IDE.
Зняття позначки «Зберігати» видаляє запис зі сховища ключів.
**Зупинити Agent Port** завершує роботу; так само діє вихід з IDE.
