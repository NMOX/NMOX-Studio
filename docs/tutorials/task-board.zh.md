...
![任务看板：三列、一张正在计时的卡片，以及标题栏里实时走动的计时](../images/task-board.png)

![看板概览中的一个冲刺 — 燃尽线画在理想线之上](../images/sprint-overview.png)
...
7. **写站会纪要。**按**站会…**。报告以 markdown 形式打开，附带一个**复制到剪贴板**按钮：**Yesterday** 和 **Today** 来自完成时间戳和按日切开的时段（正在走的计时器显示为“clock running”），**Blockers** 来自阻塞登记，**Commits (since yesterday)** 来自 `git log`。无话可说的小节会被省略，从不以空白的形式出现；报告的标题以冲刺及其天数开头（“Sprint 8 · day 3 of 14”）。

   ![一次点击就把看板变成每日报告](../images/standup.png)
