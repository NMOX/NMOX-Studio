...
![Студия блоков — палитра блоков, холст с корнем компонента и сгенерированный пользовательский элемент, где щелчок по блоку показывает его код](../images/tabs/block-studio.png)
...
2. **Добавьте блоки из палитры.** Перетащите блок **Элемент** (узел DOM)
   и задайте ему текст; добавьте поле **Состояние**; добавьте обработчик,
   который по щелчку переключает класс. Разрешены только допустимые
