...
الزقوها في ‎`.mcp.json` بتاع الوكيل بتاعكم، أو دوسوا **نسخ لـ Claude Code**
عشان تاخدوا السطر الواحد اللي Claude Code بياخده بدالها:

```bash
claude mcp add --transport http nmox-studio http://127.0.0.1:PORT/mcp --header "Authorization: Bearer TOKEN"
```

التوكن عمره ما بيتسجل. إلا لو طلبتوا غير كده، هو موجود في الديالوج ده بس
وبيموت مع البورت، فالتشغيل الجاي بيبقى ليه عنوان جديد وتوكن جديد. علّموا على
**الاحتفاظ بالعنوان والتوكن دول** وهيتحفظ في سلسلة المفاتيح بتاعة السيستم
ويتستخدم تاني، بنفس البورت، فالوكيل اللي اتظبط مرة واحدة يفضل يتوصل بكرة
كمان؛ ولو برنامج تاني أخد البورت ده، NMOX Studio بيربط بورت جديد وبيقول كده.
علّموا كمان على **التشغيل مع NMOX Studio** والبورت هيقوم مع الـ IDE. إلغاء
العلامة من «الاحتفاظ» بيمسح المدخل من سلسلة المفاتيح.
**إيقاف الـ Agent Port** بيقفله؛
