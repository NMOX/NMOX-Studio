...
![Диалог Agent Port — петлевая конечная точка, токен, создаваемый при каждом запуске (на снимке заглушка), и готовая конфигурация клиента для копирования](../images/tabs/agent-port.png)
...
**Сделайте:** Сервис ▸ **Agent Port (MCP)…** ▸ **Запустить**, затем
