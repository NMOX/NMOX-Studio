...
Вставьте её в `.mcp.json` вашего агента или нажмите **Копировать для Claude
Code** — и получите одну строку, которую Claude Code принимает вместо этого:

```bash
claude mcp add --transport http nmox-studio http://127.0.0.1:PORT/mcp --header "Authorization: Bearer TOKEN"
```

Токен никогда не пишется в журнал. Если вы не попросите иного, он существует
только в этом диалоге и умирает вместе с портом, так что следующий запуск
даёт новый адрес и новый токен. Отметьте **Сохранить этот адрес и токен** —
и он хранится в связке ключей системы и используется снова, с тем же портом,
так что агент, настроенный один раз, подключится и завтра; если этот порт
занят другой программой, NMOX Studio открывает новый и сообщает об этом.
Отметьте ещё **Запускать вместе с NMOX Studio** — и порт поднимается вместе
с IDE. Снятие отметки «Сохранить» удаляет запись из связки ключей.
**Остановить Agent Port** его завершает; выход из IDE
