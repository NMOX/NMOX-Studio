...
הדביקו אותה בקובץ ‎`.mcp.json` של הסוכן שלכם, או לחצו על **העתקה עבור
Claude Code** כדי לקבל את השורה האחת ש‑Claude Code מקבל במקום זאת:

```bash
claude mcp add --transport http nmox-studio http://127.0.0.1:PORT/mcp --header "Authorization: Bearer TOKEN"
```

האסימון לעולם אינו נרשם ביומן. אלא אם תבקשו אחרת, הוא קיים רק בתיבת
הדו‑שיח הזו ומת יחד עם הפורט, כך שבהפעלה הבאה יש כתובת חדשה ואסימון חדש.
סמנו **שמירת הכתובת והאסימון האלה** והוא יישמר במחזיק המפתחות של המערכת
וישמש שוב, עם אותו פורט, כך שסוכן שהוגדר פעם אחת יתחבר גם מחר; אם תוכנה
אחרת תפסה את הפורט הזה, NMOX Studio קושר פורט חדש ואומר זאת. סמנו גם
**הפעלה יחד עם NMOX Studio** והפורט עולה יחד עם ה‑IDE. ביטול הסימון של
השמירה מוחק את הרשומה ממחזיק המפתחות.
