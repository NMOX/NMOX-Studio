...
`Fichier ▸ Nouveau projet…` → « Vanilla Web » s’il vous en faut un).
...
2. **Ajoutez un exécuteur.** Faites glisser **IGNITION** de l’étagère d’appareils
   à gauche sur le rack. IGNITION est l’appareil « exécuter » polyglotte ;
   visant un projet Node, il lance `npm run dev` (il détecte tout seul
   votre gestionnaire de paquets et votre chaîne d’outils).

3. **Reliez-le au moniteur.** Cliquez **Arrière (Tab)** (ou pressez Tab)
   pour voir l’arrière, puis cliquez la prise **OUT** d’IGNITION et la
   prise **IN** de MONITOR — un cordon les relie. (Tirer d’une prise à
   l’autre marche aussi ; cliquer est plus simple quand le rack est large.)

4. **Déclenchez-le.** Revenez à l’avant et pressez le bouton **IGNITE**
...
5. **Enregistrez le montage.** Le bouton **Enregistrer le patch**
...
- **Les câbles coordonnent les voies.** OUT→IN est le câblage le plus
...
- Il y a 53 appareils — parcourez-les dans [devices.md](../devices.md) ou
  dans l’étagère d’appareils (clic droit sur un appareil monté pour son
  **Comment utiliser…**).
