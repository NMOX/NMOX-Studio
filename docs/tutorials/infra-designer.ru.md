...
![Стек обретает форму — DNS, балансировщик нагрузки, droplet и том с листом свойств; панель инструментов на лету считает стоимость схемы и честно говорит о режиме пробного прогона](../images/infra-designer.png)
...
5. **Разверните по-настоящему (когда будете готовы).** Добавьте токен
   поставщика в окне `Параметры` (он хранится в связке ключей ОС) — и
