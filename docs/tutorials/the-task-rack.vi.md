...
hiệu thật. Bài này dựng một bản đấu nối nhỏ xíu — chạy một thứ gì đó, và thắp
một đèn báo khi nó xong — để ẩn dụ ấy thấm vào.
...
Mở một dự án (dự án Node nào cũng được; `Tệp ▸ Dự án mới…` → “Vanilla JS”
...
2. **Thêm một thiết bị chạy.** Kéo **IGNITION** từ bảng chọn bên trái lên kệ.
...
3. **Đấu nó với MONITOR.** Nhấp nút **lật** để xem mặt sau, rồi nhấp giắc
   **OUT** của IGNITION và nhấp giắc **TAP** của MONITOR — một sợi dây patch
   nối chúng lại. (Kéo giữa các giắc cũng được; nhấp thì dễ hơn khi giá rộng.)

4. **Kích hoạt.** Lật về mặt trước và nhấn nút **GO** của IGNITION. Nó khởi
...
5. **Lưu bản đấu nối.** `⌘S` (hoặc nút **Lưu bản đấu nối**) ghi
...
- **Dây điều phối các làn.** OUT→TAP là sợi dây đơn giản nhất; cổng sẵn sàng
...
- Có 53 thiết bị — duyệt chúng ở [devices.md](../devices.md) hoặc trong các
  thẻ Cách dùng của bảng chọn.
