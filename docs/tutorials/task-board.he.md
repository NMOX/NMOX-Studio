...
![לוח המשימות: שלוש עמודות, כרטיס עם שעון פעיל, והמונה החי בכותרת](../images/task-board.png)

![ספרינט בסקירת הלוח — ה‑burndown מעל הקו האידיאלי](../images/sprint-overview.png)
...
7. **כתבו את הסטנדאפ.** לחצו **סטנדאפ…**. הדוח נפתח כ‑Markdown עם כפתור
   **העתקה ללוח העריכה**: **Yesterday** ו‑**Today** מחותמות הסיום ומסשנים
   שנחתכו לפי יום (שעון שרץ נקרא "clock running"), **Blockers** מהמרשם,
   **Commits (since yesterday)** מ‑`git log`. מקטעים שאין להם מה לומר
   מושמטים, ולעולם אינם מוצגים ריקים, והכותרת נפתחת בספרינט ובספירת הימים
   שלו ("Sprint 8 · day 3 of 14").

   ![לחיצה אחת הופכת את הלוח לדוח היומי](../images/standup.png)
