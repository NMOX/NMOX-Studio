...
![块工作室 — 积木面板、带组件根节点的画布，以及生成的自定义元素，点哪块积木就映射到哪段代码](../images/zh/tabs/block-studio.png)
...
2. **从面板添加积木。**拖一块**元素**积木（一个 DOM 节点），给它文字；加一个**状态**字段；再加一块**当事件发生**积木，在里面放一块**切换类**。只有合法的嵌套才被允许 — 画布会预先显示有效的放置位置，并拒绝非法的，就连载入时也一样。
