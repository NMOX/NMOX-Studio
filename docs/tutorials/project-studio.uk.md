...
![Студія проєкту в лівій панелі — рідне для платформи дерево файлів і панель інструментів проєкту, поряд відкрита Стійка задач](../images/tabs/project-studio.png)
...
1. **Створіть проєкт.** `Файл ▸ Новий проєкт…` → оберіть шаблон
   (Angular, Vue, Vanilla JS, Elixir/Phoenix, PHP LEMP та інші).
...
3. **Запустіть — без термінала.** Скористайтеся командою **Запустити** в
   IDE (або натисніть GO на IGNITION у стійці). Вона визначає менеджер
...
5. **Завантажте пресет.** Меню пресетів підключає готову стійку під робочий
