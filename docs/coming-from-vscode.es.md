...
| **`settings.json`** | Herramientas ▸ Opciones (en macOS, NMOX Studio ▸ Settings…). El `.vscode/settings.json` de un repositorio fija la sangría de sus archivos (`editor.tabSize`, `editor.insertSpaces`) y, al guardar, `files.trimTrailingWhitespace` y `files.insertFinalNewline`; su `.editorconfig` gana dondequiera que ambos digan algo, mientras escribes y al guardar. |
...
- **`launch.json` se lee, y lo que el depurador no puede respetar se
  ...
  ellos depuraría algo distinto de lo que dice el archivo. También
  pasaría con unos `args` escritos como una sola cadena (VS Code se la
  pasa a una consola) y con un valor de `env` que sea `null` (que elimina
  una variable). Lo mismo vale para `"request": "attach"`, una entrada de
