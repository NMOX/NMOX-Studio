...
Mula sa terminal, binubuksan ng `nmox .` ang folder na kinatatayuan mo, gaya ng `code .`: `cd myproject && nmox .`. Itinututok ang isang folder nang eksakto gaya ng pagtutok ng Buksan ang Folder… sa welcome page, may manifest man o wala; ang isang file ay bumubukas sa editor (`nmox src/app.js`). Agad bumabalik ang command — sinisimulan ng unang `nmox` ang IDE sa background, at ibinibigay ng bawat kasunod ang folder nito sa IDE na tumatakbo na. Ang `nmox` lang ay nagsisimula lamang ng IDE. Paano ilalagay ang `nmox` sa iyong PATH:
...
Sa macOS, gamitin ang `nmox .` o ang **Talaksan ▸ Buksan ang Folder…**. Hindi kayang ibigay ng *Open With* ng Finder at ng icon sa Dock ang isang folder sa isang nilagdaang Java app na sinisimulan sa paraang sinisimulan ang NMOX Studio, kaya hindi iniaalok ng app ang sarili nito roon.
