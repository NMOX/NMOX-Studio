...
> **macOS, unang pagbukas:** i-double-click ito. Minsang itatanong ng macOS kung bubuksan ang isang app na na-download mula sa internet, at sasabihin nitong sinuri na ito ng Apple: i-click ang **Open**. Ang app ay nilagdaan gamit ang Apple Developer ID at notarized, at nakakabit ang tiket sa app at sa DMG, kaya gumagana ang pagsusuri kahit offline — walang right-click at walang `xattr`. Ang panloob na updater ay nag-i-install sa iyong user directory imbes na sa app bundle, kaya hindi kailanman nasisira ng pag-update ang lagdang iyon.
>
> Kung ang isang install ng 3.0.0, 3.0.1 o 3.0.2 ay sumagot ng *"NMOX Studio.app" Not Opened* (hindi nabuksan), iyon ay depekto sa paraan ng pagsisimula ng app sa launcher script nito, na naayos sa 3.1.0: i-install ang 3.1.0 o mas bago (`brew upgrade --cask nmox-studio`, o isang bagong download).
...
Nag-aalok ang **Kasangkapan ▸ Mga Plugin ▸ Mga Update** (o ang **Tulong ▸ Tingnan ang mga update**) ng mga modyul ng produkto mula sa anumang mas bagong bersyon, mula sa update center na “NMOX Studio Updates”, na nakaturo sa pinakabagong release sa GitHub. I-install, i-restart kapag hiniling, tapos na. Tumitingin din nang kusa ang plataporma, lingguhan bilang default (baguhin ito sa **Kasangkapan ▸ Mga Plugin ▸ Mga Setting**), at hiwalay doon, minsan sa isang araw ay binabanggit ng IDE ang mas bagong release; patayin iyon sa Mga Opsyon ▸ Pangkalahatan (NMOX Studio ▸ Settings… sa macOS, Kasangkapan ▸ Mga Opsyon sa ibang OS). Nilagdaan ang bawat modyul at kasama sa produkto ang sertipiko, kaya naiinstall ang mga update nang walang tanong tungkol sa sertipiko.

Pinapalitan ng updater ang mga modyul, hindi ang application na nakapalibot sa mga ito. Ang kasamang Java runtime, ang launcher at ang NetBeans Platform mismo ay nagbabago lamang kapag nag-install ka ng release (`brew upgrade --cask nmox-studio`, o isang bagong download), at sinasabi iyon ng release notes ng release na nagbabago sa isa sa mga ito — halimbawa ang command na `nmox` at ang mga ayos sa macOS launcher sa 3.1.0. Hindi makapag-update sa loob ng app ang isang install na mas luma sa 2.35.0, dahil inilipat ng 2.35.0 ang plataporma: mag-install na lang ng kasalukuyang release.

<a id="2-first-launch"></a>
## 2. Unang pagbukas

Mula sa terminal, binubuksan ng `nmox .` ang folder na kinatatayuan mo, gaya ng `code .`: `cd myproject && nmox .`. Itinututok ang isang folder nang eksakto gaya ng pagtutok ng Buksan ang Folder… sa welcome page, may manifest man o wala; ang isang file ay bumubukas sa editor (`nmox src/app.js`), sa isang linya kung pangalanan mo ito gaya ng ginagawa ng `code -g` (`nmox src/app.js:42` — tinatanggap ang column, at bumubukas ang editor sa simula ng linya). Tinatanggihan sa terminal ang pangalang wala roon (`nmox: typo.js: no such file or folder`) sa halip na may simulan. Tinatanggap ang `-r` ng VS Code at binubuksan ng `-n` sa iisang bintana; tinatanggihan ayon sa pangalan ang `-a` at `-v`. Binubuksan ng `nmox -w file` ang file at naghihintay hanggang isara mo ang tab nito, at ikinukumpara ng `nmox -d left right` ang dalawang file sa diff view, kaya nagiging editor at difftool ng git ang NMOX Studio: `git config --global core.editor "nmox -w"`, at ipinapakita ng `nmox --help` ang mga linyang gumagawa rito na difftool at mergetool. I-save ang mensahe, isara ang tab, at magpapatuloy ang git. Kapag nakatakda ang mergetool (`mergetool.nmox.cmd` na `nmox -w "$MERGED"`), binubuksan ng `git mergetool` sa editor ang bawat file na may conflict. Sa file na iyon, at sa anumang file na may mga conflict marker ng git, may isang kulay ang kasalukuyang panig ng bawat conflict at iba naman ang papasok, at may babala ang linyang `<<<<<<<` na ang quick fix nito (⌘. sa Mac, Alt+Enter sa iba, o ang bumbilya sa gilid) ay nag-aalok ng tatlong pagpipilian ng VS Code: **Tanggapin ang kasalukuyang pagbabago**, **Tanggapin ang papasok na pagbabago** at **Tanggapin ang parehong pagbabago**. Iisang edit ang bawat isa, na binabawi ng iisang ⌘Z; kung nagbago ang bloke matapos lumabas ang babala, walang napapalitan at sinasabi iyon ng status line. Walang alok ang bloke na hindi eksaktong hugis ng git (pangalawang separator, bloke sa loob ng bloke), sa halip na hula. Ang mga kulay ay ang mga entry na “Merge conflict” sa Mga Opsyon ▸ Mga font at kulay ▸ Highlighting. Ipinapakita ng **Pangkat ▸ Gamitin ang NMOX Studio sa Git…** ang mga setting na iyon katabi ng mga halaga nila ngayon, at itinatakda ang mga ito para sa iyo. Bumubukas ang commit message bilang sariling file ng git: komento ang mga linyang `#`, ang isinulat mo lang ang sinusuri ang baybay, at may babala ang summary line na lumampas sa 72 character kung saan pumuputol ang mga tool ng git. Hina-highlight ng listahan ng `git rebase -i` ang bawat command at commit, at inaalis ng I-toggle ang comment ang isang linya nang hindi ito binubura. Kung hindi, agad bumabalik ang command — sinisimulan ng unang `nmox` ang IDE sa background, at ibinibigay ng bawat kasunod ang folder nito sa IDE na tumatakbo na. Ang `nmox` lang ay nagsisimula lamang ng IDE. Paano ilalagay ang `nmox` sa iyong PATH:

- **macOS, Homebrew:** ang cask na ang nagli-link nito para sa iyo.
- **macOS, mula sa DMG:** i-link (hindi kopyahin) ang launcher ng app —
  `sudo mkdir -p /usr/local/bin && sudo ln -s "/Applications/NMOX Studio.app/Contents/MacOS/nmox-studio" /usr/local/bin/nmox`.
  Kapag sinimulan sa pamamagitan ng link, alam nitong galing ito sa terminal; kapag sinimulan mula sa Finder o sa Dock, kumikilos ito gaya ng dati.
- **Windows:** ang kahong *Add "nmox" to PATH* ng installer, naka-tsek bilang default. Magbukas ng bagong terminal pagkatapos; ang terminal na bukas na ay may lumang PATH pa rin.
- **Linux:** ini-install ng `.deb` ang `/usr/bin/nmox`. Mula sa tarball, i-link ito mismo: `ln -s "$PWD/nmox-studio-<version>/bin/nmox" ~/.local/bin/nmox`.

Sa Linux at Windows, maibibigay mo rin ang isang folder sa NMOX Studio nang walang terminal, at itinututok ito sa parehong paraan:

- **Linux (ang `.deb`):** inililista ng iyong file manager ang NMOX Studio sa ilalim ng *Open With* para sa isang folder. Hindi ito nagiging default mo para sa mga folder; nananatiling ang file manager iyon.
- **Windows:** i-tsek ang kahong *Add "Open with NMOX Studio" to the right-click menu of folders in Explorer* ng installer (nagsisimula itong walang tsek, gaya ng sa VS Code). Pagkatapos ay nag-aalok ang Explorer ng **Open with NMOX Studio** sa isang folder at sa bakanteng espasyo sa loob nito; sa Windows 11 nasa ilalim ito ng *Show more options*. Inaalis ito ng pag-uninstall.

Sa macOS, gamitin ang `nmox .` o ang **Talaksan ▸ Buksan ang Folder…**. Hindi pa kayang ibigay ng *Open With* ng Finder at ng icon sa Dock ang isang folder sa NMOX Studio, kaya hindi iniaalok ng app ang sarili nito roon.

Bumubukas ang IDE kasama ang tatlong tab sa tabi ng lugar ng editor: **Maligayang Pagdating → Rack ng Gawain → Browser**. Isang ⌥⌘ shortcut lang ang layo ng bawat ibang window, at nakalista ang mga ito sa hanay na MGA KASANGKAPAN ng welcome page. Sa kaliwang dock: **Studio ng Proyekto** (puno ng mga file at mga template), ang batayang **Lugar ng Trabaho** at ang **Explorer ng NPM**. Gumagawa ng folder na `~/NMOX` bilang default na workspace; doon nakaturo ang rack hanggang magbukas ka ng proyekto.
...
| **⇧⌘P** | Mabilisang paghahanap din — ang kombinasyong tinatawag ng VS Code na Command Palette |
| **⌘9** | Rack ng Gawain |
| **⌥⌘0** | Lugar ng Trabaho |
| **⌥⌘1** | Task Board |
| **⌥⌘2** | Mga Test |
...
| **⌥⌘K / ⇧⌘L** | Bagong Eksperimento… / Bagong Lugar ng Pag-aaral… |
| **⇧⌘E** | Studio ng Proyekto, nakatutok sa puno ng file |
| **⇧⌘X** | Kasangkapan ▸ Mga Plugin |
| **⌃\`** | Isang terminal sa folder ng proyekto, o ang nakabukas na (Ctrl+\` sa Windows at Linux) |
| **⌥⌘P / ⌥⇧⌘K** | Lumipat ng Proyekto… / Mga Eksperimento… |

Galing sa VS Code? Itinutugma ng [Galing sa VS Code](coming-from-vscode.tl.md) ang mga kombinasyon at ang mga ideya, kasama ang pagsulat sa Windows at Linux sa tabi ng sa macOS.

**Bumubukas sa ⌘-click ang mga lokasyon sa Terminal** (Ctrl-click sa Windows at Linux). I-click ang lugar na ipinrint ng isang tool at bubukas ang file sa linyang iyon: `src/app.ts:42:7` mula sa tsc, `(/abs/app.js:10:5)` sa stack frame ng Node o Jest, `tests/test_x.py:12:` mula sa pytest, `--> src/main.rs:3:5` mula sa rustc, `File "x.py", line 12` mula sa traceback ng Python. Bumubukas ang absolute path gaya ng pagkaka-print nito. Binabasa ang relative mula sa folder ng itinututok na proyekto, kung saan sinisimulan ng ⌃\` ang shell nito; pagkatapos ng `cd` papasok sa isang subfolder, maaaring tumukoy ang path sa file na wala roon, at sinasabi ng status line kung aling path ang hinanap nito sa halip na manghula. Hindi kailanman link ang URL o ang `host:port`.
...
**Ang puno ng file** ay ang Studio ng Proyekto (⇧⌘E). I-right-click ang isang file o folder para sa Bago, Gupitin, Kopyahin, I-paste, Tanggalin at Palitan ang pangalan, at — gaya ng Explorer ng VS Code — **Kopyahin ang Path**, **Kopyahin ang Relative Path** (kaugnay ng proyekto) at **Ipakita sa Finder** (**Ipakita sa File Explorer** sa Windows, **Buksan ang Folder na Kinalalagyan** sa Linux). Para sa file na ine-edit mo, ganoon din ang ginagawa ng **Baguhin ▸ Kopyahin ang Path** (⌥⌘C, Ctrl+Alt+C sa Windows at Linux) at ng **Baguhin ▸ Kopyahin ang Relative Path** — mula rin sa terminal o sa puno, para sa editor na huli mong tinipahan.
...
**Ang mga eksperimento** ang pinakamabilis na paraan para subukan ang isang stack. Ang **Talaksan ▸ Bagong Eksperimento…** (⌥⌘K) ay pumipili ng template at gumagawa ng pansamantalang proyekto sa `~/.nmox/experiments`: walang git, walang kamakailan, pinagkakatiwalaan na, nakainstall na ang mga dependency — para **gumana agad ang unang Patakbuhin**. Bumubukas ito sa sarili nitong gabay na `EXPERIMENT.md`, na nagsasabi kung ano ang pipindutin, aling file ang babaguhin, at kung saan naroon ang talino ng IDE para sa stack na iyon. Itago ang nagiging kapaki-pakinabang: **Talaksan ▸ Mga Eksperimento…** ▸ **I-promote…** ang naglalabas nito at nag-uumpisa ng git, **I-duplicate** ang gumagawa ng kopya sa tabi para sa pangalawang paraan, at **Itapon…** ang nag-aalis ng iba. Ipinapakita ng istante ang edad ng bawat isa at ang nasukat nitong laki sa disk. Mas gusto mo ang gabay na landas? Inuuna ng dialog ang 93 espasyo ng pagkatuto.
...
**Itutok ang isang ahente sa iyong IDE.** Ang Kasangkapan ▸ Agent Port (MCP)… ay nagbubukas ng dulong MCP na maaaring tanungin ng katulong mula sa labas: ito ay **basahin-lamang ayon sa pagkakagawa**, patay hangga’t hindi mo binubuksan, nakikinig lamang sa lokal na interface, at humihingi ng tokeng nilikha nang buksan ito.

Napapalawak ang rack: maaaring magdagdag ng kagamitan ang mga plugin ng iba (i-install ang NBM nila sa Kasangkapan ▸ Mga Plugin). Upang sumulat ng isa, tingnan ang [device-spi.md](device-spi.md).
...
- **Ang minimap** — isang anyo ng buong file sa tabi ng scrollbar ng bawat editor; pindutin o hilahin upang mag-scroll. Laging kasya ang buong dokumento sa guhit: umuurong ang mga hanay habang lumalaki ang file. Ang Tingnan ▸ Minimap ay binubuksan at sinasara ito nang sabay para sa lahat ng bukás na editor.
...
- **LSP**: magbukas ng file na naka-install ang language server (typescript, gopls, rust-analyzer, pyright, …) at makakakuha ka ng mga diyagnostiko, hover, at pagpunta sa depinisyon. Nagiging mga hanay din sa **Mga aksyon** (⌘6) ang mga error at babala ng server, nakapangalan sa server (`[lsp:gopls]`), para sa bawat file na naiulat na ng server. May mga server na nag-uulat lamang tungkol sa mga file na bukas mo; ang gopls ay nag-uulat tungkol sa buong package. Walang server? Iniaalok ng IDE ang utos sa pag-install sa halip na tahimik na mabigo.

  ![Mga aksyon na naglilista ng dalawang error ng gopls, ang isa sa file na hindi kailanman binuksan, kasama ang bilang na ✕ 2 ⚠ 0 sa guhit ng kalagayan](images/lsp-action-items.png)

- **Sinusunod ang `.editorconfig`** — habang nagta-type ka at kapag nag-save ka. Ang `indent_style`, `indent_size` at `tab_width` ang nagpapasya kung ano ang isinusulat ng Tab, Enter at ng muling pag-indent, kaya tab ang nakukuha ng proyektong gumagamit ng tab at apat na espasyo ang sa proyektong apat na espasyo, kada file at kada seksyon ng glob; inilalapat ng bawat pag-save ang `trim_trailing_whitespace` at `insert_final_newline`. Umaabot sa mga bukás na editor ang pagbabago sa `.editorconfig` sa loob ng ilang segundo. Ang literal na tab na nasa file na ay iginuguhit pa rin sa lapad ng tab na nakatakda sa Mga Opsyon, at hindi inilalapat ang `charset` at `end_of_line`. Ang iyong mga kagamitan sa pag-format (GLOSS at iba pa) ang bahala sa iba.
- **Sinusunod din sa parehong paraan ang `.vscode/settings.json` ng isang repository**: ang `editor.tabSize`, `editor.insertSpaces` at `editor.indentSize` ang nagpapasya sa pag-indent, inilalapat sa pag-save ang `files.trimTrailingWhitespace` at `files.insertFinalNewline` (kapag `true`), at pinapalitan ng isang bloke ng wika gaya ng `"[typescript]": {…}` ang mga iyon para sa wika nito. Kapag may `.editorconfig` din ang proyekto, ang `.editorconfig` ang nananaig saanman parehong may sinasabi ang dalawa. Walang katapat dito ang `editor.detectIndentation` ng VS Code, na nagpapahintulot na manaig ang sariling pag-indent ng isang file.
...
Ang mga katalogo ng pagsasalin ng isang web project ay datos na binabasa ng editor, gaya ng iyong mga stylesheet at ng iyong `.env`. **Kasangkapan ▸ Suriin ang mga Salin…** ang humahanap ng mga katalogo (i18next, vue-i18n, svelte-i18n, XLIFF ng Angular, Lingui, Paraglide, react-intl, o ang sa I18n Kit), pumipili ng pinagmulang lengguwahe, at nag-uulat ng tatlong bagay bilang kulot na guhit at hanay sa Mga Gawain: **nawawala** (ang mga anyong pangmarami at pangkonteksto ay inihahambing sa batayang susi), **kapareho ng pinagmulan** (kinopya, hindi isinalin), at **hindi tugmang placeholder** — ito ang mali, dahil sirang pagsasalin ang may ibang hanay ng `{{name}}` o `%s`. Ang ikaapat, **hindi ginagamit**, lumilitaw lamang sa buong senso.

### Mga link sa iyong dokumentasyon

Binabasa sa GitHub ang isang README, at ang link doon na walang patutunguhan ay natutuklasan ng susunod na mambabasa, hindi mo. Binabasa ng **Kasangkapan ▸ Suriin ang mga Link ng Markdown…** ang bawat Markdown file sa tinutukang proyekto at sinusuri ang bawat relative na link at larawan ayon sa paraan ng pag-render ng GitHub: dapat umiral ang file o folder na pinangalanan nito, at dapat maging heading ng file na iyon ang isang `#heading` ayon sa anchor rule ng GitHub (maliliit na titik, walang bantas, gitling sa halip na espasyo, at may numerong `-1`, `-2` ang umuulit na heading). Mali ang link na walang patutunguhan; babala ang nawawalang heading, at ang link na lumalabas sa proyekto. Dumarating ang mga ito bilang kulot na guhit sa link at bilang hanay sa Mga Gawain, at binubuod ng status line ang takbo (`Mga link ng Markdown: 12 file, 148 link, 1 ang walang patutunguhan`).

Hindi sinusuri ang mga link na may scheme (`https:`, `mailto:`): walang lumalabas sa iyong makina. Hindi rin ang anumang nasa loob ng code fence o inline code, dahil halimbawa lamang ang link sa isang halimbawa, ni ang fragment ng link papunta sa isang source file (`app.js#L10` ang line anchor ng GitHub). Ang link na nagsisimula sa `/` ay binabasa mula sa ugat ng proyekto, gaya ng pagbasa rito ng GitHub.
...
Pindutin ang kaliwang gilid, piliin ang **I-debug ang file (mga breakpoint)**, at hihinto roon ang programa — kasama ang salansan, ang mga variable, at ang pagtaya ng mga ekspresyon. Gumagana ang JavaScript at TypeScript mula sa pabrika dahil sa kasamang adapter; gumagamit ang Python ng debugpy at ang Go ng delve, na ikaw ang mag-i-install. Ganoon din ang ginagawa ng **I-debug sa Chrome** para sa isang pahina: humihinto sa loob ng IDE ang mga breakpoint sa iyong pinagmulan habang tumatakbo ang browser sa isang pansamantalang profile. May isa pang pinto ang repository na may `.vscode/launch.json`: itipa ang pangalan ng isang configuration sa Mabilis na Paghahanap at sinisimulan ng Enter ang `program` na Node o Python ng configuration na iyon sa `cwd` nito, kasama ang `args` at `env` nito, o binubuksan ang `url` ng isang configuration na Chrome kasama ang `webRoot` nito; ang configuration na nagtatakda ng `envFile`, `runtimeExecutable` o anumang hindi maipapasa ng debugger ay tinatanggihan ayon sa pangalan sa guhit ng kalagayan sa halip na simulan nang wala ang mga iyon. Dumadaan muna ang lahat sa pagtatanong ng tiwala sa workspace.
...
Ang **Tingnan ▸ Mode ng presentasyon** ay sabay-sabay na nagpapalaki sa bawat bukás na editor, sa pahina sa nakapaloob na browser, sa bintana ng Output, at sa Terminal — at ibinabalik ang lahat nang eksakto pagkalabas mo. Ang **Tingnan ▸ Ipakita ang mga keystroke** ay ipinapakitang malaki ang kombinasyong katatapos mong pindutin, ngunit hindi kailanman ang tinitipa mo. Ang **Baguhin ▸ Kopyahin bilang Markdown** ay kinokopya ang pinili bilang nakabakod na bloke na may tamang tatak ng wika, at ang bersyong **may link** ay idinadagdag ang link ng GitHub sa mismong mga hanay na iyon. Ang **Baguhin ▸ Buksan sa GitHub** at **Baguhin ▸ Kopyahin ang GitHub link** (pati sa right-click, sa editor at sa puno ng Studio ng Proyekto) ay nagbibigay ng parehong link nang walang bloke: ang linya ng cursor o ang mga linya ng pinili, isang file, o isang folder (ang `tree` page nito; ang root folder ng proyekto ay ang front page ng repository). Sa sarili mong browser pumupunta ang Buksan, kung saan naka-sign in ka na, kaya gumagana ang blame at ang mga review comment. Tinatanggihan sa status line ang file na nasa labas ng git repository, ang repository na walang `origin`, o ang origin na hindi GitHub, at walang nakokopya — gayundin ang editor na may hindi pa naka-save na pagbabago, dahil magpapakita ang link ng mga linyang iba sa nakikita mo. Binubuksan ng **Pangkat ▸ Bagong Pull Request sa GitHub** (nasa menu rin ng tandang git) sa iyong browser ang mismong New Pull Request page ng GitHub para sa sangang naka-checkout sa iyo: ang hakbang pagkatapos ng push, nang hindi hinahanap nang mano-mano ang repository at ang sanga. Tinatanggihan ang detached HEAD, dahil wala itong sangang maipapanukala. Ang **Kasangkapan ▸ I-save ang Screenshot…** ay ipininta ang buong bintana sa doble ang laki, may mga bersyon para sa tab ng editor lamang, para sa clipboard, at para kopyahin ang puno ng proyekto bilang Markdown.
...
Pagkatapos nito, ang `↑2 ↓1` ay ang mga commit na mayroon ang iyong sanga na wala sa upstream nito (ipu-push) at ang kabaligtaran (ipu-pull), ipinapakita lamang kapag hindi zero at para lamang sa sangang may upstream. Nagsisimula ang menu nito sa **Lumipat ng Branch…**, **I-commit…**, **I-pull…** at **I-push…**, ang sariling mga dialog ng git module.

Sa tabi ng tanda, may mas tahimik na tala na sumasagot sa tanong na pinakamadalas mong itanong: sino ang sumulat ng linyang kinaroroonan ng cursor? Sinusundan ng `Ada Lovelace, 3 araw na ang nakalipas · Fix the parser` ang cursor, sa sarili mong wika, at pinangangalanan ng tooltip nito ang commit at ang petsa. Nag-aalok ang isang pindot ng **Ipakita ang mga Annotation** (ang sa buong file, ang parehong **Anotasyon** na inaalok ng tanda), **Buksan ang Commit sa GitHub** (ang sariling pahina ng commit, kapag GitHub remote ang origin) o **Kopyahin ang Commit ID**; sa linyang hindi pa naka-commit, diretso sa mga anotasyon ang pindot. Isang beses itong nagtatanong sa git sa bawat naka-save na bersyon ng file, hindi kailanman sa pagsisimula at hindi kailanman para sa file na nasa labas ng repository, at wala itong sinasabi para sa file na hindi sinusubaybayan ng git. **Hindi pa naka-commit** ang mababasa sa linyang binago mo pero hindi pa naka-commit. Habang may hindi pa naka-save na pagbabago ang file, **May-akda ng linya: may mga pagbabagong hindi pa naka-save** ang mababasa sa halip na pangalan, dahil binabasa ng git ang file gaya ng pagka-save nito, at hindi kailangang ang linya 40 sa screen ang linya 40 sa disk; i-save at babalik ang pangalan. Pinapatay at binubuksan ng **Tingnan ▸ May-akda ng linya** ang tala.
...
Ang mga pahina sa masalimuot na sulat ay ipinipinta nang may tamang hugis: pinagdudugtong ng Arabic, Persian, Urdu (kasama ang Nastaliq), Kurdish, Pashto, Sindhi, Uyghur, Syriac, Thaana at N’Ko ang kanilang mga titik at binabasa sa sarili nilang pagkakasunod, at ang mga numero sa sarili din nilang pagkakasunod; inilalagay ng Hindi at ng iba pang sulat na Indic (Bengali, Gurmukhi, Gujarati, Oriya, Tamil, Telugu, Kannada, Malayalam, Sinhala), ng Thai, Tibetan, Myanmar at Khmer sa tamang lugar ang kanilang mga tanda ng patinig at mga pinagsamang titik; at ang mga tuldik na isinulat bilang hiwalay na karakter (gaya ng pagsulat ng macOS sa mga pangalan ng file) ay nakapatong sa kanilang titik. Hindi ito ginagawa ng WebKit ng JavaFX nang mag-isa. Sa macOS at Windows, binubuksan ng Browser ang sariling makina ng WebKit para sa masalimuot na teksto, kaya eksaktong nasusukat ang mga field ng form, ang makapal at nakahilig na parirala, ang mga naka-justify na talata at ang mga pinili. Sa Linux, kung saan wala ang switch na iyon, ang Browser mismo ang humuhugis sa teksto at iniaangkop ang tantiya nito ng lapad sa mga font na ini-install ng iyong distribusyon, kaya ang pariralang puno ng pinagsamang titik o nakasulat sa Nastaliq ay maaaring lumihis nang ilang pixel. Hindi hinuhugis ang Lao: pati ang sariling teksto ng JavaFX ay naglilihis ng mga patinig nito. Tama ang pagpapakita ng Hebrew, Armenian, Georgian at Ethiopic nang mag-isa, kasama ang niqqud.
...
### Ang tanda na ⇄ naghahain

Lumilitaw sa guhit ng kalagayan ang tandang **⇄ naghahain** tuwing may mga tagapaglingkod na gising: ang pagtakbo ng IDE mismo, ang mga kagamitang naglilingkod, at anumang utos na nagpalimbag ng lokal na adres. Pindutin at pumili ng isa: bubukas ito sa nakapaloob na pantingin, o sa pantingin ng sistema kapag hindi ito kaya ng tab na iyon.

### ⌘I, ang pangkalahatang tagahanap

Isang kahon lamang ang umaabot sa inyong mga proyekto (ang kamakailan at ang kilala), sa bawat kagamitan ng rak — tuwid sa mga kontrol nito —, sa mga **tagapaglingkod na gising** (Enter ang magbubukas nito sa pantingin), sa mga kahilingan ng Studio ng API, sa mga koneksyon at talahanayan ng Studio ng Database, sa mga kontrata, sa mga buko ng imprastruktura, at sa mga kard ng Pisara ng Gawain, at pinapangalanan ng hanap ang hanay na kinalalagyan ng kard. Umaabot din ito sa **mga pangalan ng utos ng VS Code** (*Format Document*, *Toggle Terminal*, *Git: Commit*, *Open Settings* — bawat isa ay nakalista sa ilalim ng *Mga utos ng VS Code* katabi ng aksyong gumagawa ng parehong bagay dito, kaya ang pangalan nito rito ang itatipa ninyo sa susunod). At umaabot ito sa mga **npm script** ng tinutukang proyekto: itipa ang `dev` o `test` at mababasa sa hanap ang *Patakbuhin ang script: dev — vite*; pinapatakbo ito ng Enter gamit ang sariling package manager ng proyekto (npm, yarn o pnpm), eksaktong gaya ng double-click sa Explorer ng NPM — nagtatanong muna ang Tiwala sa Workspace sa proyektong hindi pa ninyo pinagkakatiwalaan, sumasama ang pagtakbo sa ■ ng toolbar, at pinapailaw ng dev server na nagpalimbag ng adres ang tandang ⇄. Sa isang monorepo, ang mga script ay ang mga ipinapakita ng Explorer ng NPM. Ganoon din inililista ang mga task ng isang repository na may `.vscode/tasks.json` — *Patakbuhin ang task: build — make all* — at pinapatakbo ng Enter ang task sa likod ng parehong tanong ng tiwala, sa bintana ng Output at sa ilalim ng ■ ng toolbar; tumatakbo ang shell task sa shell na gagamitin ng VS Code (ang inyong `$SHELL`, isang login shell sa macOS; PowerShell sa Windows) o sa shell na pinangangalanan ng `options.shell` nito; ang task na nangangailangan ng halagang VS Code lamang ang makapagbibigay, o umaasa sa ibang task, ay sinasabi iyon sa guhit ng kalagayan sa halip na tumakbo. Inililista ng `.vscode/launch.json` ng repository ang mga configuration nito sa tabi nila — *I-debug: Launch Program — ${workspaceFolder}/server.js* — at sinisimulan ng Enter ang breakpoint debugger sa configuration na iyon pagkatapos ng parehong tanong ng tiwala.

**Ang Hanapin sa mga proyekto (⇧⌘F) ay naghahanap sa inyong code, hindi sa binabalewala ng repositoryo.** Sa isang git na repositoryo, nilalaktawan nito ang anumang binabalewala ng sariling mga file na `.gitignore` ng repositoryo (ang nasa ugat at anumang nakapaloob sa mga subfolder) at ng `.git/info/exclude`, pati ang `.git` mismo — kaya ang paghahanap ng pangalan ng isang function ay nakakatagpo ng `src/`, hindi ng mga kopya sa `node_modules` at `dist/` na nakalista sa `.gitignore` ng mga template. Hindi ito lumalampas kailanman sa binabalewala ng git: hinahanapan ang folder na sinusubaybayan ng inyong repositoryo, anuman ang pangalan nito. Sa labas ng repositoryo, nilalaktawan din nito ang mga folder ng build at ng package na nilalaktawan ng bawat paglalakad sa produkto (`node_modules`, `dist`, `build`, `out`, `target`, `coverage`, `.next`, `.nuxt`, `.svelte-kit`, `.angular`, `.venv`, `__pycache__`). Nananatili ang mga folder sa puno ng proyekto; ang paghahanap lamang ang lumalampas sa kanila. Para hanapan pa rin ang mga iyon, lagyan ng tsek ang **Maghanap sa mga generated na source** sa dialog na Hanapin sa mga proyekto (natatandaan ito hanggang alisin ninyo ang tsek); ang Palitan sa mga proyekto ay walang ganoong kahon at hindi kailanman binabago ang binabalewala ng repositoryo. Hindi binabasa ang inyong pandaigdigang git excludes file, kaya hinahanapan pa rin ang isang path na siya lamang ang bumabalewala.

### Sinasabi ng guhit ng kalagayan kung ano ang buhay

Katabi ng tanda ng mga tagapaglingkod ay ang tinutukang proyekto kasama ang hanay ng kasangkapan nito, at ang sanga ng Git kasama ang bilang ng talaksang binago ninyo. Habang may problema ang anumang sinusuri ng IDE, ipinapakita ng bilang na **✕ 2 ⚠ 1** ang mga error at babala mula sa bawat language server at kasangkapan; pindutin ito para buksan ang **Mga aksyon**. Lahat ito ay binabasa mula sa disk o mula sa mga talaang itinatago na ng produkto: ang pagtingin ay walang halagang proseso.
...
Ang Kasangkapan ▸ Mga Opsyon ▸ Mga keyboard shortcut (sa macOS: NMOX Studio ▸ Settings… ▸ Mga keyboard shortcut) ay nagpapalit ng buong hanay: ang mga galaw at ang gupit at dikit ng Emacs sa bawat patnugot, o ang mga hanay ng Eclipse at IDEA kung doon nakatira ang alaala ng inyong mga daliri. Nakatala sa lahat ng limang hanay ang bawat daglat ng NMOX (ang pamilyang ⌥⌘ ng mga bintana, ang ⌘P na Pumunta sa file, ang ⌥⌘E ng Emmet, ang mga daglat ng VS Code), kaya hindi kailanman nagagastos ng pagpapalit ang mga daglat ng mga studio. May isang sadyang eksepsiyon: sa hanay ng Eclipse, nananatiling sariling *Switch to Editor* ng Eclipse ang ⇧⌘E, dahil iyon ang inaasahan ng taong pumili ng Eclipse.
...
Ang **Ano ang Bago…** ay nagdadala ng mga tala ng bersiyong pinapatakbo ninyo, kasama sa mismong pagbuo; sa unang pagbukas matapos ang pagbabago, kusa itong bumubukas dala ang mga labas na hindi pa nakikita ng inyong instalasyon. Ang **Mag-ulat ng Problema…** ay bumubuo ng ulat mula sa inyong kapaligiran at sa huling apatnapung guhit ng talaan, sinala na — ang inyong sariling tahanan ay nagiging `~`, ang inyong pangalan sa pagpasok ay `<user>`, at anumang mukhang lihim ay `[redacted]` —; inaayos ninyo ito, at ang **Buksan sa GitHub** ang magpupuno ng isang isyung kayo mismo ang magpapadala, o kokopyahin ninyo. Hindi kailanman nagpapadala ng anuman ang produkto sa sarili nitong pasya. Ang **Mga Keyboard Shortcut…** ay naglilista ng bawat daglat ng NMOX sa inyong gamit na hanay, pati ang mga pangkalahatan (ang mga pinto ng pagsalubong na ⌥⌘K / ⇧⌘N / ⇧⌘L), binasa mula sa buhay na mapa ng teklado, kaya hindi ito maaaring lumihis sa ginagawa ng mga menu.

<a id="12-when-somethings-wrong"></a>
## 12. Kapag may mali

### Ang Doktor ng Environment

Nasa menu na Kasangkapan, sinisiyasat nito nang buhay ang 66 panlabas na kasangkapan — node, npm, docker, forge, composer, gopls… — at ipinapakita ang natagpuang bersiyon at ang utos ng pag-instala para sa nawawala.
