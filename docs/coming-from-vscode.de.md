...
| **`settings.json`** | Extras ▸ Optionen (unter macOS NMOX Studio ▸ Einstellungen…). Die `.vscode/settings.json` eines Repositorys legt die Einrückung seiner Dateien fest (`editor.tabSize`, `editor.insertSpaces`) und beim Speichern `files.trimTrailingWhitespace` und `files.insertFinalNewline`; seine `.editorconfig` gewinnt überall, wo beide etwas sagen, beim Tippen und beim Speichern. |
