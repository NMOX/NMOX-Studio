...
| **`settings.json`** | Extras ▸ Optionen (unter macOS NMOX Studio ▸ Einstellungen…). Die `.editorconfig` Ihres Projekts wird beim Speichern beachtet. |
| **Problemfenster** | **Aufgabenliste** (⌘6): Lint- und Typbefunde der Rack-Geräte PURITY und TYPEGUARD, neben den Wellenlinien im Editor. |
...
- **Unter Linux öffnete Strg+\` früher den Fensterwechsler** — der Ausweg der Plattform für Arbeitsumgebungen (KDE), die Strg+Tab für sich beanspruchen. Jetzt öffnet es das Terminal; der Wechsler bleibt auf Strg+Tab.
