...
La ligne `brew trust` est la confirmation unique de Homebrew pour tout tap tiers — on ne vous la redemandera pas lors des mises à jour. L’application est signée avec un Apple Developer ID et notariée par Apple, donc Gatekeeper l’accepte telle quelle — le cask la copie et ne lui fait rien d’autre. L’installation manuelle depuis le DMG fonctionne de la même façon.
...
Depuis un terminal, `nmox .` ouvre le dossier où vous vous trouvez, comme le fait `code .` : `cd myproject && nmox .`. Un dossier est visé exactement comme le vise « Ouvrir un dossier… » sur la page d’accueil, qu’il ait un manifeste ou non ; un fichier s’ouvre dans l’éditeur (`nmox src/app.js`), à une ligne précise si vous en nommez une comme le fait `code -g` (`nmox src/app.js:42` — une colonne est acceptée et l’éditeur s’ouvre au début de la ligne). Un nom qui n’existe pas est refusé dans le terminal (`nmox: typo.js: no such file or folder`) au lieu de démarrer quoi que ce soit. Le `-r` de VS Code est accepté et `-n` ouvre dans l’unique fenêtre ; les options `-a` et `-v` sont refusées par leur nom. `nmox -w file` ouvre le fichier et attend que vous fermiez son onglet, et `nmox -d left right` compare deux fichiers dans la vue des différences, ce qui fait de NMOX Studio l’éditeur et le difftool de git : `git config --global core.editor "nmox -w"`, et `nmox --help` affiche les lignes qui en font le difftool et le mergetool. Enregistrez le message, fermez l’onglet, et git poursuit. Une fois le mergetool réglé (`mergetool.nmox.cmd` sur `nmox -w "$MERGED"`), `git mergetool` ouvre chaque fichier en conflit dans l’éditeur. Dans ce fichier, et dans tout fichier qui porte les marqueurs de conflit de git, le côté actuel de chaque conflit est teinté d’une couleur et le côté entrant d’une autre, et la ligne `<<<<<<<` porte un avertissement dont le correctif rapide (⌘. sur Mac, Alt+Enter ailleurs, ou l’ampoule dans la marge) propose les trois choix de VS Code : **Accepter la modification actuelle**, **Accepter la modification entrante** et **Accepter les deux modifications**. Chacun est une seule modification, qu’un seul ⌘Z annule ; si le bloc a changé depuis l’apparition de l’avertissement, rien n’est remplacé et la barre d’état le dit. Un bloc qui n’a pas exactement la forme de git (un second séparateur, un bloc dans un bloc) ne reçoit aucune proposition plutôt qu’une supposition. Les teintes sont les entrées « Conflit de fusion » dans Options ▸ Polices et couleurs ▸ Highlighting. **Équipe ▸ Utiliser NMOX Studio avec Git…** affiche ces réglages à côté des valeurs qu’ils ont aujourd’hui, et les applique pour vous. Un message de commit s’ouvre comme le fichier propre à git : les lignes `#` sont des commentaires, seul ce que vous écrivez passe au correcteur orthographique, et une ligne de résumé qui dépasse les 72 caractères où les outils de git coupent reçoit un avertissement. Une liste `git rebase -i` met en évidence chaque commande et chaque commit, et Commenter/décommenter écarte une ligne sans la supprimer. Sinon, la commande rend la main aussitôt — le premier `nmox` démarre l’IDE en arrière-plan, et chacun des suivants confie son dossier à l’IDE déjà lancé. `nmox` seul démarre simplement l’IDE. Pour mettre `nmox` dans votre PATH :
...
Sous Linux et Windows, vous pouvez aussi confier un dossier à NMOX Studio sans terminal, et il est visé de la même façon :
...
Sous macOS, utilisez `nmox .` ou **Fichier ▸ Ouvrir un dossier…**. Le *Ouvrir avec* du Finder et l’icône du Dock ne peuvent pas encore confier un dossier à NMOX Studio ; l’application ne s’y propose donc pas.
...
| **⌥⌘4** | Navigateur web (WebKit intégré, avec DevTools) |
| **⌥⌘5** | Studio de blocs |
...
**Les emplacements dans le terminal s’ouvrent d’un ⌘-clic** (Ctrl-clic sous Windows et Linux). Cliquez sur un endroit qu’un outil a affiché, et le fichier s’ouvre à cette ligne : `src/app.ts:42:7` de tsc, `(/abs/app.js:10:5)` dans une trame de pile de Node ou de Jest, `tests/test_x.py:12:` de pytest, `--> src/main.rs:3:5` de rustc, `File "x.py", line 12` d’une trace Python. Un chemin absolu s’ouvre tel qu’il est affiché. Un chemin relatif est lu depuis le dossier du projet visé, là où ⌃\` démarre son shell ; après un `cd` dans un sous-dossier, le chemin peut nommer un fichier qui n’y est pas, et la barre d’état dit quel chemin elle a cherché au lieu de deviner. Une URL ou un `host:port` n’est jamais un lien.
...
**L’arborescence des fichiers** est le Studio de projet (⇧⌘E). Un clic droit sur un fichier ou un dossier propose Nouveau, Couper, Copier, Coller, Supprimer et Renommer, et — comme dans l’Explorateur de VS Code — **Copier le chemin**, **Copier le chemin relatif** (relatif au projet) et **Afficher dans le Finder** (**Afficher dans l’Explorateur de fichiers** sous Windows, **Ouvrir le dossier parent** sous Linux). Pour le fichier que vous éditez, **Édition ▸ Copier le chemin** (⌥⌘C, Ctrl+Alt+C sous Windows et Linux) et **Édition ▸ Copier le chemin relatif** font de même — depuis le terminal ou l’arbre aussi, pour l’éditeur dans lequel vous avez tapé en dernier.
...
- **LSP** — ouvrez un fichier dont le serveur de langage est installé (typescript, gopls, rust-analyzer, pyright, …) et vous obtenez les diagnostics, les infobulles et l’accès à la définition. Les erreurs et avertissements du serveur sont aussi des lignes dans **Éléments à traiter** (⌘6), au nom du serveur (`[lsp:gopls]`), pour chaque fichier sur lequel le serveur s’est prononcé. Certains serveurs ne signalent que les fichiers que vous avez ouverts ; gopls signale tout le paquet. Il manque un serveur ? L’IDE propose la commande d’installation au lieu d’échouer en silence.

  ![Éléments à traiter listant deux erreurs de gopls, dont une dans un fichier jamais ouvert, avec le compte ✕ 2 ⚠ 0 dans la barre d’état](images/lsp-action-items.png)
...
- **Le `.vscode/settings.json` d’un dépôt est respecté de la même façon** : `editor.tabSize`, `editor.insertSpaces` et `editor.indentSize` décident de l’indentation, `files.trimTrailingWhitespace` et `files.insertFinalNewline` (quand ils valent `true`) s’appliquent à l’enregistrement, et un bloc de langage comme `"[typescript]": {…}` les remplace pour son langage. Là où le projet a aussi un `.editorconfig`, c’est le `.editorconfig` qui l’emporte partout où les deux disent quelque chose. `editor.detectIndentation` de VS Code, qui laisse l’indentation propre d’un fichier l’emporter, n’a pas d’équivalent ici.
...
### Les liens de votre documentation

Un README se lit sur GitHub, et un lien qui n’y mène nulle part est trouvé par le lecteur suivant, pas par vous. **Outils ▸ Vérifier les liens Markdown…** lit chaque fichier Markdown du projet visé et vérifie chaque lien et chaque image relatifs comme GitHub les affichera : le fichier ou le dossier nommé doit exister, et un `#heading` doit être un titre de ce fichier selon la règle d’ancre de GitHub (en minuscules, ponctuation retirée, espaces changées en tirets, un titre répété numéroté `-1`, `-2`). Un lien qui ne mène nulle part est une erreur ; un titre manquant, et un lien qui sort du projet, sont des avertissements. Ils arrivent en soulignements sur le lien et en lignes dans les tâches, et la barre d’état résume le passage (`Liens Markdown : 12 fichiers, 148 liens, 1 ne mène nulle part`).

Les liens munis d’un schéma (`https:`, `mailto:`) ne sont pas vérifiés : rien ne quitte votre machine. Ni ce qui se trouve dans un bloc de code ou un code en ligne, puisqu’un lien dans un exemple est un exemple, ni le fragment d’un lien vers un fichier source (`app.js#L10` est l’ancre de ligne de GitHub). Un lien qui commence par `/` se lit depuis la racine du projet, comme GitHub le lit.
...
Cliquez dans la marge, choisissez **Déboguer le fichier (points d’arrêt)** et le programme s’arrête là — avec la pile, les variables et l’évaluation d’expressions. JavaScript et TypeScript marchent d’origine grâce à l’adaptateur embarqué ; Python passe par debugpy et Go par delve, que vous installez vous-même. Un dépôt qui porte `.vscode/launch.json` offre une autre porte : tapez le nom d’une configuration dans la Recherche rapide, et Entrée démarre le `program` Node ou Python de cette configuration dans son `cwd`, avec ses `args` et son `env`, ou ouvre l’`url` d’une configuration Chrome avec son `webRoot` ; une configuration qui fixe `envFile`, `runtimeExecutable` ou toute autre chose que le débogueur ne peut pas transmettre est refusée par son nom dans la barre d’état au lieu d’être démarrée sans. **Déboguer dans Chrome** fait de même pour une page : les points d’arrêt de votre source s’arrêtent dans l’IDE pendant que le navigateur tourne sur un profil jetable. Tout passe d’abord par la confirmation de confiance de l’espace de travail.
...
**Affichage ▸ Mode présentation** agrandit d’un coup tous les éditeurs, la page du navigateur intégré, la fenêtre de sortie et le terminal — et remet tout exactement comme c’était en sortant. **Affichage ▸ Afficher les frappes** montre en grand l’accord que vous venez de presser, mais jamais ce que vous tapez. **Édition ▸ Copier en Markdown** copie la sélection en bloc délimité avec la bonne étiquette de langage, et sa variante **avec lien** ajoute le lien GitHub vers ces mêmes lignes. **Édition ▸ Ouvrir sur GitHub** et **Édition ▸ Copier le lien GitHub** (aussi au clic droit, dans l’éditeur et dans l’arborescence du Studio de projet) donnent le même lien sans le bloc : la ligne du curseur ou les lignes de la sélection, un fichier, ou un dossier (sa page `tree` ; le dossier racine du projet est la page d’accueil du dépôt). Ouvrir passe par votre propre navigateur, où vous êtes connecté, si bien que blame et les commentaires de revue fonctionnent. Un fichier hors d’un dépôt git, un dépôt sans `origin` ou un origin qui n’est pas GitHub est refusé dans la barre d’état sans rien copier, de même qu’un éditeur aux modifications non enregistrées, puisque le lien montrerait d’autres lignes que celles que vous voyez. **Équipe ▸ Nouvelle pull request sur GitHub** (aussi dans le menu de la pastille git) ouvre dans votre navigateur la page New Pull Request de GitHub lui-même pour la branche que vous avez extraite : l’étape d’après un push, sans chercher le dépôt et la branche à la main. Un HEAD détaché est refusé, puisqu’il n’a aucune branche à proposer. **Outils ▸ Enregistrer la capture d’écran…** peint la fenêtre entière au double de la taille, avec des variantes pour l’onglet d’édition seul, pour le presse-papiers, et pour copier l’arborescence du projet en Markdown.
...
Après elle, `↑2 ↓1` sont les commits que votre branche a et que son amont n’a pas (à pousser), et l’inverse (à tirer), affichés seulement s’ils ne sont pas nuls et seulement pour une branche qui a un amont. Son menu commence par **Changer de branche…**, **Valider…**, **Tirer (pull)…** et **Pousser (push)…**, les boîtes de dialogue du module git lui-même.

À côté de la pastille, une note plus discrète répond à la question que vous posez le plus souvent : qui a écrit la ligne où se trouve le curseur ? `Ada Lovelace, il y a 3 jours · Fix the parser` suit le curseur, dans votre langue, et son infobulle nomme le commit et la date. Un clic propose **Afficher les annotations** (celles du fichier entier, le même **Annoter** que propose la pastille), **Ouvrir le commit sur GitHub** (la page du commit lui-même, quand origin est un dépôt distant GitHub) ou **Copier l’identifiant du commit** ; sur une ligne pas encore validée, le clic mène directement aux annotations. La note interroge git une fois par version enregistrée du fichier, jamais au démarrage et jamais pour un fichier hors d’un dépôt, et ne dit rien d’un fichier que git ne suit pas. Une ligne que vous avez modifiée sans la valider affiche **Pas encore validé**. Tant que le fichier a des modifications non enregistrées, la note affiche **Auteur de la ligne : modifications non enregistrées** au lieu d’un nom, car git lit le fichier tel qu’il est enregistré, et la ligne 40 à l’écran n’est pas forcément la ligne 40 sur le disque ; enregistrez, et le nom revient. **Affichage ▸ Auteur de la ligne** masque et réaffiche la note.
...
Une seule boîte atteint vos projets (récents et connus), chaque module du rack — en sautant droit à ses commandes —, les **serveurs en marche** (Entrée l’ouvre dans le navigateur), les requêtes du Studio d’API, les connexions et les tables du Studio de bases de données, les contrats, les nœuds d’infrastructure, les cartes du Tableau des tâches, dont le résultat nomme la colonne où elles se trouvent, les **noms de commandes de VS Code** (*Format Document*, *Toggle Terminal*, *Git: Commit*, *Open Settings* — chacun listé sous *Commandes VS Code* à côté de l’action qui fait la même chose ici, si bien que son nom d’ici est celui que vous taperez la fois suivante), et les **scripts npm** du projet visé : tapez `dev` ou `test` et le résultat se lit *Exécuter le script : dev — vite* ; Entrée le lance avec le gestionnaire de paquets du projet (npm, yarn ou pnpm), exactement comme le ferait un double-clic dans l’Explorateur NPM — la confiance de l’espace de travail est demandée d’abord sur un projet que vous n’avez pas approuvé, l’exécution rejoint le ■ de la barre, et un serveur de développement qu’elle annonce allume la pastille ⇄. Dans un monorepo, ce sont les scripts que montre l’Explorateur NPM. Un dépôt qui porte `.vscode/tasks.json` liste ses tâches de la même façon — *Exécuter la tâche : build — make all* — et Entrée exécute la tâche derrière la même question de confiance, dans la fenêtre Output et sous le ■ de la barre ; une tâche shell s’exécute dans le shell que VS Code utiliserait (votre `$SHELL`, un shell de connexion sous macOS ; PowerShell sous Windows) ou dans celui que nomme son `options.shell` ; une tâche qui a besoin d’une valeur que seul VS Code peut fournir, ou qui dépend d’une autre tâche, le dit dans la barre d’état au lieu de s’exécuter. Le `.vscode/launch.json` du dépôt liste ses configurations à côté — *Déboguer : Launch Program — ${workspaceFolder}/server.js* — et Entrée démarre le débogueur à points d’arrêt sur cette configuration après la même question de confiance.

**Rechercher dans les projets (⇧⌘F) cherche dans votre code, pas dans ce que le dépôt ignore.** Dans un dépôt git, la recherche passe tout ce qu’ignorent les propres fichiers `.gitignore` du dépôt (celui de la racine et ceux des sous-dossiers) et `.git/info/exclude`, ainsi que `.git` lui-même — chercher le nom d’une fonction trouve donc `src/`, et non les copies de `node_modules` et de `dist/` que liste le `.gitignore` des modèles. Elle ne va jamais au-delà de ce que git ignore : un dossier que votre dépôt suit est parcouru, quel que soit son nom. Hors d’un dépôt, elle passe aussi les dossiers de build et de paquets que tout parcours du produit passe (`node_modules`, `dist`, `build`, `out`, `target`, `coverage`, `.next`, `.nuxt`, `.svelte-kit`, `.angular`, `.venv`, `__pycache__`). Les dossiers restent dans l’arborescence du projet ; seule la recherche les laisse de côté. Pour y chercher malgré tout, cochez **Rechercher dans les sources générées** dans la boîte de dialogue Rechercher dans les projets (le choix est mémorisé jusqu’à ce que vous le décochiez) ; Remplacer dans les projets n’offre pas cette case et ne réécrit jamais ce que le dépôt ignore. Votre fichier global d’exclusions git n’est pas lu : un chemin que lui seul ignore est donc bien parcouru.

### La barre d’état dit ce qui est vivant

À côté de la pastille des serveurs : le projet visé et sa chaîne d’outils, et la branche Git avec le nombre de fichiers modifiés. Tout cela se lit sur le disque ou dans des registres que le produit tient déjà — regarder ne coûte aucun processus. Tant que quelque chose que l’IDE vérifie a un problème, un compte **✕ 2 ⚠ 1** montre les erreurs et avertissements de chaque serveur de langage et de chaque outil ; cliquez dessus pour ouvrir Éléments à traiter.
...
NMOX Studio parle quinze langues : English, Español, Français, Deutsch, Русский, Українська, Polski, Português (Brasil), Bahasa Indonesia, Filipino, Tiếng Việt, 简体中文, हिन्दी, עברית et العربية. Choisissez la vôtre dans **Options ▸ Général ▸ Langue** — chacune est écrite dans son propre nom, pour que vous trouviez toujours la vôtre. Le choix est inscrit dans vos réglages de lancement (`etc/nmoxstudio.conf`, sous forme d’argument `--locale`) et s’applique aussi à chaud. Ce qui change : les menus, les boîtes de dialogue, les infobulles, les barres d’état, l’accueil et les Options. Ce qui reste : le vocabulaire des façades du rack (GO, STOP, EXPLAIN — des étiquettes d’appareil, comme sur un synthétiseur), et les dialogues plus profonds de la plateforme, qui n’ont pas encore de traduction. Vous n’aurez peut-être jamais à choisir : une installation neuve parle déjà la langue de votre système, et depuis un pays que nous n’avons jamais nommé — Taïwan, Singapour, le Portugal et le Québec atterrissent dans leur langue et non en anglais, parce que les catalogues portent le nom d’une langue et jamais celui d’un pays.
...
Ni fenêtre ni erreur, au premier lancement après l’installation : sur une version signée, cela ne devrait pas arriver. Si c’est le cas, la copie est endommagée ou a été modifiée après le téléchargement — vérifiez avec `codesign --verify --deep --strict "/Applications/NMOX Studio.app"` et retéléchargez si la commande échoue. Les journaux vivent sous `~/Library/Application Support/nmoxstudio/…/var/log/` si vous devez ouvrir un ticket.
