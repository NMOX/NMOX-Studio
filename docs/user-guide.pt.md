...
A linha `brew trust` é a confirmação única do Homebrew para qualquer tap de terceiros: nas atualizações ninguém pergunta de novo. O aplicativo é assinado com um Apple Developer ID e notarizado pela Apple, então o Gatekeeper o aceita como está — o cask apenas o copia e não faz mais nada com ele. Instalar à mão pelo DMG funciona do mesmo jeito.
...
No terminal, `nmox .` abre a pasta em que você está, do jeito que `code .` faz: `cd myproject && nmox .`. Uma pasta é apontada exatamente como “Abrir pasta…” da página de boas-vindas a aponta, com manifesto ou sem; um arquivo abre no editor (`nmox src/app.js`), numa linha se você a nomear do jeito que `code -g` faz (`nmox src/app.js:42` — uma coluna é aceita e o editor abre no começo da linha). Um nome que não existe é recusado no terminal (`nmox: typo.js: no such file or folder`) em vez de iniciar qualquer coisa. O `-r` do VS Code é aceito e `-n` abre na única janela; as opções `-a` e `-v` são recusadas pelo nome. `nmox -w file` abre o arquivo e espera até você fechar a aba dele, e `nmox -d left right` compara dois arquivos na visualização de diferenças, o que faz do NMOX Studio o editor e o difftool do git: `git config --global core.editor "nmox -w"`, e `nmox --help` imprime as linhas que o tornam o difftool e o mergetool. Salve a mensagem, feche a aba e o git segue em frente. Com o mergetool definido (`mergetool.nmox.cmd` com `nmox -w "$MERGED"`), o `git mergetool` abre no editor cada arquivo em conflito. Nesse arquivo, e em qualquer arquivo que traga os marcadores de conflito do git, o lado atual de cada conflito ganha uma cor e o lado de entrada outra, e a linha `<<<<<<<` traz um aviso cuja correção rápida (⌘. no Mac, Alt+Enter nos outros, ou a lâmpada na margem) oferece as três escolhas do VS Code: **Aceitar alteração atual**, **Aceitar alteração de entrada** e **Aceitar ambas as alterações**. Cada uma é uma única edição, desfeita por um único ⌘Z; se o bloco mudou depois que o aviso apareceu, nada é substituído e a barra de status diz isso. Um bloco que não tem exatamente a forma do git (um segundo separador, um bloco dentro de outro) não recebe oferta nenhuma, em vez de um palpite. As cores são as entradas “Conflito de mesclagem” em Opções ▸ Fontes e cores ▸ Highlighting. **Equipe ▸ Usar o NMOX Studio com o Git…** mostra essas configurações ao lado dos valores que elas têm agora e as define para você. Uma mensagem de commit abre como o próprio arquivo do git: as linhas `#` são comentários, só o que você escreve passa pelo corretor ortográfico, e uma linha de resumo que passe dos 72 caracteres em que as ferramentas do git cortam recebe um aviso. Uma lista de `git rebase -i` destaca cada comando e cada commit, e Alternar comentário desativa uma linha sem apagá-la. Fora isso, o comando volta na hora — o primeiro `nmox` inicia a IDE em segundo plano, e cada um dos seguintes entrega a sua pasta à IDE que já está rodando. `nmox` sozinho apenas inicia a IDE. Para pôr o `nmox` no seu PATH:
...
**Locais no terminal abrem com ⌘-clique** (Ctrl-clique no Windows e no Linux). Clique num ponto que uma ferramenta imprimiu e o arquivo abre naquela linha: `src/app.ts:42:7` do tsc, `(/abs/app.js:10:5)` num quadro de pilha do Node ou do Jest, `tests/test_x.py:12:` do pytest, `--> src/main.rs:3:5` do rustc, `File "x.py", line 12` de um traceback do Python. Um caminho absoluto abre do jeito que foi impresso. Um relativo é lido a partir da pasta do projeto apontado, onde ⌃\` inicia o shell dele; depois de um `cd` para uma subpasta, o caminho pode nomear um arquivo que não está lá, e a barra de status diz qual caminho procurou, em vez de adivinhar. Uma URL ou um `host:port` nunca é um link.
...
![O rack de tarefas](images/pt/tabs/the-task-rack.png)
...
### Os links da sua documentação

Um README é lido no GitHub, e um link nele que não leva a lugar nenhum é achado pelo próximo leitor, não por você. **Ferramentas ▸ Verificar links do Markdown…** lê cada arquivo Markdown do projeto apontado e verifica cada link e imagem relativos do jeito que o GitHub vai renderizá-los: o arquivo ou a pasta que ele nomeia precisa existir, e um `#heading` precisa ser um título desse arquivo pela regra de âncoras do GitHub (em minúsculas, sem pontuação, espaços viram hifens, um título repetido numerado `-1`, `-2`). Um link que não leva a lugar nenhum é um erro; um título que falta, e um link que sai do projeto, são avisos. Eles chegam como sublinhados no link e como linhas em Tarefas, e a barra de status resume a execução (`Links do Markdown: 12 arquivos, 148 links, 1 não leva a lugar nenhum`).

Links com esquema (`https:`, `mailto:`) não são verificados: nada sai da sua máquina. Nem o que está dentro de um bloco de código ou de um código em linha, já que um link num exemplo é um exemplo, nem o fragmento de um link para um arquivo de código-fonte (`app.js#L10` é a âncora de linha do GitHub). Um link que começa com `/` é lido a partir da raiz do projeto, como o GitHub o lê.
...
**Exibir ▸ Modo de apresentação** aumenta de uma vez todos os editores, a página do navegador embutido, a janela de saída e o terminal — e devolve tudo exatamente como estava ao sair. **Exibir ▸ Mostrar teclas** mostra em tamanho grande o atalho que você acabou de apertar, mas nunca o que você digita. **Editar ▸ Copiar como Markdown** copia a seleção como bloco cercado com a etiqueta de linguagem certa, e a variante **com link** acrescenta o link do GitHub para as mesmas linhas. **Editar ▸ Abrir no GitHub** e **Editar ▸ Copiar link do GitHub** (também pelo clique direito, no editor e na árvore do Estúdio de projeto) dão o mesmo link sem o bloco: a linha do cursor ou as linhas da seleção, um arquivo, ou uma pasta (a página `tree` dela; a pasta raiz do projeto é a página inicial do repositório). Abrir vai para o seu próprio navegador, onde você já está conectado, então blame e os comentários de revisão funcionam. Um arquivo fora de um repositório git, um repositório sem `origin` ou um origin que não é o GitHub é recusado na barra de status sem copiar nada, assim como um editor com alterações não salvas, já que o link mostraria linhas diferentes das que você vê. **Equipe ▸ Novo pull request no GitHub** (também no menu do indicador do git) abre no seu navegador a própria página New Pull Request do GitHub para o ramo em que você está: o passo depois de um push, sem procurar o repositório e o ramo à mão. Um HEAD destacado é recusado, já que não tem ramo nenhum para propor. **Ferramentas ▸ Salvar captura de tela…** pinta a janela inteira no dobro do tamanho, com variantes para a aba do editor sozinha, para a área de transferência e para copiar a árvore do projeto como Markdown.
...
Depois dele, `↑2 ↓1` são os commits que o seu ramo tem e o upstream dele não tem (para enviar) e o contrário (para trazer), mostrados só quando não são zero e só para um ramo com upstream. O menu dele começa com **Trocar de branch…**, **Fazer commit…**, **Fazer pull…** e **Fazer push…**, os próprios diálogos do módulo git.

Ao lado do indicador, uma nota mais discreta responde à pergunta que você mais faz: quem escreveu a linha em que o cursor está? `Ada Lovelace, há 3 dias · Fix the parser` acompanha o cursor, no seu idioma, e a dica dela nomeia o commit e a data. Um clique oferece **Mostrar anotações** (as do arquivo inteiro, o mesmo **Anotar** que o indicador oferece), **Abrir commit no GitHub** (a página do próprio commit, quando o origin é um remoto do GitHub) ou **Copiar ID do commit**; numa linha ainda sem commit, o clique vai direto para as anotações. A nota pergunta ao git uma vez por versão salva do arquivo, nunca na inicialização e nunca para um arquivo fora de um repositório, e não diz nada de um arquivo que o git não acompanha. Uma linha que você alterou mas ainda não commitou mostra **Ainda sem commit**. Enquanto o arquivo tiver alterações não salvas, a nota mostra **Autoria da linha: alterações não salvas** em vez de um nome, porque o git lê o arquivo como ele está salvo e a linha 40 na tela não precisa ser a linha 40 no disco; salve e o nome volta. **Exibir ▸ Autoria da linha** esconde e mostra a nota.
...
**Localizar nos projetos (⇧⌘F) pesquisa o seu código, não o que o repositório ignora.** Num repositório git, a pesquisa pula tudo o que os próprios arquivos `.gitignore` do repositório (o da raiz e os aninhados) e `.git/info/exclude` ignoram, além do próprio `.git` — assim, pesquisar o nome de uma função encontra `src/`, e não as cópias em `node_modules` e `dist/` que o `.gitignore` dos modelos lista. Ela nunca vai além do que o git ignora: uma pasta que o seu repositório rastreia é pesquisada, seja qual for o nome dela. Fora de um repositório, ela também pula as pastas de build e de pacotes que toda varredura do produto pula (`node_modules`, `dist`, `build`, `out`, `target`, `coverage`, `.next`, `.nuxt`, `.svelte-kit`, `.angular`, `.venv`, `__pycache__`). As pastas continuam na árvore do projeto; só a pesquisa passa por elas sem entrar. Para pesquisar nelas mesmo assim, marque **Pesquisar em fontes geradas** na caixa de diálogo Localizar nos projetos (a escolha fica guardada até você desmarcá-la); Substituir nos projetos não oferece essa caixa e nunca reescreve o que o repositório ignora. Seu arquivo global de exclusões do git não é lido, então um caminho que só ele ignora continua sendo pesquisado.
...
Sem janela e sem erro, no primeiro início depois de instalar: num build assinado isso não deveria acontecer. Se acontecer, a cópia está danificada ou foi alterada após o download — verifique com `codesign --verify --deep --strict "/Applications/NMOX Studio.app"` e baixe de novo se falhar. Os registros ficam em `~/Library/Application Support/nmoxstudio/…/var/log/` se você precisar abrir uma questão.
