...
الـ IDE بيحدّث نفسه: **أدوات ◂ إضافات ◂ تحديثات** بتعرض موديولات أي إصدار أحدث. ثبّتوا، وأعيدوا التشغيل لما يطلب منكم، وخلاص — من غير ما تنزّلوا التطبيق كله من الأول. تنبيه بصراحة: بيئة تشغيل Java المدمجة والـ launcher مش بيتغيروا غير مع installer كامل، عشان كده في القفزات الكبيرة للمنصة التثبيت من جديد من ملف الإصدار لسه هو الطريق الصح.

<a id="2-first-launch"></a>
## 2. التشغيل الأول

من التيرمينال، `nmoxstudio --open <folder>` بيشغّل التطبيق والمجلد ده مفتوح كمشروع والراك متوجّه عليه — نفس الباب اللي «فتح مجلد…» بيفتحه في نافذة أهلًا بيك.
...
![التشغيل الأول — نافذة أهلًا بيك وتلات تبويبات](images/tabs/workbench.png)
...
| **⇧⌘E / ⇧⌘L** | تجربة جديدة… / مساحة تعلم جديدة… |
...
![راك المهام](images/tabs/the-task-rack.png)
...
![الراك متوجّه لموقع jQuery كلاسيكي — الإعداد الجاهز منصة الويب الكلاسيكي: MAESTRO، وCRATE، وDYNAMO (مقبض الـ TASK بتاعه قرأ الـ Gruntfile الحقيقي)، وIGNITION بيقدّم الموقع بشكل ثابت، وVITALS حارس الجودة](images/task-rack.png)
...
- **الإعدادات الجاهزة** بتديكم في ضغطة واحدة راك كامل ومتوصّل — بوابة الشحن، وذكاء التطوير، ومسارات المونوريبو، وحلقة E2E، ومنصة LAMP، ومنصة Web3، ومراقبة التشغيل. التوصيلات بتتحفظ تلقائيًا لكل مشروع.

![Tab بيقلب الراك — الكابلات بتوصّل MAESTRO عبر CRATE وDYNAMO وIGNITION لحد VITALS](images/rack-rear.png)
...
![KVASIR بيشرح تشغيل حقيقي فشل: التشخيص اللي اتوافق عليه على الواجهة، والخطوات الكاملة في العارض](images/kvasir-explain.png)
...
![كود jQuery بألوان NMOX Phosphor، والتركيب في نافذة التنقل](images/editor.png)
...
**عرض ◂ وضع العرض التقديمي** بيكبّر مرة واحدة كل محرر، والصفحة في المتصفح المدمج، ونافذة Output، والتيرمينال — وبيرجّع كل حاجة زي ما كانت بالظبط لما تخرجوا منه. **عرض ◂ إظهار ضغطات المفاتيح** بيعرض بحجم كبير تركيبة المفاتيح اللي لسه اتداست، لكن عمره ما بيعرض اللي بتكتبوه. **تحرير ◂ نسخ كـ Markdown** بينسخ التحديد كبلوك كود محاط بعلامة اللغة الصح، والنسخة **نسخ كـ Markdown مع رابط** بتضيف رابط GitHub لنفس السطور. **أدوات ◂ حفظ لقطة شاشة…** بيرسم النافذة كلها بحجم مضاعف، مع نسخ لتبويب المحرر لوحده، وللحافظة، ولشجرة المشروع كـ Markdown.
...
حقل واحد بيوصل لمشاريعكم (الأخيرة والمعروفة)، ولكل جهاز في الراك — ومعاه القفز لعناصر التحكم بتاعته —، ولـ **خوادم شغالة** (Enter بيفتحها في المتصفح)، ولطلبات استوديو الـ API، ولاتصالات وجداول استوديو قواعد البيانات، وللعقود، ولعُقد البنية التحتية، ولكروت لوحة المهام، اللي نتايجها بتقول هي في أنهي عمود.
...
أدوات ◂ خيارات ◂ خريطة المفاتيح بيبدّل البروفايل كله: اختصارات الحركة والقص بتاعة Emacs في كل محرر، أو مجموعات Eclipse وIDEA، لو ذاكرة إيديكم هناك. كل اختصار في NMOX متسجّل في البروفايلات الخمسة كلها، فتبديل البروفايل عمره ما بيكلّفكم اختصارات الاستوديوهات.
