...
من التيرمينال، `nmox .`‎ بيفتح المجلد اللي أنتم واقفين فيه، زي ما `code .`‎ بيعمل: `cd myproject && nmox .`‎. المجلد بيتوجّه عليه بالظبط زي ما «فتح مجلد…» في نافذة أهلًا بيك بيوجّه عليه، فيه manifest أو مفيش؛ والملف بيتفتح في المحرر (`nmox src/app.js`)، وعلى سطر معيّن لو حددتوه بالطريقة اللي `code -g` بيحدده بيها (`nmox src/app.js:42` — العمود كمان مقبول، والمحرر بيفتح في أول السطر). والاسم اللي مش موجود بيترفض في التيرمينال (`nmox: typo.js: no such file or folder`) بدل ما يشغّل أي حاجة. الـ ‎`-r` بتاع VS Code مقبول، و‎`-n` بيفتح في النافذة الوحيدة؛ و‎`-a` و‎`-v` بيترفضوا برسالة بتذكر اسمهم. `nmox -w file` بيفتح الملف ويستنى لحد ما تقفلوا التبويب بتاعه، و`nmox -d left right` بيقارن بين ملفين في عرض الفروق، وده بيخلّي NMOX Studio المحرر والـ difftool بتوع git: `git config --global core.editor "nmox -w"`‎، و`nmox --help` بيطبع السطور اللي بتخلّيه الـ difftool والـ mergetool. احفظوا الرسالة، واقفلوا التبويب، وgit يكمّل. ولما يكون الـ mergetool متظبط (`mergetool.nmox.cmd` على `nmox -w "$MERGED"`)، الأمر `git mergetool` بيفتح في المحرر كل ملف فيه تعارض. في الملف ده، وفي أي ملف فيه علامات التعارض بتاعة git، الجانب الحالي من كل تعارض بيتلوّن بلون والجانب الوارد بلون تاني، والسطر ‎`<<<<<<<`‎ عليه تحذير، والإصلاح السريع بتاعه (⌘. على Mac، وAlt+Enter في غيره، أو اللمبة اللي في الهامش) بيعرض اختيارات VS Code التلاتة: **قبول التغيير الحالي**، و**قبول التغيير الوارد**، و**قبول التغييرين**. كل واحد فيهم تعديل واحد، بيترجع بـ ⌘Z واحدة؛ ولو البلوك اتغيّر بعد ما التحذير ظهر، مفيش حاجة بتتبدّل وشريط الحالة بيقول كده. والبلوك اللي مش على شكل git بالظبط (فاصل تاني، أو بلوك جوه بلوك) مابياخدش أي عرض، بدل ما يتخمّن. الألوان دي هي بنود «تعارض دمج» في الخيارات ◂ الخطوط والألوان ◂ Highlighting. **الفريق ◂ استخدام NMOX Studio مع Git…** بيعرض الإعدادات دي جنب القيم اللي عليها دلوقتي، وبيضبطها لكم. رسالة الـ commit بتتفتح كملف git نفسه: السطور اللي بتبدأ بـ `#` تعليقات، والإملا بيتراجع على اللي أنتم كاتبينه بس، وسطر الملخص اللي يعدّي الـ 72 حرف اللي أدوات git بتقص عندهم بياخد تحذير. ولستة `git rebase -i` بتلوّن كل أمر وكل commit، و«تبديل التعليق» بيوقّف السطر من غير ما يمسحه. غير كده الأمر بيرجع على طول — أول `nmox` بيشغّل الـ IDE في الخلفية، وكل `nmox` بعده بيسلّم المجلد بتاعه للـ IDE اللي شغال بالفعل. `nmox` لوحده بيشغّل الـ IDE وبس. إزاي تحطوا `nmox` على الـ PATH:
...
على Linux وWindows تقدروا كمان تدّوا NMOX Studio مجلد من غير تيرمينال، وبيتوجّه عليه بنفس الطريقة:
...
على macOS، استخدموا `nmox .`‎ أو **ملف ◂ فتح مجلد…**. الـ *فتح باستخدام* (Open With) بتاع Finder وأيقونة الـ Dock لسه مايقدروش يسلّموا مجلد لـ NMOX Studio، فالتطبيق مابيعرضش نفسه هناك.
...
**الأماكن اللي في التيرمينال بتتفتح بـ ⌘-كليك** (Ctrl-كليك على Windows وLinux). اضغطوا على مكان طبعته أداة، والملف يتفتح على السطر ده: من tsc `src/app.ts:42:7`، ومن فريم الـ stack في Node أو Jest ‎`(/abs/app.js:10:5)`‎، ومن pytest `tests/test_x.py:12:`‎، ومن rustc ‎`--> src/main.rs:3:5`، ومن الـ traceback بتاع Python `File "x.py", line 12`. المسار المطلق بيتفتح زي ما اتطبع. والمسار النسبي بيتقري من مجلد المشروع المتوجّه عليه، اللي ⌃\` بيشغّل فيه الـ shell بتاعه؛ وبعد `cd` لمجلد فرعي، المسار ممكن يسمّي ملف مش موجود هناك، وشريط الحالة بيقول هو دوّر على أنهي مسار بدل ما يخمّن. الـ URL أو الـ `host:port` عمره ما بيبقى رابط.
...
**شجرة الملفات** هي استوديو المشاريع (⇧⌘E). دوسوا كليك يمين على ملف أو مجلد، وهتلاقوا جديد، وقص، ونسخ، ولصق، وحذف، وإعادة التسمية، وكمان — زي الـ Explorer بتاع VS Code — **نسخ المسار**، و**نسخ المسار النسبي** (نسبةً للمشروع)، و**عرض في Finder** (**عرض في مستكشف الملفات** على Windows، و**فتح الفولدر اللي فيه** على Linux). وللملف اللي بتعدلوه، **تعديل ◂ نسخ المسار** (⌥⌘C، أو Ctrl+Alt+C على Windows وLinux)، وكمان **تعديل ◂ نسخ المسار النسبي**، بيعملوا نفس الحاجة — حتى من التيرمينال أو الشجرة، للمحرر اللي كنتوا بتكتبوا فيه آخر مرة.
...
- **LSP**: افتحوا ملف خادم اللغة بتاعه متثبّت (typescript، gopls، rust-analyzer، pyright…) وهتاخدوا تشخيصات، ومعلومات لما تقفوا بالماوس، والانتقال للتعريف. أخطاء وتحذيرات الخادم بتبقى كمان صفوف في **بنود العمل** (⌘6)، باسم الخادم (`[lsp:gopls]`)، لكل ملف الخادم بلّغ عنه. بعض الخوادم بتبلّغ بس عن الملفات اللي فاتحينها؛ وgopls بيبلّغ عن الحزمة كلها. ناقصكم خادم؟ الـ IDE بيعرض عليكم أمر التثبيت بدل ما يفشل في صمت.

  ![بنود العمل بتعرض خطأين من gopls، واحد منهم في ملف عمره ما اتفتح، وعدّاد ⁦✕ 2 ⚠ 0⁩ في شريط الحالة](images/lsp-action-items.png)
...
- **ملف ‎`.vscode/settings.json` بتاع المستودع بيتحترم بنفس الطريقة**: `editor.tabSize` و`editor.insertSpaces` و`editor.indentSize` بيحددوا المسافة البادئة، و`files.trimTrailingWhitespace` و`files.insertFinalNewline` (لما يكونوا `true`) بيتطبّقوا وقت الحفظ، وبلوك اللغة زي `"[typescript]": {…}` ليه الأولوية عليهم في لغته. ولو المشروع فيه كمان ‎`.editorconfig`، فالـ ‎`.editorconfig` هو اللي بيكسب في أي حتة الاتنين قالوا فيها حاجة. و`editor.detectIndentation` بتاع VS Code، اللي بيخلّي المسافة البادئة بتاعة الملف نفسه هي اللي تكسب، مالوش مقابل هنا.
...
### الروابط في التوثيق بتاعكم

الـ README بيتقري على GitHub، والرابط اللي فيه ومش بيودّي لأي حتة بيلاقيه القارئ اللي بعدكم، مش أنتم. **أدوات ◂ فحص روابط Markdown…** بيقرا كل ملف Markdown في المشروع المتوجّه عليه وبيفحص كل رابط وكل صورة نسبيين بالطريقة اللي GitHub هيعرضهم بيها: الملف أو المجلد اللي بيسمّيه لازم يكون موجود، و‎`#heading` لازم يكون عنوان في الملف ده حسب قاعدة المراسي بتاعة GitHub (حروف صغيرة، من غير علامات ترقيم، والمسافات بتبقى شرطات، والعنوان المتكرر بيتّرقم ‎`-1`، ‎`-2`). الرابط اللي مش بيودّي لأي حتة غلط؛ والعنوان الناقص، والرابط اللي بيطلع برّه المشروع، تحذيرات. بيوصلوا كخطوط متعرّجة على الرابط وصفوف في المهام، وشريط الحالة بيلخّص الفحص (*روابط Markdown: ⁦12⁩ ملفات، ⁦148⁩ روابط، رابط واحد مش بيودّي لأي حتة*).

الروابط اللي ليها بروتوكول (`https:`، `mailto:`‎) مش بتتفحص: مفيش حاجة بتخرج من جهازكم. ولا أي حاجة جوه code fence أو كود جوه السطر، لأن الرابط اللي في مثال هو مجرد مثال، ولا الـ fragment بتاع رابط لملف مصدر (`app.js#L10` هو مرساة السطر بتاعة GitHub). والرابط اللي بيبدأ بـ ‎`/`‎ بيتقري من جذر المشروع، زي ما GitHub بيقراه.
...
اضغطوا في الهامش، واختاروا **تنقيح الملف (نقاط التوقف)**، والبرنامج بيقف هناك — مع مكدّس الاستدعاءات، والمتغيرات، وتقييم التعبيرات. JavaScript وTypeScript بيشتغلوا على طول عن طريق المحوّل المرفق؛ Python بيستخدم debugpy وGo بيستخدم delve، ودول بتثبّتوهم بنفسكم. **التنقيح في Chrome (نقاط التوقف)** بيعمل نفس الحاجة للصفحة: نقاط التوقف في كود المصدر بتاعكم بتوقف في الـ IDE، والمتصفح شغّال ببروفايل مؤقت للاستخدام مرة واحدة. كل ده بيعدّي الأول على الثقة في مساحة العمل. والمستودع اللي فيه ‎`.vscode/launch.json` ليه باب تاني: اكتبوا اسم إعداد في البحث السريع، وEnter بيشغّل الـ `program` بتاع إعداد Node أو Python في الـ `cwd` بتاعه، مع الـ `args` والـ `env` بتوعه، أو بيفتح الـ `url` بتاع إعداد Chrome مع الـ `webRoot` بتاعه؛ والإعداد اللي بيحدد `envFile` أو `runtimeExecutable` أو أي حاجة تانية مصحح الأخطاء مايقدرش يوصّلها بيترفض بالاسم في شريط الحالة بدل ما يشتغل من غيرها.
...
**عرض ◂ وضع العرض** بيكبّر مرة واحدة كل محرر، والصفحة في المتصفح المدمج، ونافذة Output، والتيرمينال — وبيرجّع كل حاجة زي ما كانت بالظبط لما تخرجوا منه. **عرض ◂ عرض ضغطات المفاتيح** بيعرض بحجم كبير تركيبة المفاتيح اللي لسه اتداست، لكن عمره ما بيعرض اللي بتكتبوه. **تعديل ◂ نسخ كـ Markdown** بينسخ التحديد كبلوك كود محاط بعلامة اللغة الصح، والنسخة **نسخ كـ Markdown مع رابط** بتضيف رابط GitHub لنفس السطور. **تعديل ◂ فتح على GitHub** و**تعديل ◂ نسخ لينك GitHub** (وكمان بالكليك يمين، في المحرر وفي شجرة استوديو المشاريع) بيدّوا نفس الرابط من غير البلوك: سطر المؤشر أو سطور التحديد، أو ملف، أو مجلد (صفحة الـ `tree` بتاعته؛ والمجلد الجذر للمشروع هو الصفحة الرئيسية للمستودع). الفتح بيروح للمتصفح بتاعكم، اللي أنتم عاملين فيه تسجيل دخول، فالـ blame وتعليقات المراجعة بتشتغل. الملف اللي برّه مستودع git، والمستودع اللي مالوش `origin`، والـ origin اللي مش GitHub، بيترفضوا في شريط الحالة ومفيش حاجة بتتنسخ — وكمان المحرر اللي فيه تغييرات مش محفوظة، لأن الرابط هيوري سطور غير اللي أنتم شايفينها. **الفريق ◂ طلب سحب جديد على GitHub** (وموجود كمان في قايمة الشارة) بيفتح في المتصفح بتاعكم صفحة New Pull Request بتاعة GitHub نفسه للفرع اللي أنتم عاملين له checkout: الخطوة اللي بعد الـ push، من غير ما تدوّروا على المستودع والفرع بإيديكم. والـ HEAD المفصول بيترفض، لأن مالوش فرع يقترحه. **أدوات ◂ حفظ لقطة شاشة…** بيرسم النافذة كلها بحجم مضاعف، مع نسخ لتبويب المحرر لوحده، وللحافظة، ولشجرة المشروع كـ Markdown.
...
بعد الشارة، ‎`↑2 ↓1` هي الـ commits اللي في الفرع بتاعكم ومش في الـ upstream بتاعه (محتاجة push)، والعكس (محتاجة pull)، وبتظهر بس لما متبقاش صفر، وبس للفرع اللي ليه upstream. القايمة بتاعتها بتبدأ بـ **تبديل الفرع…**، و**عمل commit…**، و**عمل pull…** و**عمل push…**، ودول الديالوجات بتاعة موديول git نفسه.

جنب الشارة، فيه ملاحظة أهدى بتجاوب على السؤال اللي بتسألوه أكتر حاجة: مين كتب السطر اللي المؤشر واقف عليه؟ النص `Ada Lovelace، من 3 أيام · Fix the parser` بيمشي ورا المؤشر، بلغتكم، والتلميح بتاعه بيقول الـ commit والتاريخ. الضغط عليه بيديكم **عرض تتبع السطور** (للملف كله، نفس **تعليقات السطور** اللي الشارة بتعرضها)، أو **فتح الـ commit على GitHub** (صفحة الـ commit نفسه، لما يكون origin مستودع بعيد على GitHub)، أو **نسخ معرّف الـ commit**؛ وعلى سطر لسه ما اتعملوش commit الضغط بيروح على طول لتتبع السطور. الملاحظة بتسأل git مرة واحدة لكل نسخة محفوظة من الملف، عمرها ما بتسأل وقت التشغيل ولا عن ملف برّه المستودع، ومابتقولش حاجة عن ملف git مش متابعه. والسطر اللي غيّرتوه ولسه ما عملتوش له commit بيظهر عليه **لسه ما اتعملوش commit**. وطول ما الملف فيه تغييرات مش محفوظة، الملاحظة بتعرض **كاتب السطر: فيه تغييرات مش محفوظة** بدل الاسم، لأن git بيقرا الملف زي ما هو محفوظ، والسطر 40 على الشاشة مش لازم يكون السطر 40 على الديسك؛ احفظوا والاسم يرجع. **عرض ◂ كاتب السطر** بيقفل الملاحظة ويفتحها.
...
حقل واحد بيوصل لمشاريعكم (الأخيرة والمعروفة)، ولكل جهاز في الراك — ومعاه القفز لعناصر التحكم بتاعته —، ولـ **خوادم شغالة** (Enter بيفتحها في المتصفح)، ولطلبات استوديو الـ API، ولاتصالات وجداول استوديو قواعد البيانات، وللعقود، ولعُقد البنية التحتية، ولكروت لوحة المهام، اللي نتايجها بتقول هي في أنهي عمود، ولـ **أسامي أوامر VS Code** (*Format Document*، و*Toggle Terminal*، و*Git: Commit*، و*Open Settings* — كل واحد بيظهر تحت *أوامر VS Code* جنب الأمر اللي بيعمل نفس الحاجة هنا، فاسمه هنا هو اللي هتكتبوه المرة الجاية)، ولـ **سكريبتات npm** بتاعة المشروع المتوجّه عليه: اكتبوا `dev` أو `test` والنتيجة بتتقري *تشغيل سكريبت: ⁦dev⁩ — ⁦vite⁩*. Enter بيشغّله بمدير الحزم بتاع المشروع نفسه (npm أو yarn أو pnpm)، بالظبط زي الدبل كليك في مستكشف NPM — الثقة في مساحة العمل بتسأل الأول في مشروع لسه ماوثقتوش فيه، والتشغيل بينضم لزرار ■ في شريط الأدوات، وخادم التطوير اللي بيطبعه بينوّر علامة ⇄. في الـ monorepo السكريبتات هي نفسها اللي مستكشف NPM بيعرضها. والمستودع اللي فيه ‎`.vscode/tasks.json` بيعرض المهام بتاعته بنفس الطريقة — *تشغيل مهمة: ⁦build⁩ — ⁦make all⁩* — وEnter بيشغّل المهمة بعد نفس سؤال الثقة، في نافذة Output وتحت زرار ■ في شريط الأدوات؛ ومهمة الـ shell بتشتغل في الـ shell اللي VS Code كان هيستخدمه (الـ `$SHELL` بتاعكم، كـ login shell على macOS؛ وPowerShell على Windows) أو في اللي الـ `options.shell` بتاعها بيحدده؛ والمهمة اللي محتاجة قيمة VS Code بس اللي يقدر يوفّرها، أو معتمدة على مهمة تانية، بتقول كده في شريط الحالة بدل ما تشتغل. وملف ‎`.vscode/launch.json` بتاع المستودع بيعرض الإعدادات بتاعته جنبها — *تصحيح: ⁦Launch Program⁩ — ⁦${workspaceFolder}/server.js⁩* — وEnter بيشغّل مصحح الأخطاء بنقاط التوقف على الإعداد ده بعد نفس سؤال الثقة.

**بحث في المشاريع (⇧⌘F) بيدوّر في الكود بتاعكم، مش في اللي المستودع بيتجاهله.** في مستودع git بيتخطّى أي حاجة بتتجاهلها ملفات الـ ‎`.gitignore` بتاعة المستودع نفسه (اللي في الجذر وأي ملفات جوّه المجلدات الفرعية) و‎`.git/info/exclude`، وبيتخطّى ‎`.git` نفسه كمان — فالبحث عن اسم دالة بيلاقي `src/`‎، مش النسخ اللي في `node_modules` و`dist/`‎ اللي الـ ‎`.gitignore` بتاع القوالب بيذكرها. وعمره ما بيتخطّى أكتر من اللي git بيتجاهله: المجلد اللي المستودع بتاعكم بيتتبّعه بيتدوّر فيه، مهما كان اسمه. وبره أي مستودع بيتخطّى كمان مجلدات البناء والحزم اللي كل مرور في المنتج بيتخطّاها (`node_modules`، `dist`، `build`، `out`، `target`، `coverage`، `.next`، `.nuxt`، `.svelte-kit`، `.angular`، `.venv`، `__pycache__`). المجلدات بتفضل في شجرة المشروع؛ البحث بس هو اللي بيعدّي عليها. ولو عايزين تدوّروا فيها برضه، علّموا على **بحث في المصادر المتولّدة** في نافذة بحث في المشاريع (الاختيار بيفضل محفوظ لحد ما تشيلوا العلامة)؛ واستبدال في المشاريع مفيهوش الخانة دي، وعمره ما بيعيد كتابة اللي المستودع بيتجاهله. وملف الاستثناءات العام بتاع git عندكم مش بيتقري، فالمسار اللي هو بس بيتجاهله بيتدوّر فيه عادي.

### شريط الحالة بيقول إيه اللي شغال

جنب شارة الخوادم فيه المشروع اللي متوجهين عليه مع سلسلة أدواته، وفرع الـ git مع عدد الملفات اللي اتغيرت. ولما يكون فيه مشكلة في أي حاجة الـ IDE بيفحصها، بيظهر عدّاد **⁦✕ 2 ⚠ 1⁩** بيعرض الأخطاء والتحذيرات من كل خادم لغة ومن كل أداة؛ دوسوا عليه يفتح بنود العمل. كل ده بيتقري من الديسك أو من سجلات المنتج بيحتفظ بيها أصلًا — البصة مبتشغّلش ولا عملية.
