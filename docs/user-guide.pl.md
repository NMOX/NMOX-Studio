...
Z terminala `nmox .` otwiera katalog, w którym stoisz, tak jak robi to `code .`: `cd myproject && nmox .`. Katalog jest celowany dokładnie tak, jak celuje go „Otwórz katalog…” na stronie powitalnej, z manifestem czy bez; plik otwiera się w edytorze (`nmox src/app.js`), a na konkretnym wierszu, jeśli podasz go tak jak w `code -g` (`nmox src/app.js:42` — kolumna jest przyjmowana, a edytor otwiera się na początku wiersza). Nazwa, której nie ma, zostaje odrzucona w terminalu (`nmox: typo.js: no such file or folder`), zamiast cokolwiek uruchamiać. Przełącznik `-r` z VS Code jest przyjmowany, a `-n` otwiera w tym jednym oknie; `-a` i `-v` zostają odrzucone z nazwy. `nmox -w file` otwiera plik i czeka, aż zamkniesz jego kartę, a `nmox -d left right` porównuje dwa pliki w widoku różnic, dzięki czemu NMOX Studio staje się edytorem i difftoolem gita: `git config --global core.editor "nmox -w"`, a `nmox --help` wypisuje wiersze, które czynią go difftoolem i mergetoolem. Zapisz wiadomość, zamknij kartę, a git działa dalej. Gdy mergetool jest ustawiony (`mergetool.nmox.cmd` na `nmox -w "$MERGED"`), `git mergetool` otwiera w edytorze każdy plik z konfliktem. W tym pliku, i w każdym pliku ze znacznikami konfliktu gita, bieżąca strona każdego konfliktu jest zabarwiona jednym kolorem, a przychodząca innym, a wiersz `<<<<<<<` niesie ostrzeżenie, którego szybka poprawka (⌘. na Macu, Alt+Enter gdzie indziej albo żarówka na marginesie) proponuje trzy wybory z VS Code: **Zaakceptuj bieżącą zmianę**, **Zaakceptuj przychodzącą zmianę** i **Zaakceptuj obie zmiany**. Każdy z nich to jedna edycja, którą cofa jedno ⌘Z; jeśli blok zmienił się po pojawieniu się ostrzeżenia, nic nie zostaje zastąpione, a pasek stanu to mówi. Blok, który nie ma dokładnie kształtu gita (drugi separator, blok w bloku), nie dostaje żadnej propozycji zamiast zgadywania. Kolory to pozycje „Konflikt scalania” w oknie Opcje ▸ Czcionki i kolory ▸ Highlighting. **Zespół ▸ Używaj NMOX Studio z Git…** pokazuje te ustawienia obok ich obecnych wartości i ustawia je za ciebie. Wiadomość commita otwiera się jako własny plik gita: wiersze `#` są komentarzami, pisownia jest sprawdzana tylko w tym, co piszesz, a wiersz podsumowania dłuższy niż 72 znaki, przy których narzędzia gita ucinają, dostaje ostrzeżenie. Lista `git rebase -i` wyróżnia każde polecenie i każdy commit, a Przełącz komentarz wyłącza wiersz bez usuwania go. W przeciwnym razie polecenie wraca od razu — pierwsze `nmox` uruchamia IDE w tle, a każde kolejne przekazuje swój katalog już działającemu IDE. Samo `nmox` po prostu uruchamia IDE. Jak dodać `nmox` do PATH:
...
**Miejsca w terminalu otwierają się ⌘-kliknięciem** (Ctrl-kliknięciem w Windows i Linuksie). Kliknij miejsce, które wypisało narzędzie, a plik otworzy się na tym wierszu: `src/app.ts:42:7` z tsc, `(/abs/app.js:10:5)` w ramce stosu Node lub Jest, `tests/test_x.py:12:` z pytest, `--> src/main.rs:3:5` z rustc, `File "x.py", line 12` ze śladu stosu Pythona. Ścieżka bezwzględna otwiera się tak, jak ją wypisano. Względna jest czytana od katalogu celowanego projektu, w którym ⌃\` uruchamia swoją powłokę; po `cd` do podkatalogu ścieżka może wskazywać plik, którego tam nie ma, a pasek stanu mówi, jakiej ścieżki szukał, zamiast zgadywać. URL ani `host:port` nigdy nie jest odnośnikiem.
...
**Drzewo plików** to Studio projektu (⇧⌘E). Kliknij prawym przyciskiem plik lub katalog, a dostaniesz Nowy, Wytnij, Kopiuj, Wklej, Usuń i Zmień nazwę, a — jak w Eksploratorze VS Code — także **Kopiuj ścieżkę**, **Kopiuj ścieżkę względną** (względem projektu) i **Pokaż w Finderze** (w Windows **Pokaż w Eksploratorze plików**, w Linuksie **Otwórz folder nadrzędny**). Dla edytowanego pliku to samo robią **Edycja ▸ Kopiuj ścieżkę** (⌥⌘C, Ctrl+Alt+C w Windows i Linuksie) i **Edycja ▸ Kopiuj ścieżkę względną** — także z terminala czy drzewa, dla edytora, w którym pisałeś ostatnio.
...
### Linki w Twojej dokumentacji

README czyta się na GitHubie, a link, który prowadzi donikąd, znajdzie następny czytelnik, nie ty. **Narzędzia ▸ Sprawdź linki Markdown…** czyta każdy plik Markdown wycelowanego projektu i sprawdza każdy względny link i obraz tak, jak wyrenderuje je GitHub: nazwany plik lub katalog musi istnieć, a `#heading` musi być nagłówkiem tego pliku według reguły kotwic GitHuba (małe litery, bez interpunkcji, spacje jako łączniki, powtórzony nagłówek numerowany `-1`, `-2`). Link prowadzący donikąd to błąd; brakujący nagłówek i link wychodzący poza projekt to ostrzeżenia. Pojawiają się jako falki na linku i wiersze w zadaniach, a pasek stanu podsumowuje przebieg (`Linki Markdown: 12 plików, 148 linków, 1 prowadzi donikąd`).

Linki ze schematem (`https:`, `mailto:`) nie są sprawdzane: nic nie opuszcza twojego komputera. Nie jest też sprawdzane nic wewnątrz bloku kodu ani kodu w wierszu, bo link w przykładzie jest przykładem, ani fragment linku do pliku źródłowego (`app.js#L10` to kotwica wiersza GitHuba). Link zaczynający się od `/` jest czytany od korzenia projektu, tak jak czyta go GitHub.
...
**Widok ▸ Tryb prezentacji** naraz powiększa każdy otwarty edytor, stronę we wbudowanej przeglądarce, okno wyjścia i terminal — i przy wyjściu przywraca wszystko dokładnie tak, jak było. **Widok ▸ Pokazuj naciśnięcia klawiszy** wyświetla wielkim drukiem właśnie naciśnięty skrót, ale nigdy tego, co piszesz. **Edycja ▸ Kopiuj jako Markdown** kopiuje zaznaczenie jako ogrodzony blok z właściwą etykietą języka, a wariant **z odnośnikiem** dokłada odnośnik GitHub do tych samych wierszy. **Edycja ▸ Otwórz w GitHub** i **Edycja ▸ Kopiuj odnośnik GitHub** (także prawym przyciskiem, w edytorze i w drzewie Studia projektu) dają ten sam odnośnik bez bloku: wiersz kursora albo wiersze zaznaczenia, plik lub katalog (jego stronę `tree`; katalog główny projektu to strona główna repozytorium). Otwórz prowadzi do twojej własnej przeglądarki, w której jesteś zalogowany, więc działają blame i komentarze z przeglądu. Plik poza repozytorium git, repozytorium bez `origin` albo origin, który nie jest GitHubem, zostaje odrzucony na pasku stanu i nic nie jest kopiowane — tak samo edytor z niezapisanymi zmianami, bo odnośnik pokazałby inne wiersze niż te, które widzisz. **Zespół ▸ Nowy pull request na GitHubie** (także w menu plakietki gita) otwiera w twojej przeglądarce własną stronę GitHuba New Pull Request dla gałęzi, na której jesteś: krok po pushu, bez ręcznego szukania repozytorium i gałęzi. Odłączony HEAD zostaje odrzucony, bo nie ma gałęzi, którą mógłby zaproponować. **Narzędzia ▸ Zapisz zrzut ekranu…** maluje całe okno w podwójnym rozmiarze, a warianty obejmują samą kartę edytora, schowek i drzewo projektu jako Markdown.
...
Za nią `↑2 ↓1` to commity, które ma twoja gałąź, a nie ma jej upstream (do wypchnięcia), i odwrotnie (do pobrania); pokazują się tylko wtedy, gdy nie są zerowe, i tylko dla gałęzi z upstreamem. Jej menu zaczyna się od **Przełącz gałąź…**, **Zatwierdź…**, **Pobierz (pull)…** i **Wypchnij (push)…**, własnych okien modułu git.

Obok plakietki cichsza notka odpowiada na pytanie, które zadajesz najczęściej: kto napisał wiersz, na którym stoi kursor? `Ada Lovelace, 3 dni temu · Fix the parser` podąża za kursorem, w twoim języku, a jej podpowiedź podaje commit i datę. Kliknięcie daje do wyboru **Pokaż adnotacje** (adnotacje całego pliku, to samo **Adnotuj**, które oferuje plakietka), **Otwórz commit w GitHub** (stronę samego commita, gdy origin jest zdalnym repozytorium na GitHubie) albo **Kopiuj identyfikator commita**; na wierszu jeszcze niezatwierdzonym kliknięcie od razu otwiera adnotacje. Notka pyta gita raz na każdą zapisaną wersję pliku, nigdy przy starcie i nigdy o plik spoza repozytorium, a o pliku, którego git nie śledzi, nie mówi nic. Wiersz zmieniony, ale jeszcze niezatwierdzony, brzmi **Jeszcze niezatwierdzone**. Dopóki plik ma niezapisane zmiany, notka zamiast nazwiska pokazuje **Autor wiersza: niezapisane zmiany**, bo git czyta plik taki, jaki jest zapisany, a wiersz 40 na ekranie nie musi być wierszem 40 na dysku; zapisz, a nazwisko wróci. **Widok ▸ Autor wiersza** wyłącza i włącza notkę.
...
**Znajdź w projektach (⇧⌘F) przeszukuje twój kod, a nie to, co ignoruje repozytorium.** W repozytorium git wyszukiwanie pomija wszystko, co ignorują własne pliki `.gitignore` repozytorium (ten w katalogu głównym i wszystkie zagnieżdżone) oraz `.git/info/exclude`, a także sam `.git` — dlatego szukanie nazwy funkcji znajduje `src/`, a nie kopie w `node_modules` i `dist/`, które wymienia `.gitignore` szablonów. Nigdy nie wychodzi poza to, co ignoruje git: folder śledzony przez twoje repozytorium jest przeszukiwany, jakkolwiek się nazywa. Poza repozytorium pomija też foldery kompilacji i pakietów, które pomija każde przejście po plikach w produkcie (`node_modules`, `dist`, `build`, `out`, `target`, `coverage`, `.next`, `.nuxt`, `.svelte-kit`, `.angular`, `.venv`, `__pycache__`). Foldery zostają w drzewie projektu; omija je tylko wyszukiwanie. Aby mimo to w nich szukać, zaznacz **Szukaj w źródłach generowanych** w oknie „Znajdź w projektach” (wybór jest zapamiętywany, dopóki go nie odznaczysz); „Zamień w projektach” nie ma takiego pola i nigdy nie zmienia tego, co ignoruje repozytorium. Globalny plik wykluczeń git nie jest czytany, więc ścieżka ignorowana tylko przez niego jest przeszukiwana.
