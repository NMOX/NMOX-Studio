...
Obok plakietki cichsza notka odpowiada na pytanie, które zadajesz najczęściej: kto napisał wiersz, na którym stoi kursor? `Ada Lovelace, 3 dni temu · Fix the parser` podąża za kursorem, w twoim języku, a jej podpowiedź podaje commit i datę. Kliknięcie otwiera adnotacje całego pliku — to samo **Adnotuj**, które oferuje plakietka. Notka pyta gita raz na każdą zapisaną wersję pliku, nigdy przy starcie i nigdy o plik spoza repozytorium, a o pliku, którego git nie śledzi, nie mówi nic. Wiersz zmieniony, ale jeszcze niezatwierdzony, brzmi **Jeszcze niezatwierdzone**. Dopóki plik ma niezapisane zmiany, notka zamiast nazwiska pokazuje **Autor wiersza: niezapisane zmiany**, bo git czyta plik taki, jaki jest zapisany, a wiersz 40 na ekranie nie musi być wierszem 40 na dysku; zapisz, a nazwisko wróci. **Widok ▸ Autor wiersza** wyłącza i włącza notkę.
