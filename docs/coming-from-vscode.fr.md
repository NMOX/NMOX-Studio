...
| **Problems panel** | **Éléments à traiter** (⌘6) : les erreurs et avertissements des serveurs de langage, et les constats de lint et de typage des appareils PURITY et TYPEGUARD du rack. Comme dans VS Code, certains serveurs ne signalent que les fichiers que vous avez ouverts ; gopls signale tout le paquet. |
...
- **Sous Linux, Ctrl+\` ouvre le Terminal, pas un sélecteur de fenêtres.** La plateforme y gardait un second sélecteur pour les bureaux (KDE) qui s’emparent de Ctrl+Tab ; le sélecteur est sur Ctrl+Tab.
