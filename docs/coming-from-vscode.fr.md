...
| **`settings.json`** | Outils ▸ Options (sous macOS, NMOX Studio ▸ Settings…). Le `.vscode/settings.json` d’un dépôt règle l’indentation de ses fichiers (`editor.tabSize`, `editor.insertSpaces`) et, à l’enregistrement, `files.trimTrailingWhitespace` et `files.insertFinalNewline` ; son `.editorconfig` l’emporte partout où les deux s’expriment, pendant la frappe et à l’enregistrement. |
