...
Dari terminal, `nmox .` membuka folder tempat Anda berada, seperti `code .`: `cd myproject && nmox .`. Sebuah folder diarahkan persis seperti Buka Folder… di halaman Selamat Datang mengarahkannya, dengan atau tanpa manifes; sebuah berkas terbuka di penyunting (`nmox src/app.js`), pada sebuah baris bila Anda menyebutkannya seperti pada `code -g` (`nmox src/app.js:42` — kolom juga diterima, dan penyunting terbuka di awal baris). Nama yang tidak ada ditolak di terminal (`nmox: typo.js: no such file or folder`) alih-alih menjalankan apa pun. Opsi `-r` milik VS Code diterima dan `-n` membuka di satu-satunya jendela; `-a` dan `-v` ditolak dengan menyebut namanya. `nmox -w file` membuka berkas itu dan menunggu sampai Anda menutup tabnya, dan `nmox -d left right` membandingkan dua berkas di tampilan diff, sehingga NMOX Studio menjadi penyunting dan difftool untuk git: `git config --global core.editor "nmox -w"`, dan `nmox --help` mencetak baris-baris yang menjadikannya difftool dan mergetool. Simpan pesannya, tutup tabnya, dan git melanjutkan. Dengan mergetool terpasang (`mergetool.nmox.cmd` berisi `nmox -w "$MERGED"`), `git mergetool` membuka setiap berkas yang berkonflik di penyunting. Di berkas itu, dan di berkas apa pun yang membawa penanda konflik git, sisi saat ini dari tiap konflik diwarnai dengan satu warna dan sisi yang masuk dengan warna lain, dan baris `<<<<<<<` membawa peringatan yang perbaikan cepatnya (⌘. di Mac, Alt+Enter di tempat lain, atau bola lampu di margin) menawarkan tiga pilihan VS Code: **Terima perubahan saat ini**, **Terima perubahan masuk**, dan **Terima kedua perubahan**. Masing-masing adalah satu suntingan, yang dibatalkan dengan satu ⌘Z; jika bloknya berubah setelah peringatan muncul, tidak ada yang diganti dan baris status mengatakannya. Blok yang bentuknya tidak persis bentuk git (pemisah kedua, blok di dalam blok) tidak mendapat tawaran apa pun, alih-alih tebakan. Warnanya adalah entri “Konflik penggabungan” di Opsi ▸ Font dan warna ▸ Highlighting. **Tim ▸ Gunakan NMOX Studio dengan Git…** menampilkan pengaturan itu di samping nilainya saat ini, lalu menerapkannya untuk Anda. Pesan commit terbuka sebagai berkas milik git sendiri: baris `#` adalah komentar, hanya yang Anda tulis yang diperiksa ejaannya, dan baris ringkasan yang melewati 72 karakter tempat alat-alat git memotong mendapat peringatan. Daftar `git rebase -i` menyorot setiap perintah dan setiap commit, dan Alihkan komentar menonaktifkan sebuah baris tanpa menghapusnya. Selain itu, perintah itu langsung kembali — `nmox` yang pertama menjalankan IDE di latar belakang, dan setiap `nmox` berikutnya menyerahkan foldernya kepada IDE yang sudah berjalan. `nmox` saja hanya menjalankan IDE. Cara memasukkan `nmox` ke PATH Anda:
...
**Lokasi di terminal terbuka dengan ⌘-klik** (Ctrl-klik di Windows dan Linux). Klik tempat yang dicetak sebuah alat, dan berkasnya terbuka di baris itu: `src/app.ts:42:7` dari tsc, `(/abs/app.js:10:5)` di bingkai tumpukan Node atau Jest, `tests/test_x.py:12:` dari pytest, `--> src/main.rs:3:5` dari rustc, `File "x.py", line 12` dari traceback Python. Jalur absolut terbuka sebagaimana dicetak. Jalur relatif dibaca dari folder proyek yang diarahkan, tempat ⌃\` memulai shell-nya; setelah `cd` ke subfolder, jalurnya bisa menyebut berkas yang tidak ada di sana, dan baris status menyebut jalur mana yang dicarinya alih-alih menebak. URL atau `host:port` tidak pernah menjadi tautan.
...
**Pohon berkas** adalah Studio Proyek (⇧⌘E). Klik kanan sebuah berkas atau folder untuk Baru, Potong, Salin, Tempel, Hapus, dan Ganti Nama, dan — seperti di Explorer VS Code — **Salin Jalur**, **Salin Jalur Relatif** (relatif terhadap proyek), dan **Tampilkan di Finder** (**Tampilkan di File Explorer** di Windows, **Buka Folder Induk** di Linux). Untuk berkas yang sedang Anda sunting, **Edit ▸ Salin Jalur** (⌥⌘C, Ctrl+Alt+C di Windows dan Linux) dan **Edit ▸ Salin Jalur Relatif** melakukan hal yang sama — juga dari terminal atau pohon, untuk penyunting tempat Anda terakhir mengetik.
...
### Tautan dalam dokumentasi Anda

README dibaca di GitHub, dan tautan di dalamnya yang tidak mengarah ke mana pun ditemukan oleh pembaca berikutnya, bukan oleh Anda. **Alat ▸ Periksa Tautan Markdown…** membaca setiap berkas Markdown di proyek yang diarahkan dan memeriksa setiap tautan dan gambar relatif sebagaimana GitHub akan menampilkannya: berkas atau folder yang disebutnya harus ada, dan sebuah `#heading` harus merupakan judul di berkas itu menurut aturan jangkar GitHub (huruf kecil, tanda baca dibuang, spasi menjadi tanda hubung, judul yang berulang diberi nomor `-1`, `-2`). Tautan yang tidak mengarah ke mana pun adalah galat; judul yang tidak ada, dan tautan yang keluar dari proyek, adalah peringatan. Semuanya muncul sebagai garis berlekuk pada tautan dan baris di Tugas, dan baris status merangkum hasilnya (`Tautan Markdown: 12 berkas, 148 tautan, 1 tidak mengarah ke mana pun`).

Tautan dengan skema (`https:`, `mailto:`) tidak diperiksa: tidak ada yang keluar dari mesin Anda. Begitu pula apa pun di dalam blok kode atau kode sebaris, karena tautan dalam contoh hanyalah contoh, dan fragmen tautan ke berkas sumber (`app.js#L10` adalah jangkar baris GitHub). Tautan yang diawali `/` dibaca dari akar proyek, seperti GitHub membacanya.
...
**Tampilan ▸ Mode presentasi** sekaligus memperbesar setiap penyunting yang terbuka, halaman di peramban bawaan, jendela keluaran, dan terminal — lalu mengembalikan semuanya persis seperti semula saat Anda keluar. **Tampilan ▸ Tampilkan ketukan tombol** menampilkan besar-besar kombinasi yang baru Anda tekan, tetapi tidak pernah apa yang Anda ketik. **Edit ▸ Salin sebagai Markdown** menyalin pilihan sebagai blok berpagar dengan label bahasa yang tepat, dan variannya **dengan tautan** menambahkan tautan GitHub ke baris yang sama. **Edit ▸ Buka di GitHub** dan **Edit ▸ Salin tautan GitHub** (juga lewat klik kanan, di penyunting dan di pohon Studio Proyek) memberi tautan yang sama tanpa bloknya: baris kursor atau baris-baris pilihan, sebuah berkas, atau sebuah folder (halaman `tree`-nya; folder akar proyek adalah halaman depan repositori). Buka membawa Anda ke peramban Anda sendiri, tempat Anda sudah masuk, sehingga blame dan komentar tinjauan berfungsi. Berkas di luar repositori git, repositori tanpa `origin`, atau origin yang bukan GitHub ditolak di baris status dan tidak ada yang disalin — begitu pula penyunting yang punya perubahan belum disimpan, karena tautannya akan menampilkan baris yang berbeda dari yang Anda lihat. **Tim ▸ Pull Request Baru di GitHub** (juga ada di menu tanda git) membuka halaman New Pull Request milik GitHub sendiri untuk cabang yang sedang Anda pakai, di peramban Anda: langkah setelah push, tanpa mencari repositori dan cabangnya sendiri. HEAD yang terlepas ditolak, karena ia tidak punya cabang untuk diusulkan. **Alat ▸ Simpan Tangkapan Layar…** melukis seluruh jendela pada ukuran ganda, dengan varian untuk tab penyunting saja, untuk papan klip, dan untuk menyalin pohon proyek sebagai Markdown.
...
Sesudahnya, `↑2 ↓1` adalah commit yang dimiliki cabang Anda tetapi tidak dimiliki upstream-nya (untuk di-push) dan sebaliknya (untuk di-pull), hanya ditampilkan bila bukan nol dan hanya untuk cabang yang punya upstream. Menunya diawali dengan **Ganti Branch…**, **Commit…**, **Pull…** dan **Push…**, dialog milik modul git sendiri.

Di samping tanda itu, sebuah catatan yang lebih tenang menjawab pertanyaan yang paling sering Anda ajukan: siapa yang menulis baris tempat kursor berada? `Ada Lovelace, 3 hari yang lalu · Fix the parser` mengikuti kursor, dalam bahasa Anda, dan tooltip-nya menyebut commit dan tanggalnya. Sekali klik menawarkan **Tampilkan Anotasi** (anotasi seluruh berkas, **Anotasi** yang sama yang ditawarkan tanda itu), **Buka Commit di GitHub** (halaman commit itu sendiri, bila origin adalah remote GitHub), atau **Salin ID Commit**; pada baris yang belum di-commit, klik langsung membuka anotasi. Catatan ini bertanya kepada git sekali untuk tiap versi berkas yang tersimpan, tidak pernah saat mulai dan tidak pernah untuk berkas di luar repositori, dan diam untuk berkas yang tidak dilacak git. Baris yang Anda ubah tetapi belum di-commit berbunyi **Belum di-commit**. Selama berkas punya perubahan yang belum disimpan, catatannya berbunyi **Penulis baris: ada perubahan yang belum disimpan** alih-alih sebuah nama, karena git membaca berkas sebagaimana tersimpan, dan baris 40 di layar belum tentu baris 40 di cakram; simpan, dan namanya kembali. **Tampilan ▸ Penulis baris** mematikan dan menyalakan catatan itu.
...
**Cari di proyek (⇧⌘F) mencari di kode Anda, bukan di apa yang diabaikan repositori.** Di repositori git, pencarian melewati semua yang diabaikan oleh berkas `.gitignore` milik repositori itu sendiri (yang di akar dan yang bersarang di subfolder) dan `.git/info/exclude`, juga `.git` itu sendiri — jadi mencari nama sebuah fungsi menemukan `src/`, bukan salinan di `node_modules` dan `dist/` yang dicantumkan `.gitignore` templat. Pencarian tidak pernah melampaui apa yang diabaikan git: folder yang dilacak repositori Anda tetap dicari, apa pun namanya. Di luar repositori, pencarian juga melewati folder build dan paket yang dilewati setiap penelusuran di produk ini (`node_modules`, `dist`, `build`, `out`, `target`, `coverage`, `.next`, `.nuxt`, `.svelte-kit`, `.angular`, `.venv`, `__pycache__`). Folder-folder itu tetap ada di pohon proyek; hanya pencarian yang melewatinya. Untuk tetap mencari di sana, centang **Cari di sumber hasil generate** di dialog Cari di proyek (pilihan itu diingat sampai Anda menghapus centangnya); Ganti di proyek tidak menawarkan kotak itu dan tidak pernah menulis ulang apa yang diabaikan repositori. Berkas pengecualian git global Anda tidak dibaca, jadi jalur yang hanya diabaikan olehnya tetap dicari.
