...
Desde una terminal, `nmox .` abre la carpeta en la que estás, como hace `code .`: `cd myproject && nmox .`. Una carpeta queda apuntada exactamente como la apunta «Abrir carpeta…» en la página de bienvenida, tenga manifiesto o no; un archivo se abre en el editor (`nmox src/app.js`), en una línea si la nombras como lo hace `code -g` (`nmox src/app.js:42`: se admite una columna y el editor se abre al principio de la línea). Un nombre que no existe se rechaza en la terminal (`nmox: typo.js: no such file or folder`) en lugar de arrancar nada. Se admite el `-r` de VS Code y `-n` abre en la única ventana; `-a` y `-v` se rechazan nombrándolas. `nmox -w file` abre el archivo y espera hasta que cierres su pestaña, y `nmox -d left right` compara dos archivos en la vista de diferencias, con lo que NMOX Studio pasa a ser el editor y el difftool de git: `git config --global core.editor "nmox -w"`, y `nmox --help` imprime las líneas que lo convierten en el difftool y el mergetool. Guarda el mensaje, cierra la pestaña y git sigue su camino. Con el mergetool configurado (`mergetool.nmox.cmd` con `nmox -w "$MERGED"`), `git mergetool` abre en el editor cada archivo con conflictos. En ese archivo, y en cualquier archivo que lleve los marcadores de conflicto de git, el lado actual de cada conflicto se tiñe de un color y el entrante de otro, y la línea `<<<<<<<` lleva una advertencia cuya corrección rápida (⌘. en un Mac, Alt+Enter en los demás, o la bombilla del margen) ofrece las tres opciones de VS Code: **Aceptar cambio actual**, **Aceptar cambio entrante** y **Aceptar ambos cambios**. Cada una es una sola edición, que un solo ⌘Z deshace; si el bloque cambió después de aparecer la advertencia, no se reemplaza nada y la barra de estado lo dice. Un bloque que no tiene exactamente la forma de git (un segundo separador, un bloque dentro de otro) no recibe ninguna oferta, en lugar de una suposición. Los tintes son las entradas «Conflicto de fusión» de Opciones ▸ Fuentes y colores ▸ Highlighting. **Equipo ▸ Usar NMOX Studio con Git…** muestra esos ajustes junto a los valores que tienen ahora y los configura por ti. Un mensaje de commit se abre como el archivo propio de git: las líneas `#` son comentarios, solo se revisa la ortografía de lo que escribes, y una línea de resumen que pase de los 72 caracteres en los que cortan las herramientas de git recibe una advertencia. Una lista de `git rebase -i` resalta cada comando y cada commit, y Alternar comentario descarta una línea sin borrarla. De lo contrario, el comando vuelve al instante: el primer `nmox` arranca el IDE en segundo plano, y cada uno de los siguientes le pasa su carpeta al IDE que ya está en marcha. `nmox` a secas solo arranca el IDE. Para tener `nmox` en tu PATH:
...
En Linux y Windows también puedes pasarle una carpeta a NMOX Studio sin terminal, y queda apuntada de la misma manera:
...
En macOS, usa `nmox .` o **Archivo ▸ Abrir carpeta…**. El *Abrir con* del Finder y el icono del Dock todavía no pueden pasarle una carpeta a NMOX Studio, así que la app no se ofrece ahí.
...
**Las ubicaciones de la terminal se abren con ⌘-clic** (Ctrl-clic en Windows y Linux). Haz clic en un lugar que imprimió una herramienta y el archivo se abre en esa línea: `src/app.ts:42:7` de tsc, `(/abs/app.js:10:5)` en un marco de pila de Node o de Jest, `tests/test_x.py:12:` de pytest, `--> src/main.rs:3:5` de rustc, `File "x.py", line 12` de un traceback de Python. Una ruta absoluta se abre tal como se imprimió. Una relativa se lee desde la carpeta del proyecto apuntado, donde ⌃\` arranca su shell; después de un `cd` a una subcarpeta, la ruta puede nombrar un archivo que no está allí, y la barra de estado dice qué ruta buscó en lugar de adivinar. Una URL o un `host:port` nunca es un enlace.
...
**El árbol de archivos** es el Estudio de proyecto (⇧⌘E). Haz clic derecho en un archivo o una carpeta para Nuevo, Cortar, Copiar, Pegar, Eliminar y Renombrar y, como en el Explorer de VS Code, **Copiar ruta**, **Copiar ruta relativa** (relativa al proyecto) y **Mostrar en Finder** (**Mostrar en el Explorador de archivos** en Windows, **Abrir la carpeta contenedora** en Linux). Para el archivo que estás editando, **Edición ▸ Copiar ruta** (⌥⌘C, Ctrl+Alt+C en Windows y Linux) y **Edición ▸ Copiar ruta relativa** hacen lo mismo, también desde la terminal o el árbol, para el editor en el que escribiste por última vez.
...
- **LSP** — abre un archivo cuyo servidor de lenguaje esté instalado (typescript, gopls, rust-analyzer, pyright, …) y tendrás diagnósticos, información al pasar el ratón e ir a la definición. Los errores y avisos del servidor también son filas en **Elementos de acción** (⌘6), con el nombre del servidor (`[lsp:gopls]`), para cada archivo del que el servidor haya informado. Algunos servidores solo informan de los archivos que tienes abiertos; gopls informa de todo el paquete. ¿Falta un servidor? El IDE te ofrece la orden de instalación en vez de fallar en silencio.

  ![Elementos de acción con dos errores de gopls, uno en un archivo que nunca se abrió, y el recuento ✕ 2 ⚠ 0 en la barra de estado](images/lsp-action-items.png)
...
- **El `.vscode/settings.json` de un repositorio se respeta del mismo modo**: `editor.tabSize`, `editor.insertSpaces` y `editor.indentSize` deciden la sangría, `files.trimTrailingWhitespace` y `files.insertFinalNewline` (cuando valen `true`) se aplican al guardar, y un bloque de lenguaje como `"[typescript]": {…}` los sustituye para su lenguaje. Cuando el proyecto tiene además un `.editorconfig`, el `.editorconfig` gana dondequiera que ambos digan algo. El `editor.detectIndentation` de VS Code, que deja ganar a la sangría propia del archivo, no tiene equivalente aquí.
...
### Los enlaces de tu documentación

Un README se lee en GitHub, y un enlace suyo que no lleva a ninguna parte lo descubre el siguiente lector, no tú. **Herramientas ▸ Comprobar enlaces de Markdown…** lee cada archivo Markdown del proyecto apuntado y comprueba cada enlace e imagen relativos tal como GitHub los mostrará: el archivo o la carpeta que nombra tiene que existir, y un `#heading` tiene que ser un encabezado de ese archivo según la regla de anclas de GitHub (en minúsculas, sin puntuación, los espacios como guiones y un encabezado repetido numerado `-1`, `-2`). Un enlace que no lleva a ninguna parte es un error; un encabezado que falta, y un enlace que sale del proyecto, son advertencias. Llegan como subrayados en el enlace y como filas en Tareas, y la barra de estado resume la pasada (`Enlaces de Markdown: 12 archivos, 148 enlaces, 1 no lleva a ninguna parte`).

Los enlaces con esquema (`https:`, `mailto:`) no se comprueban: nada sale de tu máquina. Tampoco lo que hay dentro de un bloque de código o de un código en línea, porque un enlace en un ejemplo es un ejemplo, ni el fragmento de un enlace a un archivo de código (`app.js#L10` es el ancla de línea de GitHub). Un enlace que empieza por `/` se lee desde la raíz del proyecto, como lo lee GitHub.
...
Haz clic en el margen izquierdo, elige **Depurar archivo (puntos de interrupción)** y el programa se detiene ahí — con la pila, las variables y la evaluación de expresiones. JavaScript y TypeScript funcionan de fábrica gracias al adaptador incluido; Python usa debugpy y Go usa delve, que instalas tú. **Depurar en Chrome** hace lo mismo con una página: los puntos de interrupción de tu código fuente se detienen dentro del IDE mientras el navegador corre en un perfil desechable. Todo pasa antes por la confirmación de confianza del espacio de trabajo. Un repositorio que trae `.vscode/launch.json` tiene una tercera puerta: escribe el nombre de una configuración en la Búsqueda rápida y Entrar arranca el `program` de Node o Python de esa configuración en su `cwd`, con sus `args` y su `env`, o abre la `url` de una configuración de Chrome con su `webRoot`; una configuración que fija `envFile`, `runtimeExecutable` o cualquier otra cosa que el depurador no puede transmitir se rechaza por su nombre en la barra de estado en lugar de arrancarse sin ello.
...
**Ver ▸ Modo presentación** agranda de una vez todos los editores, la página del navegador integrado, la ventana de salida y la terminal — y lo devuelve todo exactamente como estaba al salir. **Ver ▸ Mostrar pulsaciones de teclas** muestra en grande el atajo que acabas de pulsar, pero nunca lo que escribes. **Edición ▸ Copiar como Markdown** copia la selección como un bloque delimitado con la etiqueta de lenguaje correcta, y su variante **con enlace** añade el enlace de GitHub a esas mismas líneas. **Edición ▸ Abrir en GitHub** y **Edición ▸ Copiar enlace de GitHub** (también con clic derecho, en el editor y en el árbol del Estudio de proyecto) dan el mismo enlace sin el bloque: la línea del cursor o las líneas de la selección, un archivo, o una carpeta (su página `tree`; la carpeta raíz del proyecto es la portada del repositorio). Abrir va a tu propio navegador, donde tienes la sesión iniciada, así que funcionan blame y los comentarios de revisión. Un archivo fuera de un repositorio git, un repositorio sin `origin` o un origin que no es GitHub se rechaza en la barra de estado sin copiar nada, y también un editor con cambios sin guardar, porque el enlace mostraría líneas distintas de las que ves. **Equipo ▸ Nueva pull request en GitHub** (también en el menú del indicador de git) abre en tu navegador la propia página New Pull Request de GitHub para la rama que tienes activa: el paso que sigue a un push, sin buscar a mano el repositorio y la rama. Un HEAD separado se rechaza, porque no tiene ninguna rama que proponer. **Herramientas ▸ Guardar captura de pantalla…** pinta la ventana entera al doble de tamaño, y hay variantes para la pestaña del editor sola, para el portapapeles y para copiar el árbol del proyecto como Markdown.
...
Tras él, `↑2 ↓1` son los commits que tiene tu rama y no su upstream (para enviar) y al revés (para traer), y solo aparecen cuando no son cero y en una rama con upstream. Su menú empieza con **Cambiar de rama…**, **Confirmar…**, **Traer (pull)…** y **Enviar (push)…**, los propios diálogos del módulo git.

Junto al indicador, una nota más discreta responde a la pregunta que más te haces: ¿quién escribió la línea en la que está el cursor? `Ada Lovelace, hace 3 días · Fix the parser` sigue al cursor, en tu idioma, y su descripción emergente nombra el commit y la fecha. Un clic ofrece **Mostrar anotaciones** (las del archivo entero: el mismo **Anotar** que ofrece el indicador), **Abrir commit en GitHub** (la página del propio commit, cuando origin es un remoto de GitHub) o **Copiar ID del commit**; en una línea que todavía no has confirmado, el clic va directo a las anotaciones. La nota pregunta a git una vez por cada versión guardada del archivo, nunca al arrancar y nunca por un archivo fuera de un repositorio, y no dice nada de un archivo que git no sigue. Una línea que cambiaste y todavía no has confirmado dice **Todavía sin confirmar**. Mientras el archivo tenga cambios sin guardar, la nota dice **Autoría de la línea: cambios sin guardar** en lugar de un nombre, porque git lee el archivo tal como está guardado y la línea 40 de la pantalla no tiene por qué ser la línea 40 del disco; guarda y el nombre vuelve. **Ver ▸ Autoría de la línea** oculta la nota y la vuelve a mostrar.
...
Una sola caja alcanza tus proyectos (los recientes y los conocidos), todos los dispositivos del rack — saltando directamente a sus controles —, los **servidores en marcha** (Entrar lo abre en el navegador), las peticiones del Estudio de API, las conexiones y tablas del Estudio de bases de datos, los contratos, los nodos de infraestructura, las tarjetas del Tablero de tareas (el resultado nombra la columna en que están), **los nombres de los comandos de VS Code** (*Format Document*, *Toggle Terminal*, *Git: Commit*, *Open Settings* — cada uno aparece bajo *Comandos de VS Code* junto a la acción que hace lo mismo aquí, así que su nombre aquí es el que escribirás la próxima vez) y los **scripts de npm** del proyecto apuntado: escribe `dev` o `test` y el resultado dice *Ejecutar script: dev — vite*; Entrar lo ejecuta con el propio gestor de paquetes del proyecto (npm, yarn o pnpm), exactamente como lo haría un doble clic en el Explorador de NPM — la confianza del espacio de trabajo pregunta antes en un proyecto en el que aún no confías, la ejecución se suma al ■ de la barra de herramientas, y un servidor de desarrollo que imprima su dirección enciende el indicador ⇄. En un monorepo, los scripts son los que muestra el Explorador de NPM. Un repositorio que trae `.vscode/tasks.json` enumera sus tareas del mismo modo — *Ejecutar tarea: build — make all* — y Entrar ejecuta la tarea tras la misma pregunta de confianza, en la ventana Output y bajo el ■ de la barra de herramientas; una tarea de consola se ejecuta en la consola que usaría VS Code (tu `$SHELL`, como consola de inicio de sesión en macOS; PowerShell en Windows) o en la que nombre su `options.shell`; una tarea que necesita un valor que solo VS Code puede proporcionar, o que depende de otra tarea, lo dice en la barra de estado en lugar de ejecutarse. El `.vscode/launch.json` del repositorio enumera sus configuraciones al lado — *Depurar: Launch Program — ${workspaceFolder}/server.js* — y Entrar arranca el depurador de puntos de interrupción sobre esa configuración tras la misma pregunta de confianza.

**Buscar en los proyectos (⇧⌘F) busca en tu código, no en lo que el repositorio ignora.** En un repositorio git se salta todo lo que ignoran los propios archivos `.gitignore` del repositorio (el de la raíz y los que haya en subcarpetas) y `.git/info/exclude`, además del propio `.git`; así, buscar el nombre de una función encuentra `src/`, no las copias de `node_modules` y `dist/` que el `.gitignore` de las plantillas incluye. Nunca va más allá de lo que git ignora: una carpeta que tu repositorio versiona se busca, se llame como se llame. Fuera de un repositorio se salta también las carpetas de compilación y de paquetes que se salta cualquier recorrido del producto (`node_modules`, `dist`, `build`, `out`, `target`, `coverage`, `.next`, `.nuxt`, `.svelte-kit`, `.angular`, `.venv`, `__pycache__`). Las carpetas siguen en el árbol del proyecto; solo la búsqueda pasa de largo. Para buscar en ellas de todos modos, marca **Buscar en fuentes generadas** en el diálogo Buscar en los proyectos (se recuerda hasta que la desmarques); Reemplazar en los proyectos no ofrece esa casilla y nunca reescribe lo que el repositorio ignora. Tu archivo global de exclusiones de git no se lee, así que una ruta que solo él ignora sí se busca.

### La barra de estado dice qué está vivo

Junto al indicador de servidores están el proyecto apuntado y su cadena de herramientas, y la rama de Git con cuántos archivos has cambiado. Todo se lee del disco o de registros que el producto ya lleva: mirar no cuesta procesos. Mientras algo de lo que el IDE comprueba tenga un problema, un recuento **✕ 2 ⚠ 1** muestra los errores y avisos de todos los servidores de lenguaje y herramientas; pulsa en él para abrir Elementos de acción.
