...
**El árbol de archivos** es el Estudio de proyecto (⇧⌘E). Haz clic derecho en un archivo o una carpeta para Nuevo, Cortar, Copiar, Pegar, Eliminar y Renombrar y, como en el Explorer de VS Code, **Copiar ruta**, **Copiar ruta relativa** (relativa al proyecto) y **Mostrar en Finder** (**Mostrar en el Explorador de archivos** en Windows, **Abrir la carpeta contenedora** en Linux).
...
Junto al indicador, una nota más discreta responde a la pregunta que más te haces: ¿quién escribió la línea en la que está el cursor? `Ada Lovelace, hace 3 días · Fix the parser` sigue al cursor, en tu idioma, y su descripción emergente nombra el commit y la fecha. Un clic abre las anotaciones del archivo entero: el mismo **Anotar** que ofrece el indicador. La nota pregunta a git una vez por cada versión guardada del archivo, nunca al arrancar y nunca por un archivo fuera de un repositorio, y no dice nada de un archivo que git no sigue. Una línea que cambiaste y todavía no has confirmado dice **Todavía sin confirmar**. Mientras el archivo tenga cambios sin guardar, la nota dice **Autoría de la línea: cambios sin guardar** en lugar de un nombre, porque git lee el archivo tal como está guardado y la línea 40 de la pantalla no tiene por qué ser la línea 40 del disco; guarda y el nombre vuelve. **Ver ▸ Autoría de la línea** oculta la nota y la vuelve a mostrar.
