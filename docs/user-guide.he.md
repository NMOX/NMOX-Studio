...
ה‑IDE מעדכן את עצמו: **כלים ◂ תוספים ◂ עדכונים** מציע את המודולים של כל גרסה חדשה יותר. התקינו, הפעילו מחדש כשתתבקשו, וזהו — בלי להוריד מחדש את כל האפליקציה. מגבלה כנה: סביבת הריצה של Java המצורפת והמשגר משתנים רק עם מתקין מלא, ולכן בקפיצות גדולות של הפלטפורמה התקנה מחדש מקובץ שחרור היא עדיין הדרך הנכונה.

<a id="2-first-launch"></a>
## 2. הפעלה ראשונה

ממסוף, `nmoxstudio --open <תיקייה>` מפעיל את האפליקציה כשהתיקייה הזו פתוחה כפרויקט והראק מכוון אליה — אותה דלת ש"פתיחת תיקייה…" פותחת בחלון ברוכים הבאים.
...
![הפעלה ראשונה — חלון ברוכים הבאים עם שלוש לשוניות](images/tabs/workbench.png)
...
![ראק המשימות](images/tabs/the-task-rack.png)
...
![הראק מכוון אל אתר jQuery קלאסי — ההגדרה המוכנה ספסל הווב הקלאסי: MAESTRO, CRATE, DYNAMO (שחוגת ה‑TASK שלו קראה את ה‑Gruntfile האמיתי), IGNITION מגיש את האתר באופן סטטי, VITALS שומר על האיכות](images/task-rack.png)
...
- **הגדרות מוכנות** נותנות בלחיצה אחת ראק שלם ומחווט — שער השילוח, מודיעין פיתוח, מסלולי מונוריפו, לולאת E2E, ספסל LAMP, ספסל Web3, משמר זמינות. החיווט נשמר אוטומטית לכל פרויקט.

![Tab הופך את הראק — כבלים מובילים את MAESTRO דרך CRATE, DYNAMO ו‑IGNITION עד VITALS](images/rack-rear.png)
...
![KVASIR מסביר הרצה אמיתית שנכשלה: האבחון שאושר על הלוח הקדמי, והשלבים המלאים במציג](images/kvasir-explain.png)
...
![קוד jQuery בפלטת NMOX Phosphor, והמבנה בנווט](images/editor.png)
...
**עזרה ◂ אתר NMOX Studio (מקומי)** מגיש את אתר המוצר מתוך המוצר עצמו, בממשק המקומי. הוא מדבר את ארבע־עשרה השפות שה‑IDE מדבר; בורר השפה נמצא בתחתית הדף.
...
שדה אחד מגיע אל הפרויקטים שלכם (האחרונים והמוכרים), אל כל כלי בראק — כולל קפיצה לפקדים שלו —, אל **שרתים חיים** (Enter פותח אותם בדפדפן), אל הבקשות של אולפן ה‑API, אל החיבורים והטבלאות של אולפן מסדי הנתונים, אל החוזים, אל צמתי התשתית ואל הכרטיסים של לוח המשימות, שהתוצאות שלהם מציינות באיזו עמודה הם נמצאים.

### שורת המצב אומרת מה חי

לצד שבב השרתים נמצאים הפרויקט שמכוונים אליו עם שרשרת הכלים שלו, וענף ה‑git עם מספר הקבצים שהשתנו. כל זה נקרא מהדיסק או מרישומים שהמוצר מנהל ממילא — מבט אינו עולה אף תהליך.
...
כלים ◂ אפשרויות ◂ מיפוי מקשים מחליף את כל הפרופיל: קיצורי התנועה והגזירה של Emacs בכל עורך, או הסטים של Eclipse ושל IDEA, אם שם נמצא זיכרון השריר שלכם. כל קיצור של NMOX רשום בכל חמשת הפרופילים, כך שהחלפת פרופיל לעולם אינה עולה לכם בקיצורי האולפנים.
...
NMOX Studio מדבר ארבע־עשרה שפות: English, Español, Français, Deutsch, Русский, Українська, Polski, Português (Brasil), Bahasa Indonesia, Filipino, Tiếng Việt, 简体中文, हिन्दी ו־עברית. בעברית החלון כולו נפרש מימין לשמאל, לא רק הטקסט. בחרו את השפה שלכם ב‑**אפשרויות ◂ כללי ◂ שפה** — כל שפה מופיעה בשמה שלה, כדי שתמיד תמצאו את שלכם. הבחירה נכתבת להגדרות ההפעלה שלכם (`etc/nmoxstudio.conf`, כארגומנט `--locale`) ונכנסת לתוקף גם מיד. מה מתחלף: התפריטים, תיבות הדו־שיח, חלוניות העצות, שורות המצב, מסך ברוכים הבאים והאפשרויות. מה נשאר: אוצר המילים של חזיתות הראק (GO, STOP, EXPLAIN — תוויות חומרה, כמו על סינתיסייזר) ותיבות הדו־שיח העמוקות יותר של הפלטפורמה, שעדיין אין להן תרגום. ייתכן שלא תצטרכו לבחור בכלל: התקנה חדשה כבר מדברת בשפת המערכת שלכם, וגם ממדינה שמעולם לא ציינו — טייוואן, סינגפור, פורטוגל וקוויבק מגיעות לשפה שלהן ולא לאנגלית, כי הקטלוגים נקראים על שם שפה ואף פעם לא על שם מדינה.
