...
ממסוף, `nmox .`‎ פותח את התיקייה שבה אתם נמצאים, כמו ש‑`code .`‎ עושה: `cd myproject && nmox .`‎. התיקייה מקבלת כיוון בדיוק כמו ש"פתיחת תיקייה…" בחלון ברוכים הבאים מכוונת אותה, עם מניפסט או בלעדיו; קובץ נפתח בעורך (`nmox src/app.js`), ובשורה מסוימת אם מציינים אותה כמו ש‑`code -g` עושה (`nmox src/app.js:42` — גם עמודה מתקבלת, והעורך נפתח בתחילת השורה). שם שלא קיים נדחה במסוף (`nmox: typo.js: no such file or folder`) במקום להפעיל משהו. הדגל ‎`-r` של VS Code מתקבל, ו‑‎`-n` פותח בחלון היחיד; ‎`-a` ו‑‎`-v` נדחים בהודעה שמציינת את שמם. `nmox -w file` פותח את הקובץ ומחכה עד שתסגרו את הלשונית שלו, ו‑`nmox -d left right` משווה בין שני קבצים בתצוגת ההשוואה, וכך NMOX Studio נעשה העורך וכלי ה‑difftool של git: `git config --global core.editor "nmox -w"`‎, ו‑`nmox --help` מדפיס את השורות שהופכות אותו ל‑difftool ול‑mergetool. שמרו את ההודעה, סגרו את הלשונית, ו‑git ממשיך. כשה‑mergetool מוגדר (`mergetool.nmox.cmd` עם `nmox -w "$MERGED"`), הפקודה `git mergetool` פותחת בעורך כל קובץ שיש בו התנגשות. בקובץ הזה, ובכל קובץ שנושא את סימני ההתנגשות של git, הצד הנוכחי של כל התנגשות צבוע בגוון אחד והצד הנכנס בגוון אחר, ובשורה ‎`<<<<<<<`‎ יש אזהרה שהתיקון המהיר שלה (⌘. ב‑Mac, ‏Alt+Enter במקומות אחרים, או הנורה בשוליים) מציע את שלוש האפשרויות של VS Code: **קבלת השינוי הנוכחי**, **קבלת השינוי הנכנס** ו**קבלת שני השינויים**. כל אחת מהן היא עריכה אחת, ש‑⌘Z אחד מבטל; אם הבלוק השתנה אחרי שהאזהרה הופיעה, שום דבר לא מוחלף ושורת המצב אומרת זאת. בלוק שאינו בדיוק בצורה של git (מפריד שני, בלוק בתוך בלוק) לא מקבל שום הצעה, במקום ניחוש. הגוונים הם הערכים "התנגשות מיזוג" תחת אפשרויות ◂ גופנים וצבעים ◂ Highlighting. **צוות ◂ שימוש ב־NMOX Studio עם Git…** מציג את ההגדרות האלה לצד הערכים שיש להן עכשיו, וקובע אותן בשבילכם. הודעת commit נפתחת כקובץ של git עצמו: שורות ה‑`#` הן הערות, רק מה שאתם כותבים עובר בדיקת איות, ושורת סיכום שחורגת מ‑72 התווים שבהם הכלים של git חותכים מקבלת אזהרה. רשימת `git rebase -i` מדגישה כל פקודה וכל commit, ו"החלפת מצב הערה" מוציאה שורה משימוש בלי למחוק אותה. אחרת, הפקודה חוזרת מיד — ה‑`nmox` הראשון מפעיל את ה‑IDE ברקע, וכל `nmox` אחריו מעביר את התיקייה שלו ל‑IDE שכבר רץ. `nmox` לבדו רק מפעיל את ה‑IDE. כך מכניסים את `nmox` ל‑PATH:
...
**מיקומים במסוף נפתחים בלחיצת ⌘** (בלחיצת Ctrl ב‑Windows וב‑Linux). לחצו על מקום שכלי הדפיס, והקובץ נפתח בשורה הזו: הפלט של tsc `src/app.ts:42:7`, מסגרת מחסנית של Node או של Jest ‎`(/abs/app.js:10:5)`‎, הפלט של pytest `tests/test_x.py:12:`‎, הפלט של rustc ‎`--> src/main.rs:3:5`, ו‑traceback של Python `File "x.py", line 12`. נתיב מוחלט נפתח כפי שהודפס. נתיב יחסי נקרא מתיקיית הפרויקט שמכוונים אליו, שבה ⌃\` מפעיל את ה‑shell שלו; אחרי `cd` לתת־תיקייה, הנתיב עלול לציין קובץ שאינו שם, ושורת המצב אומרת איזה נתיב חיפשה במקום לנחש. כתובת URL או `host:port` אף פעם אינן קישור.
...
**עץ הקבצים** הוא אולפן הפרויקטים (⇧⌘E). לחיצה ימנית על קובץ או תיקייה מציעה חדש, גזירה, העתקה, הדבקה, מחיקה ושינוי שם, וגם — כמו ב‑Explorer של VS Code — **העתקת נתיב**, **העתקת נתיב יחסי** (יחסית לפרויקט) ו**הצגה ב-Finder** (**הצגה בסייר הקבצים** ב‑Windows, **פתיחת התיקייה המכילה** ב‑Linux). עבור הקובץ שאתם עורכים, **עריכה ◂ העתקת נתיב** (⌥⌘C, או Ctrl+Alt+C ב‑Windows וב‑Linux) וגם **עריכה ◂ העתקת נתיב יחסי** עושים אותו דבר — גם מהמסוף או מהעץ, בשביל העורך שבו הקלדתם לאחרונה.
...
קובץ README נקרא ב־GitHub, וקישור בו שלא מוביל לשום מקום מתגלה אצל הקורא הבא, לא אצלכם. **כלים ◂ בדיקת קישורי Markdown…** קורא כל קובץ Markdown בפרויקט שמכוונים אליו ובודק כל קישור וכל תמונה יחסיים כפי ש־GitHub יציג אותם: הקובץ או התיקייה שהם נוקבים בשמם חייבים להתקיים, ו־‎`#heading` חייב להיות כותרת של אותו קובץ לפי כלל העוגנים של GitHub (אותיות קטנות, בלי סימני פיסוק, רווחים הופכים למקפים, וכותרת שחוזרת ממוספרת ‎`-1`, ‎`-2`). קישור שלא מוביל לשום מקום הוא שגיאה; כותרת חסרה, וקישור שיוצא מהפרויקט, הם אזהרות. הם מגיעים כקווים גליים על הקישור וכשורות במשימות, ושורת המצב מסכמת את הריצה (*קישורי Markdown: ⁦12⁩ קבצים, ⁦148⁩ קישורים, קישור אחד אינו מוביל לשום מקום*).

קישורים עם סכמה (`https:`, `mailto:`‎) לא נבדקים: שום דבר לא יוצא מהמחשב שלכם. גם לא מה שנמצא בתוך בלוק קוד או קוד בתוך השורה, כי קישור בדוגמה הוא דוגמה, וגם לא הפרגמנט של קישור לקובץ מקור (`app.js#L10` הוא עוגן השורה של GitHub). קישור שמתחיל ב־‎`/`‎ נקרא משורש הפרויקט, כפי ש־GitHub קורא אותו.
...
**תצוגה ◂ מצב הצגה** מגדיל בבת אחת כל עורך, את הדף בדפדפן המובנה, את חלון Output ואת המסוף — ומשחזר הכול בדיוק ביציאה. **תצוגה ◂ הצגת הקשות** מציג בגדול את צירוף המקשים שנלחץ זה עתה, אבל לעולם לא את מה שאתם מקלידים. **עריכה ◂ העתקה כ‑Markdown** מעתיקה את הבחירה כבלוק מגודר עם תג השפה הנכון, והגרסה **העתקה כ‑Markdown עם קישור** מוסיפה את הקישור ל‑GitHub לאותן שורות. **עריכה ◂ פתיחה ב‑GitHub** ו**עריכה ◂ העתקת קישור ל‑GitHub** (גם בלחיצה ימנית, בעורך ובעץ של אולפן הפרויקטים) נותנות את אותו קישור בלי הבלוק: שורת הסמן או שורות הבחירה, קובץ, או תיקייה (דף ה‑`tree` שלה; תיקיית השורש של הפרויקט היא דף הבית של המאגר). הפתיחה נעשית בדפדפן שלכם, שבו אתם כבר מחוברים, כך ש‑blame והערות הסקירה עובדים. קובץ מחוץ למאגר git, מאגר בלי `origin` או origin שאינו GitHub נדחים בשורת המצב ושום דבר לא מועתק — וכך גם עורך עם שינויים שלא נשמרו, כי הקישור היה מציג שורות שונות מאלה שאתם רואים. **צוות ◂ בקשת משיכה חדשה ב‑GitHub** (גם בתפריט של השבב) פותח בדפדפן שלכם את דף ה‑New Pull Request של GitHub עצמו, עבור הענף שיש לכם ב‑checkout: הצעד שאחרי push, בלי לחפש ידנית את המאגר ואת הענף. מצב של HEAD מנותק נדחה, כי אין בו ענף להציע. **כלים ◂ שמירת צילום מסך…** מצייר את כל החלון בגודל כפול, עם גרסאות ללשונית העורך לבדה, ללוח הגזירים ולעץ הפרויקט כ‑Markdown.
...
אחרי השבב, ‎`↑2 ↓1` הם ה‑commits שיש בענף שלכם ואין ב‑upstream שלו (לדחיפה) ולהפך (למשיכה), והם מוצגים רק כשהם אינם אפס ורק לענף שיש לו upstream. התפריט שלו מתחיל ב‑**החלפת ענף…**, ב‑**ביצוע commit…**, ב‑**משיכה (pull)…** וב‑**דחיפה (push)…**, תיבות הדו‑שיח של מודול git עצמו.

לצד השבב, הערה שקטה יותר עונה על השאלה שאתם שואלים הכי הרבה: מי כתב את השורה שהסמן עומד עליה? הטקסט `Ada Lovelace, לפני 3 ימים · Fix the parser` עוקב אחרי הסמן, בשפה שלכם, והרמז שצץ מעליו מציין את ה‑commit ואת התאריך. לחיצה עליו מציעה **הצגת ייחוס שורות** (של הקובץ כולו, אותו **ייחוס שורות** שהשבב מציע), **פתיחת הקומיט ב‑GitHub** (הדף של הקומיט עצמו, כש‑origin הוא מאגר מרוחק ב‑GitHub) או **העתקת מזהה הקומיט**; בשורה שעוד לא נכנסה ל‑commit הלחיצה פותחת ישר את הייחוס. ההערה שואלת את git פעם אחת לכל גרסה שמורה של הקובץ, אף פעם לא בהפעלה ואף פעם לא בשביל קובץ שמחוץ למאגר, ואינה אומרת דבר על קובץ ש‑git לא עוקב אחריו. שורה ששיניתם ועוד לא נכנסה ל‑commit מופיעה כ**עדיין לא בקומיט**. כל עוד יש בקובץ שינויים שלא נשמרו, ההערה מציגה **מחבר השורה: יש שינויים שלא נשמרו** במקום שם, כי git קורא את הקובץ כפי שהוא שמור, ושורה 40 על המסך אינה בהכרח שורה 40 בדיסק; שמרו והשם חוזר. **תצוגה ◂ מחבר השורה** מכבה ומדליקה את ההערה.
...
**חיפוש בפרויקטים (⇧⌘F) מחפש בקוד שלכם, לא במה שהמאגר מתעלם ממנו.** במאגר git הוא מדלג על כל מה שקובצי ה‑‎`.gitignore` של המאגר עצמו (זה שבשורש וכל אלה שבתיקיות המשנה) ו‑‎`.git/info/exclude` מתעלמים ממנו, וגם על ‎`.git` עצמו — כך שחיפוש של שם פונקציה מוצא את `src/`‎, ולא את העותקים שב‑`node_modules` וב‑`dist/`‎ שה‑‎`.gitignore` של התבניות מפרט. הוא אף פעם לא מדלג מעבר למה ש‑git מתעלם ממנו: תיקייה שהמאגר שלכם עוקב אחריה נסרקת, יהיה שמה אשר יהיה. מחוץ למאגר הוא מדלג גם על תיקיות הבנייה והחבילות שכל סריקה במוצר מדלגת עליהן (`node_modules`, `dist`, `build`, `out`, `target`, `coverage`, `.next`, `.nuxt`, `.svelte-kit`, `.angular`, `.venv`, `__pycache__`). התיקיות נשארות בעץ הפרויקט; רק החיפוש עובר על פניהן. כדי לחפש בהן בכל זאת, סמנו את **חיפוש במקורות שנוצרו** בתיבת הדו־שיח של חיפוש בפרויקטים (הבחירה נשמרת עד שתבטלו את הסימון); החלפה בפרויקטים לא מציעה תיבה כזאת, ולעולם לא משכתבת את מה שהמאגר מתעלם ממנו. קובץ ההחרגות הגלובלי של git שלכם לא נקרא, ולכן נתיב שרק הוא מתעלם ממנו עדיין נסרק.
