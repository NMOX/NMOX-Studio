...
**Cây tệp** là Studio dự án (⇧⌘E). Nhấp chuột phải vào một tệp hoặc thư mục để có Mới, Cắt, Sao chép, Dán, Xóa và Đổi tên, và — như Explorer của VS Code — **Sao chép đường dẫn**, **Sao chép đường dẫn tương đối** (tương đối so với dự án) và **Hiện trong Finder** (**Hiện trong File Explorer** trên Windows, **Mở thư mục chứa** trên Linux). Với tệp bạn đang sửa, **Chỉnh sửa ▸ Sao chép đường dẫn** (⌥⌘C, Ctrl+Alt+C trên Windows và Linux) và **Chỉnh sửa ▸ Sao chép đường dẫn tương đối** làm đúng như vậy — kể cả từ Terminal hay cây tệp, cho trình soạn thảo bạn gõ phím gần nhất.
...
Bên cạnh dấu nhánh, một ghi chú kín đáo hơn trả lời câu hỏi bạn hỏi thường xuyên nhất: ai đã viết dòng mà con trỏ đang ở? `Ada Lovelace, 3 ngày trước · Fix the parser` đi theo con trỏ, bằng ngôn ngữ của bạn, và chú thích nổi của nó nêu commit và ngày. Một cú nhấp đưa ra **Hiện chú giải** (chú giải của cả tệp, đúng mục **Chú giải** mà dấu nhánh đưa ra), **Mở commit trên GitHub** (trang riêng của commit đó, khi origin là một remote GitHub) hoặc **Sao chép mã commit**; trên một dòng chưa commit, cú nhấp đi thẳng tới chú giải. Ghi chú hỏi git một lần cho mỗi phiên bản đã lưu của tệp, không bao giờ lúc khởi động và không bao giờ cho một tệp nằm ngoài kho, và không nói gì với tệp mà git không theo dõi. Một dòng bạn đã sửa nhưng chưa commit sẽ hiện **Chưa commit**. Khi tệp còn thay đổi chưa lưu, ghi chú hiện **Tác giả dòng: có thay đổi chưa lưu** thay cho một cái tên, vì git đọc tệp như nó đã được lưu, và dòng 40 trên màn hình chưa chắc là dòng 40 trên đĩa; lưu lại là cái tên quay về. **Xem ▸ Tác giả dòng** tắt và bật ghi chú.
...
**Tìm trong dự án (⇧⌘F) tìm trong mã của bạn, không tìm trong những gì kho bỏ qua.** Trong một kho git, nó bỏ qua mọi thứ mà chính các tệp `.gitignore` của kho (tệp ở gốc và mọi tệp lồng trong thư mục con) và `.git/info/exclude` bỏ qua, cùng với chính `.git` — vì vậy tìm tên một hàm sẽ thấy `src/`, không phải các bản sao trong `node_modules` và `dist/` mà `.gitignore` của các mẫu liệt kê. Nó không bao giờ bỏ qua nhiều hơn những gì git bỏ qua: một thư mục mà kho của bạn theo dõi vẫn được tìm, dù tên là gì. Bên ngoài một kho, nó còn bỏ qua các thư mục build và gói mà mọi lượt duyệt trong sản phẩm đều bỏ qua (`node_modules`, `dist`, `build`, `out`, `target`, `coverage`, `.next`, `.nuxt`, `.svelte-kit`, `.angular`, `.venv`, `__pycache__`). Các thư mục vẫn nằm trong cây dự án; chỉ có việc tìm kiếm đi ngang qua chúng. Để vẫn tìm trong đó, hãy đánh dấu **Tìm trong nguồn được sinh ra** trong hộp thoại Tìm trong dự án (lựa chọn được nhớ cho đến khi bạn bỏ dấu); Thay thế trong dự án không có ô đó và không bao giờ viết lại những gì kho bỏ qua. Tệp loại trừ git toàn cục của bạn không được đọc, nên một đường dẫn chỉ nó bỏ qua vẫn được tìm.
