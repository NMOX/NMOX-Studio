...
class VendorResourcesTest {
     ...
     * {@code version="3.5.1"} at ~offset 1060) — which is why the scan
     * window is 2 KB, not the banner-only 200 bytes.
     */
    ...
    private static final int SCAN_WINDOW = 2048;
    ...
    void bundledBuildsCarryTheirMarkers() throws Exception {
        for (Map.Entry<String, List<String>> pin : MARKERS.entrySet()) {
            ...
            String head = new String(bytes, 0, Math.min(bytes.length, SCAN_WINDOW),
