...
/**
 * Ledger 29 (v1.45.0), studio half: while Project Studio is showing, its
 * activated nodes carry the aimed directory's DataFolder node. Same
 * contract as {@code RackAimSelectionTest} — the three aim-following
 * windows publish identically, so the global selection is consistent
 * whichever one is active.
 */
class ProjectStudioAimSelectionTest {
    ...
    @Test
    @DisplayName("showing studio publishes the aim; re-aim updates; close detaches")
    void aimBecomesTheActivatedNode(@TempDir Path a, @TempDir Path b, @TempDir Path c)
        ...
        assertThat(tc[0].getActivatedNodes())
                .as("a closed studio neither publishes nor pins the old DataObject")
                .isEmpty();
    }
}
