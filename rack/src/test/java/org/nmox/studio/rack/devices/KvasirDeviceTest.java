...
/**
 * KVASIR's gates, mutation-proven. {@code consult} is the single path to
...
class KvasirDeviceTest {
    ...
    private static KvasirDevice wired(SpyTransport spy) {
        KvasirDevice device = new KvasirDevice();
        device.client = new KvasirClient(spy);
    ...
    void consultSeedsConversation() {
        ...
        // (byte parity with assemblePrompt is pinned in KvasirConversationTest)
        assertThat(convo.history().get(0).text())
                .isEqualTo(org.nmox.studio.rack.engine.KvasirConversation
    ...
    void consentGateBlocksTheCall() {
        SpyTransport spy = new SpyTransport();
        KvasirDevice device = wired(spy); // key present
    ...
    void cableCooldown() {
        ...
        now[0] = KvasirDevice.AUTO_COOLDOWN_MS - 1;
