...
/**
 * One KVASIR conversation about one fixed subject — a code selection
 ...
 */
public final class KvasirConversation {
    ...
    public KvasirConversation(CodeQuestion subject) {
        this.subject = subject;
        this.title = subject.fileName();
        this.openingTurn = userText -> KvasirClient.assembleCodePrompt(
                new CodeQuestion(subject.fileName(), subject.language(),
                        subject.code(), userText));
    }

    private KvasirConversation(String title, Function<String, String> openingTurn) {
        this.subject = null;
        this.title = title;
     ...
     * opening turn is EXACTLY {@link KvasirClient#assemblePrompt} — the
     ...
     */
    public static KvasirConversation forFailure(FailureContext ctx) {
        return new KvasirConversation(ctx == null ? "(no failure)" : ctx.device(),
                userText -> ctx == null ? "" : KvasirClient.assemblePrompt(ctx));
    }

    /**
     * A conversation about text a STUDIO assembled and already redacted
     * (the {@code core.spi.KvasirAsk} flows — API Studio's response
     ...
     */
    public static KvasirConversation forDisclosure(String title, String body) {
    ...
    public List<Turn> outgoing(String userText) {
        ...
        if (out.isEmpty()) {
            ...
            if (text.length() > MAX_FOLLOW_UP_CHARS) {
                text = KvasirClient.CodeQuestion.truncate(text, MAX_FOLLOW_UP_CHARS);
            }
