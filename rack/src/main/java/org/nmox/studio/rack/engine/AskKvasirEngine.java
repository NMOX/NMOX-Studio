...
/**
 * The Ask KVASIR consult body — the editor twin of KvasirDevice's
 ...
 */
public final class AskKvasirEngine {
    ...
    private final Predicate<KvasirConversation> consentGate;

    /** Production wiring is supplied by the caller; tests inject spies. */
    public AskKvasirEngine(KvasirClient client, Supplier<char[]> keySource,
            Predicate<KvasirConversation> consentGate) {
        this.client = client;
     ...
     */
    public Result converse(KvasirConversation convo, String userText, String model) {
    ...
    private Result converseLocked(KvasirConversation convo, String userText, String model) {
        if (!convo.hasSubject()) {
        ...
        if (!convo.canAsk()) {
            return new Result(Status.FAILED,
                    "Conversation cap reached (" + KvasirConversation.MAX_EXCHANGES
        ...
        try {
            ...
            return new Result(Status.FAILED, "KVASIR could not answer: " + e.getMessage());
        } finally {
            if (key != null) {
                Arrays.fill(key, '\0');
            }
    ...
    public Result answer(CodeQuestion q, String model) {
        ...
        return converse(new KvasirConversation(q), q.question(), model);
    }
