...
/**
 * The Edit with KVASIR consult body — {@link AskKvasirEngine}'s editing
 ...
 * {@link KvasirEdit#extractFencedCode} and anything that is not exactly
 ...
 */
public final class KvasirEditEngine {
    ...
    public KvasirEditEngine(KvasirClient client, Supplier<char[]> keySource,
     ...
     * KVASIR flow, so a decline provably sends nothing.
     */
    public Proposal propose(EditRequest r, String model) {
        ...
        try {
            ...
            String reply = client.converse(
                    List.of(new Turn("user", KvasirEdit.assembleEditPrompt(r))),
                    model, key);
            String extracted = KvasirEdit.extractFencedCode(reply);
            String replacement = extracted == null ? null
                    : KvasirEdit.matchTrailingNewline(r.code(), extracted);
            if (replacement == null) {
