...
/**
 * The pure core of Edit with KVASIR — the flow that turns a selection
 ...
 *
 * <p>Two laws differ from Ask KVASIR on purpose. First, an over-cap
 ...
 */
public final class KvasirEdit {

    private KvasirEdit() {
    }

    /** Same ceiling as Ask KVASIR's disclosure — but enforced by refusal. */
    public static final int MAX_CODE_CHARS = KvasirClient.CodeQuestion.MAX_CODE_CHARS;
        ...
        public EditRequest {
            ...
            if (code.length() > MAX_CODE_CHARS) {
                ...
                throw new IllegalArgumentException("Selection too large for an "
                        + "KVASIR edit (" + code.length() + " chars, cap "
                        + MAX_CODE_CHARS + ").");
            }
     ...
     * {@code original} there — the file may have changed while KVASIR was
