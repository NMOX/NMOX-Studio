...
/**
 * NIMBUS Nuxt Console: the Vue meta-framework's dev/build loop through
 * its nuxi CLI. DEV serves with the URL out feeding SCOPE, BUILD
 * compiles production, PREVIEW serves that build, and DIAG runs nuxi
 * typecheck - while the version cluster tracks the installed nuxt
 * against the registry so the rack nags when the framework moves.
 */
public class NuxtDevice extends CommandDevice {
    ...
    public NuxtDevice() {
        ...
        diag.addActionListener(e -> launch(List.of("npx", "nuxi", "typecheck")));
    ...
    private void refreshVersions() {
        String raw = ProjectInspector.dependencyVersion(projectDir(), "nuxt");
        installedVersion = raw == null ? null : AngularVersions.clean(raw);
        showVersions();
        if (installedVersion == null) {
            onEdt(() -> statusLcd.setText("NO nuxt IN package.json"));
            return;
        }
    ...
    private void showVersions() {
        ...
        onEdt(() -> {
            ...
            versionLcd.setText("nuxt " + (installed == null ? "?" : installed)
