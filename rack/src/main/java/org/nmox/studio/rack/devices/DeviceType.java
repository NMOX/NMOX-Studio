...
public enum DeviceType {
    ...
    public String getUsage() {
        return switch (this) {
            ...
            // "on OPEN" only: receive() handles exactly the open jack — the old
            // "or any trigger in" described a jack that does not exist (2026-09-17)
            case BROWSER -> "Opens the system browser at the dialed URL when OPEN fires (patch a READY trigger in).\nPatch a URL data jack in and SCOPE follows wherever the server actually is.";
            ...
            case KVASIR -> "EXPLAIN reads the last failed run off the flight recorder and asks your AI — Claude (Anthropic), ChatGPT (OpenAI) or Gemini (Google) — what went wrong and how to fix it — QUERY-blue, no project mutation.\nKEY… picks the provider and sets its key (OS keychain, or ANTHROPIC_API_KEY/CLAUDE_API_KEY, OPENAI_API_KEY/CHATGPT_API_KEY, GEMINI_API_KEY/GOOGLE_API_KEY); first press asks a one-time consent per provider naming exactly what's sent; MODEL dials FAST or DEEP; VIEW opens the full answer.";
        };
    }
