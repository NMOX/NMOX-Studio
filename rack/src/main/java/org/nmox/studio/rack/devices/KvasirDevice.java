 ...
 */
public class KvasirDevice extends RackDevice {
    ...
    private static final String THINKING = "CONSULTING KVASIR…";
    private static final String OFFLINE = "OFFLINE — COULD NOT REACH KVASIR";
    ...
    Supplier<char[]> keySource = KvasirKeys::read;
    /** Whether consent is already granted (no prompt). Tests inject. */
    BooleanSupplier consentCheck = KvasirConsent::isGranted;
    ...
    private volatile org.nmox.studio.rack.engine.KvasirConversation lastConversation;
    private volatile KvasirClient.FailureContext lastFailure;
    /** EDT-confined: the open conversation window, so VIEW fronts not twins. */
    private javax.swing.JDialog openConversationDialog;
    private org.nmox.studio.rack.engine.KvasirConversation openDialogConvo;
    ...
    private void onExplain() {
        ...
        offEdt(() -> {
            try {
                ...
                if (!consent) {
                    consent = KvasirConsent.requestConsent(maybe.get());
                    if (!consent) {
    ...
    String consult(boolean consentGranted) {
        ...
        try {
            ...
            org.nmox.studio.rack.engine.KvasirConversation convo =
                    org.nmox.studio.rack.engine.KvasirConversation.forFailure(maybe.get());
            convo.record("", text);
    ...
    org.nmox.studio.rack.engine.KvasirConversation lastConversationForTest() {
    ...
    private void showFullText() {
        ...
        org.nmox.studio.rack.engine.KvasirConversation convo = lastConversation;
        KvasirClient.FailureContext ctx = lastFailure;
        if (convo != null && ctx != null) {
            ...
            org.nmox.studio.rack.engine.AskKvasirEngine engine =
                    new org.nmox.studio.rack.engine.AskKvasirEngine(
                            client, keySource,
                            c -> org.nmox.studio.rack.service.KvasirConsent.requestConsent(ctx));
            openConversationDialog = new org.nmox.studio.rack.service.AskKvasirDialog(
        ...
        area.getAccessibleContext().setAccessibleName("KVASIR explanation");
