...
- **v1.62.0**: the Tailwind + journey release — TailwindServer LSP on text/css//x-scss//html gated by `usesTailwind` (config file or tailwindcss dep incl. v4 CSS-first; malformed manifests opt out safely; mutation-proven), running alongside the CSS server (LSPBindings binds ALL providers per mime — decompiled proof); Language Servers catalog + Doctor row (61 tools); journey findings from a live create→serve walk: wizard first-run default unified to ~/NMOX (recent location still wins), wizard install manager-aware with the pre-trust blessing written in place
...
- `docs/devices.md` - The 45-device reference (GENERATED from DeviceType by DeviceDocsTest; CI fails on drift)
