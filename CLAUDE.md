...
- **v1.65.0**: VELOCITY — the Vite console (48th device), mirroring NEXUS/HALO for the tool under most modern React/Vue/Svelte/Solid SPAs: DEV (serve + URL→SCOPE, READY/SERVING), BUILD (production bundle), PREVIEW (serves the built dist), and vite-vs-registry version currency. DeviceType FRAMEWORKS entry, CI-step capable, FrameworkDeviceTest coverage, devices.md regenerated; IGNITION still serves Vite generically for the un-opinionated case
...
- **v1.66.0**: COSMOS — the Astro console (48th device): DEV (serve + URL→SCOPE, READY/SERVING), BUILD (static site), PREVIEW (serves dist), DIAG (astro check), astro-vs-registry version currency, onFinished serving-deregister built in; FRAMEWORKS palette + CI-step; FrameworkDeviceTest + ServingDevicesTest; devices.md regenerated
