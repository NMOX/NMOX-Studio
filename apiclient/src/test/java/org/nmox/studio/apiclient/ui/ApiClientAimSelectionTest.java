...
/**
 * Tech-debt ledger 72, closed v2.186.0: API Studio publishes the aimed
 * project's node as its ambient selection, so the platform's
 ...
 */
class ApiClientAimSelectionTest {
    ...
    private static File selectedDir(javax.swing.JComponent window) throws Exception {
        File[] out = {null};
        SwingUtilities.invokeAndWait(() -> {
            Node[] nodes = ((ApiClientTopComponent) window).getActivatedNodes();
    ...
    void publishesWhileShowingOnly(@TempDir File aimA, @TempDir File aimB) throws Exception {
        ApiClientTopComponent[] holder = new ApiClientTopComponent[1];
        SwingUtilities.invokeAndWait(() -> holder[0] = new ApiClientTopComponent());
        ApiClientTopComponent tc = holder[0];
